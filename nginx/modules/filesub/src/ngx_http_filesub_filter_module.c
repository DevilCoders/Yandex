/*
 * mostly copied from contrib/nginx/core/src/http/modules/ngx_http_sub_filter_module.c
 * changed parts are names of commands (filesub_filter) and parameter initializing (see ngx_http_sub_filter)
 */
...
int NGX_FILESUB_MAX_LENGTH = 256*1024;
ngx_str_t STUB_FILE_NAME = ngx_string("http_filesub_filter: substitution file");
...
static ngx_command_t  ngx_http_sub_filter_commands[] = {
    ...
    { ngx_string("filesub_filter_types"),
    ...
    { ngx_string("filesub_filter_once"),
    ...
    { ngx_string("filesub_filter_last_modified"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
...
ngx_module_t  ngx_http_filesub_filter_module = {
    NGX_MODULE_V1,
...
ngx_http_sub_header_filter(ngx_http_request_t *r)
{
    ...
    slcf = ngx_http_get_module_loc_conf(r, ngx_http_filesub_filter_module);
    ...
    ngx_http_set_ctx(r, ctx, ngx_http_filesub_filter_module);
...
ngx_http_sub_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ctx = ngx_http_get_module_ctx(r, ngx_http_filesub_filter_module);
...
ngx_http_sub_filter(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_file_t                         sub_value_file;
    int                                nread=-1, buf_offset=0;
    u_char                            *buf;
    ...
    ngx_memzero(&sub_value_file, sizeof(ngx_file_t));
    sub_value_file.name = STUB_FILE_NAME;
    sub_value_file.log = cf->log;

    sub_value_file.fd = ngx_open_file((const char *) pair->value.value.data, NGX_FILE_RDONLY, NGX_FILE_OPEN, 0);

    if (sub_value_file.fd == NGX_INVALID_FILE) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "http_filesub_filter: open() failed");

        return NGX_CONF_ERROR;
    }

    buf = ngx_pnalloc(cf->pool, NGX_FILESUB_MAX_LENGTH);

    while((nread = ngx_read_file(&sub_value_file, buf + buf_offset, NGX_FILESUB_MAX_LENGTH, buf_offset)) > 0) {
        if (buf_offset + nread > NGX_FILESUB_MAX_LENGTH) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                               "file too long ");
            ngx_free(buf);
            ngx_close_file(sub_value_file.fd);
            return NGX_CONF_ERROR;
        }
        buf_offset += nread;
    }
    ...
    if (nread == -1) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "http_filesub_filter: read() failed");
        ngx_free(buf);
        ngx_close_file(sub_value_file.fd);
        return NGX_CONF_ERROR;
    }

    if (ngx_close_file(sub_value_file.fd) == NGX_FILE_ERROR) {
        ngx_conf_log_error(NGX_LOG_ALERT, cf, ngx_errno,
                      "http_filesub_filter: ngx_close_file failed");
    }

    pair->value.value.data = buf;
    pair->value.value.len = buf_offset;
