...
! function ($) {
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        treeVerticalEmptycls: 'vertical-empty',
        treeSpacecls: 'space',
        treeNodecls: 'node',
        ...
        onTreeFormatter: function (row) {
            ...
            for (var i = 0; i < plevel; i++) {
                hierarchy.unshift(hierarchy[0]._parent);
            }
            for (var i = 0; i < plevel; i++) {
                if (hierarchy[i]._last) {
                    paddings.push('<i class="' + options.treeVerticalEmptycls + '"></i>');
                } else {
            ...
            $.each(this.header.fields, function (i, field) {
                if (treeShowField === field) {
                    ...
                    that.header.formatters[i] = function (value, row, index) {
                        ...
                        colTree.push('<span class="' + that.options.treeTextcls + '" style="float: none">');
                        if (_formatter) {
