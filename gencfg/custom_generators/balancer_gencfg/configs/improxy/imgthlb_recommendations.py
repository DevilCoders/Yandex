...
class Project(object):
    ...
    def _generateThumbSpreaderConfig(self, config):
        return [
    ...
    def _generateBanConfig(self, config):
        ...
        backends = self._set_group_topology('{}_RECOMMENDATIONS_BALANCER_BAN'.format(location.upper()))
                    ...
                    (Modules.Balancer, {
                        ...
                        'on_error': [
                            (Modules.Balancer, {
                                ...
                                'stats_attr': "{}_ban".format(self.BACKEND_STATS_ATTR),
                                'proxy_options': OrderedDict([
                                    ('connect_timeout', LuaGlobal('ban_connect_timeout', '5ms')),
                                    ('backend_timeout', LuaGlobal('ban_backend_timeout', '10ms')),
    ...
    def _config(
        ...
        for index_type, regexp_config in thdb_func(IndexTypes, Backends, ExperimentalBackends, production_locations,
            ...
            regexp_config = self._generateThumbSpreaderConfig(regexp_config)
            # response headers
            webHeadersOpts = dict()
    ...
    def _balancer(self):
        ...
        project_domains = ['*']
        if self.DOMAIN != '*':
            project_domains.append(self.DOMAIN)
...
class Images(Project):
    ...
    def __init__(self, location, transport, topology='trunk'):
        ...
        # TUN SLB scheme
        self.DOMAIN = 'imtub-itditp.yandex-team.ru'
        self.THUMBS_BAN = True
        self.REUSE_PORT = False
        self.WEBP = True
