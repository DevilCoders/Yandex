 ...
 */
@SuppressWarnings({"UnnecessaryAnonymousClass", "MissingOverride", "deprecation"})
public class EphemeralSharedValue implements Closeable, SharedValueReader
{
     ...
     */
    public EphemeralSharedValue(CuratorFramework client, String path, byte[] seedValue)
    {
    ...
    @VisibleForTesting
    protected EphemeralSharedValue(WatcherRemoveCuratorFramework client, String path, byte[] seedValue, CuratorWatcher watcher)
    {
        this.client = client;
        this.path = PathUtils.validatePath(path);
        this.seedValue = Arrays.copyOf(seedValue, seedValue.length);
    ...
    public void start() throws Exception
    {
        ...
        try
        {
            client.create().creatingParentContainersIfNeeded().withMode(CreateMode.EPHEMERAL).forPath(path, seedValue);
        }
                ...
                (
                        new Function<SharedValueListener, Void>()
                        {
                            ...
                            public Void apply(SharedValueListener listener)
                            {
                                try
                                {
                                    listener.valueHasChanged(EphemeralSharedValue.this, localValue);
                                }
                                catch ( Exception e )
                                {
                                    ThreadUtils.checkInterrupted(e);
