...
class Check:
    ...
    def just_comment(self):
        logging.info(
            "Just commenting %s action at %s %s %s",
            self.status,
            self.ticket_id,
            self.company_type,
            self.company_plan,
    ...
    def resolve(self):
        ...
        if self.status == "waitForAnswer":
            if not self.is_a_good_time(now, 10, 19, [0, 1, 2, 3, 4]):
                return

            crit_delta = 7 * 24 * 3600
        ...
        elif self.status == "watchingByDevelopers":
            if not self.is_a_good_time(now, 10, 19, [0, 1, 2, 3, 4]):
                return
