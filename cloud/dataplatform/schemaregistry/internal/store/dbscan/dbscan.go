...
func ScanAll(dst interface{}, rows Rows) error {
	return processRows(dst, rows, true /* multipleRows */)
}
...
func ScanOne(dst interface{}, rows Rows) error {
	return processRows(dst, rows, false /* multipleRows */)
}
...
var errNotFound = xerrors.New("scany: no row was found")
...
func processRows(dst interface{}, rows Rows, multipleRows bool) error {
	...
	if multipleRows {
		...
		if err != nil {
			return xerrors.Errorf("parse slice destination: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return xerrors.Errorf("scany: rows final error: %w", err)
	}

	if err := rows.Close(); err != nil {
		return xerrors.Errorf("scany: close rows after processing: %w", err)
	}

	exactlyOneRow := !multipleRows
	if exactlyOneRow {
		if rowsAffected == 0 {
			return xerrors.Errorf("%w", errNotFound)
		} else if rowsAffected > 1 {
			return xerrors.Errorf("scany: expected 1 row, got: %d", rowsAffected)
		}
	}
	return nil
}
...
func parseSliceDestination(dst interface{}) (*sliceDestinationMeta, error) {
	...
	if err != nil {
		return nil, xerrors.Errorf("parse destination: %w", err)
	}
...
func (rs *RowScanner) Scan(dst interface{}) error {
	...
	if err != nil {
		return xerrors.Errorf("parse destination:%w", err)
	}
	return rs.doScan(dstVal)
}
...
func parseDestination(dst interface{}) (reflect.Value, error) {
	...
	if !dstVal.IsValid() || (dstVal.Kind() == reflect.Ptr && dstVal.IsNil()) {
		return reflect.Value{}, xerrors.Errorf("scany: destination must be a non nil pointer")
	}
	if dstVal.Kind() != reflect.Ptr {
		return reflect.Value{}, xerrors.Errorf("scany: destination must be a pointer, got: %v", dstVal.Type())
	}
...
func (rs *RowScanner) doScan(dstValue reflect.Value) error {
	if !rs.started {
		if err := rs.start(rs, dstValue); err != nil {
			return xerrors.Errorf("do scan: %w", err)
		}
		rs.started = true
	}
	var err error
...
func startScanner(rs *RowScanner, dstValue reflect.Value) error {
	...
	if err != nil {
		return xerrors.Errorf("scany: get rows columns: %w", err)
	}
	if err := rs.ensureDistinctColumns(); err != nil {
		return xerrors.Errorf("distinct cols: %w", err)
	}
...
func (rs *RowScanner) scanStruct(structValue reflect.Value) error {
	...
	if err != nil {
		return xerrors.Errorf("scany: scan row into struct fields: %w", err)
	}
	return nil
}
...
func (rs *RowScanner) scanMap(mapValue reflect.Value) error {
	...
	if err := rs.rows.Scan(scans...); err != nil {
		return xerrors.Errorf("scany: scan rows into map: %w", err)
	}
...
func (rs *RowScanner) scanPrimitive(value reflect.Value) error {
	return rs.rows.Scan(value.Addr().Interface())
}
...
func (rs *RowScanner) ensureDistinctColumns() error {
	...
	for _, column := range rs.columns {
		if _, ok := seen[column]; ok {
			return xerrors.Errorf("scany: rows contain a duplicate column '%s'", column)
		}
		seen[column] = struct{}{}
	}
	return nil
