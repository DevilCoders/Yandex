...
func TestHTTPGetSchema(t *testing.T) {
	...
	t.Run("should validate version number", func(t *testing.T) {
		...
		req, _ := http.NewRequest("GET", fmt.Sprintf("/v1/namespaces/%s/schemas/%s/versions/invalidNumber", nsName, schemaName), nil)
		mux.ServeHTTP(w, req)
		assert.Equal(t, 500, w.Code)
		assert.JSONEq(t, `{"code":2,"message":"invalid version number","error":"invalid version number"}`, w.Body.String())
	})
	t.Run("should return http error if getSchema fails", func(t *testing.T) {
		...
		w := httptest.NewRecorder()
		req, _ := http.NewRequest("GET", fmt.Sprintf("/v1/namespaces/%s/schemas/%s/versions/%d", nsName, schemaName, version), nil)
		mux.ServeHTTP(w, req)
		assert.Equal(t, 500, w.Code)
		assert.JSONEq(t, `{"code":2,"message":"get error","error":"get error"}`, w.Body.String())
	})
...
func TestHTTPSchemaCreate(t *testing.T) {
	...
	t.Run("should return error if schema create fails", func(t *testing.T) {
		...
		w := httptest.NewRecorder()
		req, _ := http.NewRequest("POST", fmt.Sprintf("/v1/namespaces/%s/schemas/%s", nsName, scName), bytes.NewBuffer(body))
