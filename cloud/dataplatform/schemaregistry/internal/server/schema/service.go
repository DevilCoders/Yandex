...
func (s *Service) Diff(ctx context.Context, nsName, schemaName string, data []byte) ([]string, error) {
	ns, err := s.NamespaceSvc.Get(ctx, nsName)
	if err != nil {
		return nil, err
	}
	...
	checkerFn := getCompatibilityChecker("DIFF")
	err = checkerFn(parsedSchema, []ParsedSchema{prevSchema})
	if err != nil {
		if dErr, ok := err.(diffErr); ok {
			return dErr.Diffs(), nil
		}
	}
	return nil, nil
}
...
func getIDforSchema(ns, schema, dataUUID string) string {
	...
	return uuid.NewV3(uuid.FromStringOrNil("6ba7b812-9dad-11d1-80b4-00c04fd430c8"), key).String()
}
