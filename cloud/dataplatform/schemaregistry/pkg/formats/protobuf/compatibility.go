...
func compareSchemas(current, prev *protoregistry.Files, notAllowedChanges []diffKind) error {
	...
	prev.RangeFiles(func(fd protoreflect.FileDescriptor) bool {
		if strings.HasPrefix(string(fd.FullName()), "google") {
			return true
		}
...
func compareMessages(current, prev protoreflect.MessageDescriptor, diffs *compatibilityErr) {
	...
	for i := 0; i < current.Fields().Len(); i++ {
		currentField := current.Fields().Get(i)
		prevField := prev.Fields().ByNumber(currentField.Number())
		if prevField == nil {
			diffs.add(fieldAdd, current, `field "%s" added to message "%s"`, currentField.Name(), current.Name())
		}
	}
