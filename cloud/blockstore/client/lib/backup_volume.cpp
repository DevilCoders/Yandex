...
protected:
    ...
    bool DescribeVolume()
    {
        ...
        BlocksCount = response.GetVolume().GetBlocksCount();
        SrcStorageMediaKind = response.GetVolume().GetStorageMediaKind();

        if (DstStorageMediaKind == NCloud::NProto::STORAGE_MEDIA_DEFAULT) {
            DstStorageMediaKind = SrcStorageMediaKind;
        }

        const auto& encryptionKeyHash = response.GetVolume().GetEncryptionKeyHash();
        if (encryptionKeyHash) {
            EncryptionSpec.SetKeyHash(encryptionKeyHash);
            EncryptionSpec.SetDisableEncryption(true);
        } else {
            EncryptionSpec.MutableKey()->SetMode(NProto::NO_ENCRYPTION);
        }

        if (TabletVersion == 0) {
            TabletVersion = response.GetVolume().GetTabletVersion();
        }
