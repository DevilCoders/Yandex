    ...
    try {
        ...
        if (!AtomicGet(AppContext.ShouldStop)) {
            ...
            if (testResult == NProto::TEST_STATUS_FAILURE) {
                AtomicIncrement(AppContext.FailedTests);
                return 1;
            }
        }
    ...
    const TVector<ui32>& successOnError)
{
    ...
    if (test.HasStartEndpointRequest()) {
        ...
        if (ClientConfig->GetUnixSocketPath()) {
            TestContext.DataClient = ClientFactory.CreateDataClient(
                successOnError,
                test.GetEncryptionSpec());
        } else {
    ...
    if (validationClient) {
        ...
        if (isEmptyVolume) {
            ...
            for (const auto* dependency: dependencies) {
                if (!AtomicGet(dependency->Finished)) {
    ...
    const TVector<ui32>& successOnError)
{
    ...
    if (!isAliased && !test.GetVolumeName()) {
