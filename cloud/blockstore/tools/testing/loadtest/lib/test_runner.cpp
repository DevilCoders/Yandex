...
bool TTestRunner::CheckExitCondition() const
{
    ...
    return (StopRequested() && RequestsSent == RequestsCompleted)
        || total && total <= RequestsCompleted;
}

bool TTestRunner::CheckSendRequestCondition() const
{
    const auto total = Requests->Size();
    return !StopRequested() && (!total || total > RequestsSent);
}
