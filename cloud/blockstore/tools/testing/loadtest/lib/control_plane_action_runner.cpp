    ...
    const NProto::TActionGraph::TControlPlaneAction& action)
{
    ...
    *cmsAction->MutableHost() = volumeInfo.AgentIds[agentNo];
    ...
    auto* changeAgentState = changeState.MutableChangeAgentState();
    changeAgentState->SetAgentId(volumeInfo.AgentIds[agentNo]);
    changeAgentState->SetState(2); // AGENT_STATE_UNAVAILABLE
