...
def run_mock(args):
    ...
    if port is not None:
        ...
        if args.format == 'text':
            ...
            if rc != 0:
                print('command execution on one or more hosts failed', file=sys.stderr)
    ...
    delay = RESTART_DELAY_CONTROL if args.target.find("control") >= 0 else RESTART_DELAY
    ...
    start_ts = datetime.strftime(datetime.utcnow() - timedelta(seconds=uptime), DATE_FORMAT)
