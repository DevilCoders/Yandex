...
struct TOptions
{
    ...
    TOptions(int argc, const char** argv)
    {
        ...
        opts.AddLongOption(
                "write-merged-blob-threshold",
                "WriteMergedBlobThreshold")
            .RequiredArgument("INTEGER")
            .DefaultValue(3_MB)
            .StoreResult(&WriteMergedBlobThreshold);
...
struct TBlobIndex
{
    ...
    // Options
    const ui32 MaxBlobSize;
    const ui32 WriteMergedBlobThreshold;
    ...
    TBlobIndex(
        ...
        : Calc(std::move(calc))
        , MaxBlobSize(maxBlobSize)
        , WriteMergedBlobThreshold(writeMergedBlobThreshold)
    ...
    TVector<TBlobBlocks> FindBlocks(ui64 index, ui32 blocks) const
    {
        ...
        while (it != MergedIndex.end()) {
            const auto maxRange = TBlockRange64(
            ...
            for (const auto& blobInfo: it->second) {
                const auto blobRange =
                    TBlockRange64(blobInfo.FirstIndex, it->first);
    ...
    */
    ...
    {
        Y_VERIFY(blocks.size());
        Y_VERIFY(IsSorted(blocks.begin(), blocks.end()));
        ...
        const auto onRange = [&] () {
            Y_VERIFY(prevRangeIndex != Max<ui64>());
    ...
    void ValidateGarbage(TLog& log)
    {
        ...
        for (const auto& x: Blobs) {
            ...
            Y_VERIFY(!IsBlockMaskFull(x.second.Mask, maxBlocksInBlob));
        }
...
public:
    TEventProcessor(const TOptions& options, TLog& log)
        ...
        , BlobIndex(
            ...
            options.MaxBlobSize,
            options.WriteMergedBlobThreshold,
...
private:
    ...
    void WriteBlob(const TVector<ui64>& blocks)
    {
        Log << "mixed write\t" << blocks.size()
            ...
            << "\t" << blocks.back()
            << "\n";
        Y_VERIFY(blocks.size() <= Options.MaxBlobSize / BlockSize);
    ...
    void WriteBlob(const ui64 index, const ui32 blocks)
    {
        ...
        Y_VERIFY(blocks <= Options.MaxBlobSize / BlockSize);
    ...
    void DoCompact(const TCompactionRange& topRange)
    {
        ...
        if (blocks.size() * BlockSize <= Options.WriteMergedBlobThreshold) {
            Sort(blocks);
            WriteBlob(blocks);
            Stat.CompactionStat.Write.Reg(blocks.size(), 1);
        } else {
