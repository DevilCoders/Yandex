...
// DO_TEST
    ...
    Y_UNIT_TEST(BasicThrottling)
    {
        ...
        for (ui32 i = 0; i < 9; ++i) {
            DO_TEST(tp, 0, 10'000, 4_KB, EOpType::Read);
        }
        DO_TEST(tp, 38'131, 10'000, 4_KB, EOpType::Read);
        DO_TEST(tp, 38'131, 10'000, 4_KB, EOpType::Write);
        tp.OnPostponedEvent({}, {4_KB, EOpType::Read, 1});
        tp.OnPostponedEvent({}, {4_KB, EOpType::Write, 1});
        DO_TEST(tp, 0, 48'131, 4_KB, EOpType::Read);
        DO_TEST(tp, 103'813, 48'131, 4_KB, EOpType::Write);
        tp.OnPostponedEvent({}, {4_KB, EOpType::Write, 1});
        DO_TEST(tp, 0, 151'944, 4_KB, EOpType::Write);

        // MaxPostponedWeight test

        for (ui32 i = 0; i < 8; ++i) {
            DO_TEST(tp, 103'813, 151'944, 4_KB, EOpType::Read);
        }

        DO_TEST_REJECT(tp, 151'944, 4_KB, EOpType::Write);
        DO_TEST_REJECT(tp, 151'944, 4_KB, EOpType::Read);

        for (ui32 i = 0; i < 8; ++i) {
            tp.OnPostponedEvent({}, {4_KB, EOpType::Read, 1});
        }

        DO_TEST(tp, 103'813, 151'944, 4_KB, EOpType::Write);
        DO_TEST(tp, 103'813, 151'944, 4_KB, EOpType::Write);
        DO_TEST_REJECT(tp, 151'944, 4_KB, EOpType::Write);
        DO_TEST_REJECT(tp, 151'944, 4_KB, EOpType::Read);
        tp.OnPostponedEvent({}, {4_KB, EOpType::Write, 1});
        ...
        for (ui32 i = 0; i < 9; ++i) {
            DO_TEST(tp, 0, 10'000'000, 4_KB, EOpType::Read);
        }
        DO_TEST(tp, 38'131, 10'000'000, 4_KB, EOpType::Read);
        DO_TEST(tp, 38'131, 10'000'000, 4_KB, EOpType::Write);
        tp.OnPostponedEvent({}, {4_KB, EOpType::Read, 1});
        tp.OnPostponedEvent({}, {4_KB, EOpType::Write, 1});
        DO_TEST(tp, 0, 10'038'131, 4_KB, EOpType::Read);
    }
    ...
    Y_UNIT_TEST(ThrottlingByBandwidth)
    {
        ...
        DO_TEST(tp, 1'075'954, 48'131, 1_MB, EOpType::Read);
        DO_TEST(tp, 1'075'954, 48'131, 1_MB, EOpType::Write);
        DO_TEST(tp, 100'000, 48'131, 1_MB, EOpType::Describe);
    }
    ...
    Y_UNIT_TEST(SmallMaxBurst)
    {
        ...
        DO_TEST(tp, 0, 10'000, 4_KB, EOpType::Read);
        DO_TEST(tp, 7'626, 10'000, 4_KB, EOpType::Read);
    }
    ...
    Y_UNIT_TEST(ThrottlingWithBackpressure)
    {
        ...
        for (size_t i = 0; i < 9; ++i) {
            DO_TEST(tp, 0, t.MicroSeconds(), 4_KB, EOpType::Write);
        }
        DO_TEST(tp, 38'131, t.MicroSeconds(), 4_KB, EOpType::Write);
...
#define TEST_FEATURE(featureName)                                              \
        {                                                                      \
            ...
            for (ui32 i = 0; i < 9; ++i) {                                     \
                DO_TEST(tp, 0, t.MicroSeconds(), 4_KB, EOpType::Read);         \
            }                                                                  \
            DO_TEST(tp, 38'131, t.MicroSeconds(), 4_KB, EOpType::Read);        \
            tp.OnPostponedEvent({}, {4_KB, EOpType::Read, 1});                 \
                                                                               \
            t += eternity;                                                     \
            for (ui32 i = 0; i < 4; ++i) {                                     \
            ...
            DO_TEST(tp, 38'130, t.MicroSeconds(), 4_KB, EOpType::Write);       \
            tp.OnPostponedEvent({}, {4_KB, EOpType::Write, 1});                \
        }                                                                      \
// TEST_FEATURE
...
#undef TEST_FEATURE
    ...
    Y_UNIT_TEST(PostponedQueueAndBackpressure)
    {
        ...
        DO_TEST(tp, delay, now, 1_MB, EOpType::Write);

        for (int i = 0; i < 9; ++i) {
            UNIT_ASSERT(tp.TryPostpone(
                TInstant::MicroSeconds(now), {1_MB, EOpType::Write}
            ));
        }
        UNIT_ASSERT(!tp.TryPostpone(
            TInstant::MicroSeconds(now), {1_MB, EOpType::Write}
    ...
    Y_UNIT_TEST(PostponeCountAndReset)
    {
        ...
        DO_TEST_V(tp, 0, 0, 4_KB, EOpType::Read, 1);
        DO_TEST_V(tp, delay, 0, 4_KB, EOpType::Write, 1);
        DO_TEST_REJECT_V(tp, 0, 4_KB, EOpType::Write, 1);
        ...
        DO_TEST_REJECT_V(tp, 0, 4_KB, EOpType::Read, 1);
        DO_TEST_REJECT_V(tp, 0, 4_KB, EOpType::Write, 1);
        DO_TEST_V(tp, 0, 0, 4_KB, EOpType::Read, 2);
        DO_TEST_V(tp, delay, 0, 4_KB, EOpType::Write, 2);
        DO_TEST_REJECT_V(tp, 0, 4_KB, EOpType::Write, 2);
        tp.OnPostponedEvent(TInstant::Zero(), {4_KB, EOpType::Write, 1});
        DO_TEST_REJECT_V(tp, 0, 4_KB, EOpType::Write, 2);
        tp.OnPostponedEvent(TInstant::Zero(), {4_KB, EOpType::Write, 2});
        DO_TEST_V(tp, delay, 0, 4_KB, EOpType::Write, 2);
    }
    ...
    Y_UNIT_TEST(BoostRate)
    {
        ...
        DO_TEST(tp, 95'628, 10'000, 2_MB, EOpType::Read);
        UNIT_ASSERT_VALUES_EQUAL(10'000, tp.GetCurrentBoostBudget().MilliSeconds());
        DO_TEST(tp, 0, 105'628, 2_MB, EOpType::Read);
        UNIT_ASSERT_VALUES_EQUAL(9'043, tp.GetCurrentBoostBudget().MilliSeconds());
        DO_TEST(tp, 186'537, 105'628, 2_MB, EOpType::Read);
        UNIT_ASSERT_VALUES_EQUAL(9'043, tp.GetCurrentBoostBudget().MilliSeconds());
        DO_TEST(tp, 0, 10'000'000, 5_MB, EOpType::Read);
        UNIT_ASSERT_VALUES_EQUAL(9'043, tp.GetCurrentBoostBudget().MilliSeconds());
    }
    ...
    Y_UNIT_TEST(SeparateReadAndWriteLimits)
    {
        ...
        for (ui32 i = 0; i < 49; ++i) {
            DO_TEST(tp, 0, 10'000'000, 4_KB, EOpType::Write);
        }
        DO_TEST(tp, 5'900, 10'000'000, 4_KB, EOpType::Write);
        tp.OnPostponedEvent({}, {4_KB, EOpType::Write, 1});
        DO_TEST(tp, 0, 10'005'900, 4_KB, EOpType::Write);

        DO_TEST(tp, 1'075'954, 10'005'900, 1_MB, EOpType::Read);
        DO_TEST(tp, 513'666, 10'005'900, 1_MB, EOpType::Write);
    }
    ...
    Y_UNIT_TEST(MaxDelay)
    {
        ...
        DO_TEST(tp, 75'955, 10'000, 1_MB, EOpType::Read);
        DO_TEST(tp, 0, 85'955, 1_MB, EOpType::Read);
        DO_TEST(tp, 587'977, 85'955, 512_KB, EOpType::Read);
    ...
    Y_UNIT_TEST(MaxWriteCostMultiplier)
    {
        ...
        DO_TEST(tp, 4'379'770, 10'000, 1_MB, EOpType::Write);
    }
