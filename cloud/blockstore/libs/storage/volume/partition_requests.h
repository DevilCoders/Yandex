    ...
    TBlockRange64* blockRange)
{
    ...
    for (ui32 i = 0; i < partitions.size(); ++i) {
        ...
        (*requests)[i].Event = std::make_unique<TEvVolume::TGetCompactionStatusMethod::TRequest>();
    ...
    TBlockRange64* blockRange)
{
    ...
    for (ui32 i = 0; i < partitions.size(); ++i) {
        ...
        (*requests)[i].TabletId = partitions[i].TabletId;
        (*requests)[i].PartitionId = i;
        (*requests)[i].Event = std::make_unique<TEvVolume::TRebuildMetadataMethod::TRequest>();
        (*requests)[i].Event->Record = ev->Get()->Record;
    }
    ...
    TBlockRange64* blockRange)
{
    ...
    for (ui32 i = 0; i < partitions.size(); ++i) {
        ...
        (*requests)[i].TabletId = partitions[i].TabletId;
        (*requests)[i].PartitionId = i;
        (*requests)[i].Event = std::make_unique<TEvVolume::TGetRebuildMetadataStatusMethod::TRequest>();
        (*requests)[i].Event->Record = ev->Get()->Record;
    }
