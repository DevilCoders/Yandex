...
//  BLOCKSTORE_SIMPLE_COUNTER
    ...
    BLOCKSTORE_SIMPLE_COUNTER(ReadSuffer);
    BLOCKSTORE_SIMPLE_COUNTER(WriteSuffer);
...
#define BLOCKSTORE_PERCENTILE_COUNTER_WITH_SIZE(counter)                       \
    out.counter##Large = CreatePercentileCounterField(                         \
            r.counter.GetLarge().CalculatePercentiles());                      \
    out.counter##Small = CreatePercentileCounterField(                         \
            r.counter.GetSmall().CalculatePercentiles());                      \
//  BLOCKSTORE_PERCENTILE_COUNTER_WITH_SIZE

    BLOCKSTORE_PERCENTILE_COUNTER_WITH_SIZE(ReadBlocks);
    BLOCKSTORE_PERCENTILE_COUNTER_WITH_SIZE(WriteBlocks);
    BLOCKSTORE_PERCENTILE_COUNTER_WITH_SIZE(WriteBlob);
    BLOCKSTORE_PERCENTILE_COUNTER_WITH_SIZE(ReadBlob);
    BLOCKSTORE_PERCENTILE_COUNTER_WITH_SIZE(ZeroBlocks);
#undef BLOCKSTORE_PERCENTILE_COUNTER_WITH_SIZE
    ...
    const TActorContext& ctx)
{
    ...
    if (!VolumeIdQueueForYdbStatsUpload) {
        for (auto& p: State.GetVolumes()) {
            if (!p.second.VolumeInfo.Defined()) {
                continue;
            }
