...
Y_UNIT_TEST_SUITE(TServiceCreateVolumeTest)
{
            ...
            [&] (TTestActorRuntimeBase& runtime, TAutoPtr<IEventHandle>& event) {
                switch (event->GetTypeRewrite()) {
                    case NSchemeShard::TEvSchemeShard::EvDescribeSchemeResult: {
                        ...
                        if (path == "path%2Fto%2Ftest_volume" && describeRequested) {
    ...
    Y_UNIT_TEST(ShouldRejectVolumeCreationWithoutDiskId)
    {
        TTestEnv env;
        ui32 nodeIdx = SetupTestEnv(env);
        ...
        TString diskId;
        service.SendCreateVolumeRequest(diskId);
        auto response = service.RecvCreateVolumeResponse();
    ...
    Y_UNIT_TEST(ShouldAllowTokenVersionInAssignVolume)
    {
        ...
        UNIT_ASSERT_VALUES_EQUAL(E_CONFIG_VERSION_MISMATCH, response->GetStatus());
    }
