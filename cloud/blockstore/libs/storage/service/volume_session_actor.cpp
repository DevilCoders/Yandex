...
STFUNC(TVolumeSessionActor::StateDescribe)
{
    switch (ev->GetTypeRewrite()) {
        ...
        HFunc(TEvService::TEvMountVolumeRequest, PostponeMountVolume);
        HFunc(TEvService::TEvUnmountVolumeRequest, PostponeUnmountVolume);
        HFunc(TEvService::TEvChangeVolumeBindingRequest, PostponeChangeVolumeBindingRequest);
...
STFUNC(TVolumeSessionActor::StateWork)
{
    switch (ev->GetTypeRewrite()) {
        ...
        HFunc(TEvService::TEvMountVolumeRequest, HandleMountVolume);
        HFunc(TEvService::TEvUnmountVolumeRequest, HandleUnmountVolume);
        HFunc(TEvServicePrivate::TEvInactiveClientsTimeout, HandleInactiveClientsTimeout);

        HFunc(TEvServicePrivate::TEvMountRequestProcessed, HandleMountRequestProcessed);
        HFunc(TEvServicePrivate::TEvUnmountRequestProcessed, HandleUnmountRequestProcessed);

        HFunc(TEvServicePrivate::TEvStartVolumeRequest, HandleStartVolumeRequest);
        HFunc(TEvServicePrivate::TEvStopVolumeRequest, HandleStopVolumeRequest);
        HFunc(TEvServicePrivate::TEvVolumeTabletStatus, HandleVolumeTabletStatus);
        HFunc(TEvServicePrivate::TEvStartVolumeActorStopped, HandleStartVolumeActorStopped);

        HFunc(TEvServicePrivate::TEvVolumePipeReset , HandleVolumePipeReset);

        HFunc(TEvService::TEvChangeVolumeBindingRequest, HandleChangeVolumeBindingRequest);
        HFunc(TEvService::TEvMountVolumeResponse, HandleMountResponseAfterBindingChange);
