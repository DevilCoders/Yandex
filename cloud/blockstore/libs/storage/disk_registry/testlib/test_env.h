...
public:
    ...
    auto CreateDeallocateDiskRequest(const TString& diskId, bool force)
    {
    ...
    auto CreateAllowDiskAllocationRequest(bool value = true)
    {
        auto request = std::make_unique<TEvDiskRegistry::TEvAllowDiskAllocationRequest>();
        request->Record.SetAllow(value);
    ...
    auto CreateCreatePlacementGroupRequest(TString groupId)
    {
    ...
    auto CreateResumeDeviceRequest(TString agentId, TString path)
    {
        auto request =
            std::make_unique<TEvDiskRegistry::TEvResumeDeviceRequest>();

        request->Record.SetHost(std::move(agentId));
