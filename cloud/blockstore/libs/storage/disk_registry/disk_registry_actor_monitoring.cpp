    ...
    const TString& id) const
{
    HTML(out) {
        ...
        if (info.Migrations) {
            H3() { out << "Active migrations"; DumpSize(out, info.Migrations); }
...
void TDiskRegistryActor::RenderMigrationList(IOutputStream& out) const
{
    ...
    HTML(out) {
        H3() { out << "Migration List"; DumpSize(out, migrations); }
...
{
    HTML(out) {
        H3() { out << "BrokenDisks"; DumpSize(out, State->GetBrokenDisks()); }
...
void TDiskRegistryActor::RenderRacks(IOutputStream& out) const
{
    ...
    HTML(out) {
        H3() { out << "Racks"; DumpSize(out, racks); }
        ...
        TABLE_SORTABLE_CLASS("table table-bordered") {
            ...
            for (const auto& rack: racks) {
                TABLER() {
                    ...
                    TABLED() {
                        UL() {
                            for (const auto& g: rack.PlacementGroups) {
                                LI() { out << g; }
                            }
                        }
                    }
...
{
    HTML(out) {
        H3() { out << "Agents"; DumpSize(out, State->GetAgents()); }
...
void TDiskRegistryActor::RenderConfig(IOutputStream& out) const
{
    HTML(out) {
        ...
        H4() { out << "Version: " << config.GetVersion(); }
        H4() { out << "Known agents"; }
        UL() {
...
void TDiskRegistryActor::RenderDirtyDeviceList(IOutputStream& out) const
{
    ...
    HTML(out) {
        H3() { out << "Dirty devices"; DumpSize(out, devices); }
    ...
    HTML(out) {
        if (State) {
            if (!State->IsDiskAllocationAllowed()) {
                H3() { out << "<font color=red>Allocations disallowed</font>"; }
            }
            ...
            H3() { out << "Initialization in progress..."; }
        }

        H3() { out << "Storage Config"; }
        Config->DumpHtml(out);
...
void TDiskRegistryActor::RenderState(IOutputStream& out) const
{
    ...
    HTML(out) {
        H3() {
            out << "Current state: " << GetStateName(CurrentState);
            if (CurrentState == STATE_RESTORE) {
                out << " <font color=red>Need to kill tablet</font>";
            }
        }
    }
}
    ...
    const TActorContext& ctx)
{
    ...
    if (action == "dev") {
        RenderDeviceHtmlInfo(out, params.Get("DeviceUUID"));
    } else if (action == "agent") {
        RenderAgentHtmlInfo(out, params.Get("AgentID"));
    } else if (action == "disk") {
        RenderDiskHtmlInfo(out, params.Get("DiskID"));
    } else if (action == "volumeRealloc") {
        HandleHttpInfo_VolumeRealloc(ev, params.Get("DiskID"), ctx);
        return;
    } else if (action == "replaceDevice") {
        HandleHttpInfo_ReplaceDevice(
            ev,
            params.Get("DiskID"),
            params.Get("DeviceID"),
            ctx);
