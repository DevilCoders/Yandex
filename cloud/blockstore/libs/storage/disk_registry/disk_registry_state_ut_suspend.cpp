...
Y_UNIT_TEST_SUITE(TDiskRegistryStateSuspendTest)
{
    Y_UNIT_TEST(ShouldSuspendDevices)
    {
        ...
        executor.WriteTx([&] (TDiskRegistryDatabase db) {
            UNIT_ASSERT_SUCCESS(state.ResumeDevice(db, "uuid-2.1"));
            UNIT_ASSERT_SUCCESS(state.ResumeDevice(db, "uuid-2.2"));
            ...
            state.MarkDeviceAsClean(db, "uuid-2.1");
            state.MarkDeviceAsClean(db, "uuid-2.2");
        ...
        executor.WriteTx([&] (TDiskRegistryDatabase db) {
            UNIT_ASSERT_SUCCESS(state.ResumeDevice(db, "uuid-2.3"));
            UNIT_ASSERT_SUCCESS(state.ResumeDevice(db, "uuid-2.4"));
            ...
            state.MarkDeviceAsClean(db, "uuid-2.3");
            state.MarkDeviceAsClean(db, "uuid-2.4");
