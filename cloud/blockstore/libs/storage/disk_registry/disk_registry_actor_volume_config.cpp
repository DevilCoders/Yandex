    ...
    const TActorContext& ctx)
{
    ...
    if (kind == EErrorKind::ErrorRetriable || code == E_CONFIG_VERSION_MISMATCH ||
        code == MAKE_SCHEMESHARD_ERROR(NKikimrScheme::StatusPathDoesNotExist))
    {
        auto gracePeriod = Config->GetDiskRegistryVolumeConfigUpdatePeriod();
        auto* request = new TEvDiskRegistryPrivate::TEvUpdateVolumeConfigRequest(diskId);

        ctx.ExecutorThread.Schedule(gracePeriod, new IEventHandle(ctx.SelfID, ctx.SelfID, request));
        return;
    }
