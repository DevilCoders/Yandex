...
void TWriteBlobActor::SendPutRequest(const TActorContext& ctx)
{
    ...
    if (const auto* guardedSgList = std::get_if<TGuardedSgList>(&Request->Data)) {
        if (auto guard = guardedSgList->Acquire()) {
            ...
            auto error = MakeError(E_CANCELLED, "Local request was cancelled");
    ...
    const TActorContext& ctx)
{
    ...
    if (msg->StorageStatusFlags.Check(NKikimrBlobStorage::EStatusFlags::StatusIsValid)) {
        ...
        State->UpdateChannelFreeSpaceShare(channel, msg->ApproximateFreeSpaceShare);

        if (msg->StorageStatusFlags.Check(NKikimrBlobStorage::StatusDiskSpaceLightYellowMove)) {
