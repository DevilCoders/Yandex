    ...
    const TActorContext& ctx)
{
    ...
    if (msg->LastReadBlobId == FinalBlobId) {
        ...
        if (msg->LastReadBlobId.UniqueId() == MaxUniqueId) {
            BlobIdToRead = MakePartialBlobId(
                msg->LastReadBlobId.CommitId() + 1,
                0);
        } else {
            BlobIdToRead = MakePartialBlobId(
                msg->LastReadBlobId.CommitId(),
                msg->LastReadBlobId.UniqueId() + 1);
        }
