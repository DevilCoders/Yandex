...
void TClientEndpoint::RecvRequestCompleted(TRecvWr* recv, ibv_wc_status status)
{
    ...
    if (req->In.Length > Config->MaxBufferSize) {
        STORAGE_ERROR("RECV #" << recv->wr.wr_id
            << " error: request exceeds maximum supported size "
...
void TClientEndpoint::ReadRequestData(TRequestPtr req, TSendWr* send)
{
    ...
    NVerbs::PostSend(Connection->qp, &wr);
...
{
    if (status != IBV_WC_SUCCESS) {
        STORAGE_ERROR("READ #" << send->wr.wr_id << " error: "
            << NVerbs::GetStatusString(status));
    }
    ...
    if (req != nullptr) {
        if (IsRetriable(status)) {
            return ReadRequestData(std::move(req), send);
        }

        RecvBuffers.ReleaseBuffer(req->InBuffer);
        SendBuffers.ReleaseBuffer(req->OutBuffer);
    } else {
        STORAGE_ERROR("READ #" << send->wr.wr_id << " error: request is empty");
    }
...
{
    if (status != IBV_WC_SUCCESS) {
        STORAGE_ERROR("WRITE #" << send->wr.wr_id << " error: "
            << NVerbs::GetStatusString(status));
    }
    ...
    if (req != nullptr) {
        if (IsRetriable(status)) {
            return WriteResponseData(std::move(req), send);
        }

        RecvBuffers.ReleaseBuffer(req->InBuffer);
        SendBuffers.ReleaseBuffer(req->OutBuffer);
    } else {
        STORAGE_ERROR("WRITE #" << send->wr.wr_id << " error: request is empty");
    }
    ...
    ibv_wc_status status)
{
    ...
    if (req != nullptr) {
        if (IsRetriable(status)) {
            return SendResponse(std::move(req), send);
        }
        ...
        STORAGE_ERROR("SEND #" << send->wr.wr_id
            << "error: request context is empty");
    }
...
public:
    ...
    void SendError(void* context, ui32 error, TStringBuf message) override
    {
        TRequestPtr req(static_cast<TRequest*>(context));
        req->Status = error;
        req->ResponseBytes = Min<ui32>(message.length(), req->OutBuffer.Length);

        memcpy(
            reinterpret_cast<void*>(req->OutBuffer.Address),
            message.data(),
            req->ResponseBytes);
...
public:
    TCompletionPoller(TServerConfigPtr config, TLog log)
        : Config(std::move(config))
        , Log(log)
    {
        if (Config->WaitMode == EWaitMode::Poll) {
...
void TServer::BeginListen(TServerEndpoint* endpoint)
{
    ...
    NVerbs::BindAddress(endpoint->Connection.get(), addrinfo->ai_src_addr);
    NVerbs::Listen(endpoint->Connection.get(), Config->Backlog);
}
...
void TServer::BeginConnect(TClientEndpointPtr endpoint)
{
    ...
    NVerbs::Accept(endpoint->Connection.get(), &acceptParams);
}
    ...
    rdma_cm_event* event)
{
    ...
    NVerbs::DestroyQP(endpoint->Connection.get());
...
void TServer::RejectWithStatus(rdma_cm_id* id, int status)
{
    ...
    NVerbs::Reject(id, &rejectMsg, sizeof(TRejectMessage));
}
