...
constexpr TDuration MAX_RECONNECT_DELAY = TDuration::Minutes(10);
...
bool IsRetriable(ibv_wc_status status)
{
    switch (status) {
...
public:
    ...
    {
        auto [it, inserted] = Requests.emplace(req->ReqId, std::move(req));
        Y_VERIFY(inserted);
    }
...
private:
    ...
    void SendRequestError(TRequest* req, TSendWr* send, ibv_wc_status status);
...
{
    CompletionChannel = NVerbs::CreateCompletionChannel(Connection->verbs);
    ...
    CompletionQueue = NVerbs::CreateCompletionQueue(
        Connection->verbs,
        &cq_attrs);
    ...
    NVerbs::CreateQP(Connection.get(), &qp_attrs);
...
{
    auto len = Min(req->ResponseBuffer.size(), msg.size());
    memcpy((char*)req->ResponseBuffer.data(), msg.data(), len);
    Handler->HandleResponse(std::move(req), err, len);
}
...
bool TClientEndpoint::HandleCompletionEvents()
{
    ...
    if (Config.WaitMode == EWaitMode::Poll) {
        NVerbs::GetCompletionEvent(cq);
        NVerbs::RequestCompletionEvent(cq, 0);
    }

    bool hasWork = NVerbs::PollCompletionQueue((ibv_cq_ex*)cq, this);

    if (Config.WaitMode == EWaitMode::Poll) {
        NVerbs::AckCompletionEvents(cq, 1);
    }
...
void TClientEndpoint::SendRequest(TRequestPtr req, TSendWr* send)
{
    ...
    NVerbs::PostSend(Connection->qp, &send->wr);
    ...
    ui64 ts)
{
    ...
    if (status != IBV_WC_SUCCESS || req == nullptr) {
        return SendRequestError(req, send, status);
    }
    ...
    ibv_wc_status status)
{
    if (status != IBV_WC_SUCCESS) {
        STORAGE_ERROR("SEND #" << send->wr.wr_id << " error: "
            << NVerbs::GetStatusString(status));
    }

    ui32 error = IsRetriable(status) ? E_REJECTED : E_FAIL;
    
    if (ptr != nullptr) {
        if (auto req = ActiveRequests.Pop(ptr->ReqId)) {
            HandleError(
                std::move(req),
                error,
                TStringBuilder()
            ...
            STORAGE_ERROR("SEND #" << send->wr.wr_id << " error: request "
                << ptr->ReqId << " not found");

            ReportRdmaError();
        }
    } else {
        STORAGE_ERROR("SEND #" << send->wr.wr_id << " error: request is empty");
        ReportRdmaError();
    }
...
void TClientEndpoint::RecvResponse(TRecvWr* recv)
{
    ...
    NVerbs::PostRecv(Connection->qp, &recv->wr);
    ...
    ui64 ts)
{
    ...
    if (!req) {
        STORAGE_ERROR("RECV #" << recv->wr.wr_id << " error: request "
            << reqId << " not found");
...
{
    STORAGE_ERROR("RECV #" << recv->wr.wr_id << " error: "
        << NVerbs::GetStatusString(status));
    ...
    // TODO There is no way to tell which request triggered the error. The best
    // we can do is to cancel requests that were active way too long
}

void TClientEndpoint::ResetConnection(NVerbs::TConnectionPtr conn)
{
    NVerbs::DestroyQP(Connection.get());
...
public:
    TConnectionPoller(IConnectionEventHandler* eventHandler, TLog log)
        : EventHandler(eventHandler)
        , Log(log)
    {
        EventChannel = NVerbs::CreateEventChannel();
    ...
    {
        return NVerbs::CreateConnection(
            EventChannel.get(),
            nullptr,    // context
            RDMA_PS_TCP);
    }
...
private:
    ...
    {
        while (auto event = NVerbs::GetConnectionEvent(EventChannel.get())) {
...
public:
    TCompletionPoller(TClientConfigPtr config, TLog log)
    ...
    void Attach(TClientEndpoint* endpoint)
    {
        if (Config->WaitMode == EWaitMode::Poll) {
            ...
            NVerbs::RequestCompletionEvent(endpoint->CompletionQueue.get(), 0);
        }
    }
    ...
    , Counters(new TEndpointCounters())
{
    ...
    NRdma::NVerbs::GetDeviceList();
    NRdma::NVerbs::GetAddressInfo("localhost", 10020, nullptr);
}
...
void TClient::Start()
{
    ...
    for (size_t i = 0; i < CompletionPollers.size(); ++i) {
        CompletionPollers[i] = std::make_unique<TCompletionPoller>(Config, Log);
        CompletionPollers[i]->Start();
    }

    ConnectionPoller = std::make_unique<TConnectionPoller>(this, Log);
...
void TClient::BeginResolveAddress(TClientEndpoint* endpoint)
{
    ...
    auto addrinfo = NVerbs::GetAddressInfo(
        endpoint->Host,
        endpoint->Port,
        &hints);

    STORAGE_DEBUG("RESOLVE address "
...
void TClient::BeginResolveRoute(TClientEndpoint* endpoint)
{
    ...
    NVerbs::ResolveRoute(endpoint->Connection.get(), RESOLVE_TIMEOUT);
}
...
void TClient::BeginConnect(TClientEndpoint* endpoint)
{
    ...
    NVerbs::Connect(endpoint->Connection.get(), &connectParams);
}
    ...
    NVerbs::TConnectionEventPtr event)
{
    ...
    {
        STORAGE_ERROR("CONNECT error: unable to parse accept message");
    ...
    if (version != RDMA_PROTO_VERSION) {
        STORAGE_ERROR("CONNECT error: incompatible protocol version "
            << version << " != " << int(RDMA_PROTO_VERSION));
