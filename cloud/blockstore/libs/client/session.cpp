    ...
    const NProto::THeaders& headers)
{
    ...
    with_lock (SessionInfo.MountLock) {
        if (SessionInfo.MountState == EMountState::MountInProgress) {
            ...
            return SessionInfo.MountResponse.GetFuture().Apply([=] (const auto& future) mutable {
                Y_UNUSED(future);
        ...
        if (SessionInfo.MountState == EMountState::UnmountInProgress) {
            auto weak_ptr = weak_from_this();
            return SessionInfo.UnmountResponse.GetFuture().Apply([=] (const auto& future) mutable {
                Y_UNUSED(future);
                if (auto p = weak_ptr.lock()) {
                    return p->MountVolume(
                        accessMode,
    ...
    TPromise<typename T::TResponse> response)
{
    ...
    if (error.GetCode() == E_BS_INVALID_SESSION) {
        ...
        auto volumeInfo = VolumeStats->GetVolumeInfo(
            ...
            Config->GetClientId());
        if (volumeInfo) {
            volumeInfo->AddRetryStats(T::RequestType, errorKind);
        }
...
{
    // prevent https://st.yandex-team.ru/NBS-420
