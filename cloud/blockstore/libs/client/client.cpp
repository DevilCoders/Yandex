    ...
    const NProto::TReadBlocksResponse& response)
{
    ...
    if (!guard) {
        return TErrorResponse(E_CANCELLED, "Local request was cancelled");
    }
...
class TEndpoint final
    : public TEndpointBaseImpl<NProto::TBlockStoreService::Stub>
{
public:
    using TEndpointBaseImpl::TEndpointBaseImpl;
...
class TDataEndpoint final
    : public TEndpointBaseImpl<NProto::TBlockStoreDataService::Stub>
{
public:
    using TEndpointBaseImpl::TEndpointBaseImpl;
...
class TSocketDataEndpoint final
    : public TEndpointBaseImpl<NProto::TBlockStoreDataService::Stub>
    , public std::enable_shared_from_this<TSocketDataEndpoint>
{
    enum {
        Disconnected = 0,
        Connecting = 1,
...
public:
    TSocketDataEndpoint(
            std::shared_ptr<TClient> client,
            TString socketPath)
        : TEndpointBaseImpl(std::move(client), nullptr)
...
#undef BLOCKSTORE_IMPLEMENT_METHOD
    ...
    void Connect()
    {
        ...
        auto args = Client->CreateChannelArguments();
        auto channel = Client->CreateUnixSocketChannel(SocketPath, args);

        if (channel) {
            Service = NProto::TBlockStoreDataService::NewStub(channel);
        }
...
private:
        ...
        std::shared_ptr<typename TMethod::TRequest> request)
    {
        ...
        auto future = ExecuteRequest<TMethod>(
            std::move(callContext),
            std::move(request));
