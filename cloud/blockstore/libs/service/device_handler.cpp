    ...
    ui32 blockSize)
{
    ...
    if (!guard) {
        return MakeError(E_CANCELLED, "Request was cancelled.");
    }
    ...
    std::shared_ptr<TRequest> request)
{
    ...
    return future.Subscribe([=] (const auto& f) {
        ...
        .Apply([=, buf = std::move(buffer)] (const auto& future) {
            ...
            if (!guard) {
                return static_cast<NProto::TReadBlocksLocalResponse>(
                    TErrorResponse(E_CANCELLED, "Local request was cancelled"));
            }
    ...
    const TModifyRequest& request)
{
    ...
    const auto& guardedSgList = request.SgList;
    ...
    return future.Apply([=, sgList = guardedSgList, buf = std::move(buffer)]
...
{
    {
        auto guard = requestSgList.Acquire();
        if (!guard) {
    ...
    request->MutableHeaders()->SetRequestId(ctx->RequestId);
    request->MutableHeaders()->SetTimestamp(TInstant::Now().MicroSeconds());
    request->MutableHeaders()->SetClientId(std::move(clientId));
    request->SetStartIndex(blocksInfo.Range.Start);
    request->BlocksCount = blocksInfo.Range.Size();
    request->BlockSize = blockSize;
