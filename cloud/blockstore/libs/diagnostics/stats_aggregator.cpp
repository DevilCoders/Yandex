...
TStringBuf ExtractNumber(const TString& str)
{
    ui32 i = 0;
    for(; i < str.size(); ++i) {
...
struct THistogramPercentiles
{
    ...
    TVector<TBucketInfo> ReadSolomonHistogram(TLog& Log, TDynamicCounterPtr counters)
    {
        ...
        for (const auto& [counterName, bucket] : bucketCounters) {
            ui32 limit = Max();
            if (counterName.LabelValue != "Inf" &&
                !TryFromString(ExtractNumber(counterName.LabelValue), limit))
            {
                STORAGE_WARN("cannot parse bucket limit for " << counterName.LabelValue);
                return {};
            };
            auto counter = counters->GetCounter(counterName.LabelValue, true);
            if (!counter) {
...
struct TClientRequestPercentiles
{
    ...
    TRequestPercentiles Large;
    TRequestPercentiles Small;
    ...
    void BuildPercentiles(TLog& Log, TDynamicCounterPtr counters)
    {
        ...
        Large.BuildPercentiles(Log, counters->FindSubgroup("sizeclass", "Large"));
        Small.BuildPercentiles(Log, counters->FindSubgroup("sizeclass", "Small"));
    }
};
