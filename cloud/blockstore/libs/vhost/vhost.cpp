...
public:
    ...
    {
        SgList.Destroy();
        vhd_complete_bio(VhdBdevIo, GetVhostResult(result));
    }
...
public:
    TVhostDevice(
            ...
            NRdma::TUnregisterMemory* unregisterMemory)
        : VhdQueue(vhdQueue)
        ...
        , Cookie(cookie)
    {
        ...
        VhdBdevInfo.map_cb = registerMemory;
        VhdBdevInfo.unmap_cb = unregisterMemory;
    }
...
public:
    ...
    TVhostRequestPtr DequeueRequest() override
    {
        ...
        while (vhd_dequeue_request(VhdRequestQueue, &vhdRequest)) {
            ...
            if (!vhostRequest) {
                vhd_complete_bio(vhdRequest.bio, VHD_BDEV_IOERR);
                continue;
            }
