...
def storage_config_patch_with_sparse_merged_blobs(tablet_version):
    storage = default_storage_config_patch(tablet_version)
    storage.UseSparseMergedBlobs = True
    return storage
...
TESTS = [
    ...
    TestCase(
        "version1-overlay-disk-complex-with-sparse-merged-blobs",
        "cloud/blockstore/tests/loadtest/local-overlay/local-tablet-version-1-overlay-disk-complex.txt",
        1,
        storage_config_patch=storage_config_patch_with_sparse_merged_blobs(1),
    ),
    TestCase(
...
def __run_test(test_case):
    ...
    env = LocalLoadTest(
        ...
        static_pdisk_size=32 * 1024 * 1024 * 1024,
