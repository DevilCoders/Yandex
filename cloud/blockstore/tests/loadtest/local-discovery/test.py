...
def test_load():
    ...
    instances_path = os.path.join(common.output_path(), "instances.txt")

    def is_good(port):
        return port in good_ports

    with open(instances_path, "w") as dst, open(results_path, "r") as f:
        for line in f:
            data = json.loads(line)
            ports = [x['Port'] for x in data['Instances']]
            print(list(map(is_good, ports)), file=dst)

    instances_file = common.canonical_file(instances_path)

    if ret is not None:
        ret.append(instances_file)
    else:
        ret = instances_file
