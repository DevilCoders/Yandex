...
from cloud.blockstore.config.server_pb2 import TServerAppConfig, TKikimrServiceConfig, TServerConfig
...
class LocalNbs(Daemon):
    ...
    def __init__(
        ...
        super(LocalNbs, self).__init__(
            commands=self.__make_run_commands(), cwd=self.__cwd,
            restart_allowed=restart_allowed, restart_interval=restart_interval,
            ping_port=self.__mon_port, ping_path=ping_path,
            ping_success_codes=[200], core_pattern=cp)
...
"""
        ...
        command = [
            ...
            "--server",
            "grpc://localhost:" + str(port),
            "db", "schema", "exec", scheme_op
        ]

        logger.info("Modify scheme {}".format(command))
    ...
    def __run(self):
        ...
        self.write_configs()
        logger.info("commands is {}".format(self.__make_run_commands()))
