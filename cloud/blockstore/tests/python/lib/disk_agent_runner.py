...
from ydb.tests.library.harness.daemon import Daemon, DaemonError
...
class LocalDiskAgent(Daemon):
    ...
    def __init__(
        ...
        super(LocalDiskAgent, self).__init__(
            command=self.__make_run_command(), cwd=self.__cwd, timeout=180,
            core_pattern=cp)
...
"""
    ...
    def __run(self):
        ...
        logger.info("command is {}".format(
            " ".join(self.__make_run_command())))
