...
class AgentInfo:
    ...
    def __init__(self, agent_id, device_ids):
...
def setup_disk_registry_config(
    ...
    for agent_info in agent_infos:
        ...
        for device_id in agent_info.device_ids:
            tmp_file.write(b'Devices: "%s"\n' % device_id.encode("utf8"))
...
def allow_disk_allocations(nbs_port, nbs_client_binary_path):
    result = call(
        [nbs_client_binary_path,
            "ExecuteAction",
            "--action", "allowdiskallocation",
            "--input-bytes", '{"Allow": true}',
            "--host", "localhost",
            "--port", str(nbs_port)]
    )
