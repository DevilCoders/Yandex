...
func GetCompactionStatus(
	ctx context.Context,
	client *nbs.Client,
	request *protos.TGetCompactionStatusRequest,
) (*protos.TGetCompactionStatusResponse, error) {

	response := &protos.TGetCompactionStatusResponse{}

	err := executeAction(ctx, client, "GetCompactionStatus", request, response)
	if err != nil {
		return nil, err
	}
