...
public:
    ...
    TMaybe<TGroupDescr> ReadGroupDescr(IInputStream& stream)
    {
        ...
        if (auto match = GroupRE.Capture(line); !match.empty()) {
            ...
            Y_VERIFY(false, "can't parse group info: %s", line.c_str());
        }
        ...
        while (stream.ReadLine(line)) {
            ...
            if (auto match = FreeBlocksRegExp.Capture(line); !match.empty()) {
                Y_VERIFY(!groupDescr.FreeBlocks);
                Y_VERIFY(!groupDescr.FreeInodes);
                Y_VERIFY(!groupDescr.UnusedInodes);
            ...
            if (auto match = BlockBitmapRegExp.Capture(line); !match.empty()) {
                Y_VERIFY(!groupDescr.BlockBitmap);
                Y_VERIFY(!groupDescr.BlockBitmapCS);
            ...
            if (auto match = InodeBitmapRegExp.Capture(line); !match.empty()) {
                Y_VERIFY(!groupDescr.InodeBitmap);
                Y_VERIFY(!groupDescr.InodeBitmapCS);
            ...
            if (auto match = PrimarySuperblockRegExp.Capture(line); !match.empty()) {
                Y_VERIFY(!groupDescr.PrimarySuperblock);
                Y_VERIFY(!groupDescr.GroupDescriptors);
            ...
            if (auto match = BackupSuperblockRegExp.Capture(line); !match.empty()) {
                Y_VERIFY(!groupDescr.BackupSuperblock);
                Y_VERIFY(!groupDescr.GroupDescriptors);
            ...
            if (auto match = ReservedGDTBlocksRegExp.Capture(line); !match.empty()) {
                Y_VERIFY(!groupDescr.ReservedGDTBlocks);
