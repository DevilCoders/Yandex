...
func (client *discoveryClient) getCachedAddr() *instanceAddress {
	client.mtx.Lock()
	defer client.mtx.Unlock()

	return client.cachedAddr
}
...
func (client *discoveryClient) onSuccess(
	...
	if client.cachedAddr == nil {
		client.cachedAddr = addr
...
func (client *discoveryClient) onFailure(
	...
	if client.cachedAddr != nil &&
		client.cachedAddr.Host == addr.Host &&
		client.cachedAddr.Port == addr.Port {
...
func (client *discoveryClient) shoot(
	...
	if err == nil {
		client.onSuccess(ctx, impl, addr)
	} else {
		client.onFailure(ctx, impl, addr)
	}
...
func (client *discoveryClient) discoverInstancesImpl(
	...
	endpoint, err := findClosest(ctx, client.endpoints)

	if err != nil {
		return nil, err
	}

	return client.invokeDiscoverInstances(ctx, endpoint, req)
}
...
func (client *discoveryClient) mainShoot(
	...
	if addr := client.getCachedAddr(); addr != nil {
...
func (client *discoveryClient) discoverInstance(ctx context.Context) (
	*Client,
	error,
) {
	...
	return &Client{safeClient{impl}}, nil
}
