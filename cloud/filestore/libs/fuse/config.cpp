...
#define FILESTORE_FUSE_CONFIG(xxx)                                             \
    ...
    xxx(Debug,                  bool,           false                         )\
                                                                               \
    xxx(FuseMaxWritePages,      ui32,           256                           )\
    xxx(MountSeqNumber,         ui64,           0                             )\
    xxx(VhostQueuesCount,       ui32,           0                             )\
// FILESTORE_FUSE_CONFIG

#define FILESTORE_FUSE_DECLARE_CONFIG(name, type, value)                       \
    Y_DECLARE_UNUSED static const type TFuseConfigDefault##name = value;       \
// FILESTORE_FUSE_DECLARE_CONFIG

FILESTORE_FUSE_CONFIG(FILESTORE_FUSE_DECLARE_CONFIG)
...
#define FILESTORE_FILESYSTEM_CONFIG(xxx)                                       \
    ...
    xxx(XAttrCacheTimeout,      TDuration,      TDuration::Seconds(15)        )\
                                                                               \
    xxx(MaxListBufferSize,      ui32,           4_MB                          )\
                                                                               \
// FILESTORE_FUSE_CONFIG
...
#define FILESTORE_FUSE_GETTER(name, type, ...)                                 \
    FILESTORE_CONFIG_GETTER(TFuseConfig, name, type, ...)                      \
// FILESTORE_FUSE_GETTER

#define FILESTORE_FS_GETTER(name, type, ...)                                   \
    FILESTORE_CONFIG_GETTER(TFileSystemConfig, name, type, ...)                \
// FILESTORE_FS_GETTER

FILESTORE_FUSE_CONFIG(FILESTORE_FUSE_GETTER)
FILESTORE_FILESYSTEM_CONFIG(FILESTORE_FS_GETTER)

#undef FILESTORE_CONFIG_GETTER
#undef FILESTORE_FUSE_GETTER
...
void TFuseConfig::Dump(IOutputStream& out) const
{
    FILESTORE_FUSE_CONFIG(FILESTORE_CONFIG_DUMP);
}
...
void TFuseConfig::DumpHtml(IOutputStream& out) const
{
    HTML(out) {
