    ...
    enum fuse_cancelation_code code)
{
    ...
    enum vhd_bdev_io_result result;
    switch (code) {
        case FUSE_ERROR: result = VHD_BDEV_IOERR; break;
        case FUSE_SUSPEND: result = VHD_BDEV_CANCELED; break;
    }
    complete_request(vhd_req, result);
    return 0;
}
