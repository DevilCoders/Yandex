...
void TIndexNode::Rename(const TString& name, const TIndexNodePtr& newparent, const TString& newname)
{
    return NLowLevel::RenameAt(NodeFd, name.data(), newparent->NodeFd, newname.data());
}
...
TVector<std::pair<TString, TFileStat>> TIndexNode::List()
{
    return NLowLevel::ListDirAt(NodeFd);
}
