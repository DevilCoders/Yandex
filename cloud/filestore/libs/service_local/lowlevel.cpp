...
void RenameAt(const TFile& handle, const TString& name, const TFile& newhandle, const TString& newname)
{
    int res = renameat(handle.GetHandle(), name.data(), newhandle.GetHandle(), newname.data());
    Y_ENSURE_EX(res != -1, TServiceError(GetSystemErrorCode())
...
TVector<std::pair<TString, TFileStat>> ListDirAt(const TFile& handle)
{
