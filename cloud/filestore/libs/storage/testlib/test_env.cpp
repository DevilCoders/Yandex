    ...
    , NextDynamicNode(Config.StaticNodes)
{
    ...
    StorageCounters = CreateStorageCounters(Runtime.GetAppData().Counters);
    StatsRegistry = CreateRequestStatsRegistryStub();
}
...
ui32 TTestEnv::CreateNode(const TString& name)
{
    ...
    auto indexService = CreateStorageService(StorageConfig, StatsRegistry);
