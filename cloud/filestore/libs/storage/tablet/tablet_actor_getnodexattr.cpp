    ...
    const TActorContext& ctx)
{
    ...
    FILESTORE_VALIDATE_EVENT_SESSION(GetNodeXAttr, msg->Record);
    ...
    if (nodeId == InvalidNodeId) {
        auto response = std::make_unique<TEvService::TEvGetNodeXAttrResponse>(
    ...
    if (auto error = ValidateXAttrName(name); HasError(error)) {
        auto response = std::make_unique<TEvService::TEvGetNodeXAttrResponse>(error);
        NCloud::Reply(ctx, *ev, std::move(response));
        return;
    }
