...
TStringBuf TFreshBlocks::AllocateBlock(TStringBuf content)
{
    auto block = Allocator->Allocate(content.Size());
    memcpy(block.Data, content.Data(), content.Size());
    return { static_cast<const char*>(block.Data), content.Size() };
}
...
bool TFreshBlocks::AddBlock(
    ...
    ui64 maxCommitId)
{
    auto content = AllocateBlock(blockData);
