...
struct TDisjointRangeMap: TSet<TDeletedRange, TDeletedRangeLess, TStlAllocator>
{
    ...
    TDisjointRangeMap()
        : TBase(GetAllocatorByTag(EAllocatorTag::DeletionMarkers))
    {}
};
...
private:
    TDisjointRangeMapByNodeId DisjointRangeMapByNodeId {
        GetAllocatorByTag(EAllocatorTag::DeletionMarkers)
    };
...
{
    auto& map = DisjointRangeMapByNodeId[deletionMarker.NodeId];
...
TDeletionMarkers::TDeletionMarkers()
    : Impl(new TImpl())
{}
