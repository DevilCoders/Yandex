...
Y_UNIT_TEST_SUITE(TIndexTabletTest_Data)
{
    ...
    Y_UNIT_TEST(ShouldFlushFreshBytesByLargeOffset)
    {
        ...
        // writing fresh block, then bytes
        tablet.WriteData(handle, 13_GB, 10, 'a');
        ...
        TString expected;
        expected.ReserveAndResize(15);
        memset(expected.begin(), 'a', 5);
        memset(expected.begin() + 5, 'b', 10);
    ...
    Y_UNIT_TEST(ShouldFlushAfterEmptyFlush)
    {
...

        {
            auto response = tablet.GetStorageStats();
            const auto& stats = response->Record.GetStats();
            UNIT_ASSERT_VALUES_EQUAL(stats.GetFreshBytesCount(), 4_KB);
        }
    ...
    Y_UNIT_TEST(ShouldNotWriteToYellowGroups)
    {
        ...
        tablet.WriteData(handle, 16_KB, 1_KB, 'a'); // fresh bytes
