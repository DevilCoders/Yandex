...
public:
    ...
    NThreading::TFuture<TCompletedRequest> ExecuteNextRequest() override
    {
        ...
        case NProto::ACTION_WRITE:
            return DoWrite();
        default:
            Y_FAIL("unexpected action: %u", (ui32)action);
        }
    }
...
private:
    ...
    TFuture<TCompletedRequest> DoCreateHandle()
    {
        ...
        return Session->CreateHandle(Ctx, std::move(request)).Apply(
        ...
        TInstant started)
    {
        try {
            ...
            if (InitialFileSize) {
                ...
                setAttr = Session->SetNodeAttr(Ctx, std::move(request));
            } else {
    ...
    TFuture<TCompletedRequest> DoRead()
    {
        ...
        auto self = weak_from_this();
        return Session->ReadData(Ctx, std::move(request)).Apply(
        ...
        ui64 byteOffset)
    {
        try {
            ...
            for (ui64 offset = 0; offset < ReadBytes; offset += SEGMENT_SIZE) {
                ...
                if (Spec.GetValidationEnabled()) {
                    Y_VERIFY(handleInfo.Segments);

                    auto& segments = *handleInfo.Segments;
                    Y_VERIFY(segmentId < segments.size());
    ...
    TFuture<TCompletedRequest> DoWrite(THandleInfo handleInfo = {})
    {
        ...
        *request->MutableBuffer() = std::move(buffer);
        ...
        auto self = weak_from_this();
        return Session->WriteData(Ctx, std::move(request)).Apply(
