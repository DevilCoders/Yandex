...
Creates/deletes monrun config
Only 'command' and 'name' are mandatory.
By default 'file' is '/etc/monrun/conf.d/' + name + '.conf'

Upstream: https://github.yandex-team.ru/Salt/salt/blob/master/salt/states/monrun.py
'''
...
def _regenerate_tasks():
    regenerate_bin = '/usr/bin/monrun --gen-jobs >/dev/null 2>&1'

    result = os.system(regenerate_bin)
    if result == 0:
        log.info("Regenerating manifest.json succeeded.")
        return True
    else:
        log.info("Regenerating manifest.json failed.")
        return False
...
def present(name,
    ...
    for kwarg in kwargs.keys():
        if kwarg in ('__id__', 'fun', 'state', '__env__', '__sls__',
    ...
    if os.path.exists(file):
        old_config = ConfigParser.ConfigParser(None, OrderedDict)
        if not replace:
    ...
    new_config = ConfigParser.ConfigParser(None, OrderedDict)
    for key in sorted(checks):
        values = dict(checks[key])
        for global_key in monitoring_conf:
            if not global_key in values:
                values[global_key] = monitoring_conf[global_key]
        if not 'command' in values:
            if not command:
