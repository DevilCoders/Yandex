...
/usr/local/bin/spasibo-krasj:
  file.managed:
    - source: salt://{{ slspath }}/bin/spasibo-krasj.py
    - mode: 0755
    - require:
...
configure-underlay:
  service.running:
    ...
    - require:
      ...
      - file: /usr/local/bin/spasibo-krasj
      - file: /etc/yc/yc-network-profiles.json
...
/etc/systemd/system/configure-underlay.service:
  file.managed:
    - source: salt://{{ slspath }}/conf/configure-underlay.service.j2
    - template: jinja
    - require:
...
/usr/local/bin/configure_underlay_devices.py:
  file.managed:
    - source: salt://{{ slspath }}/bin/configure_underlay_devices.py
    - mode: 0755
...
/etc/systemd/system/tune-host-networking.service:
  file.managed:
    - source: salt://{{ slspath }}/conf/tune-host-networking.service.j2
    - template: jinja
    - require:
      - file: /usr/local/bin/tune_host_networking.py
...
/usr/local/bin/tune_host_networking.py:
  file.managed:
    - source: salt://{{ slspath }}/bin/tune_host_networking.py
    - mode: 0755

/etc/yc/tune-host-networking.yaml:
  file.managed:
    - template: jinja
    - source: salt://{{ slspath }}/conf/tune-host-networking.yaml
{%- endif %}
