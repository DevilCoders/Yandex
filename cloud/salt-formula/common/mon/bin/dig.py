...
class MonrunException(Exception):
    ...
    def __str__(self):
        return "stage %s failed %s %s" % (self.stage, self.wrapped_exc.__class__.__name__, self.wrapped_exc)
...
def gen_config():
    global CONFIG
    config_path = '/home/monitor/agents/etc/dig.conf'
    ...
    CONFIG['YANDEX_NS_SERVERS'] = [item.strip() for item in config.get('dig', 'yandex_ns_servers').split(',')]
    CONFIG['COMMON_YANDEX_HOSTNAME'] = config.get('dig', 'common_yandex_hostname')
...
def resolve_nameservers(ns_servers, timeout=None):
    ...
    for ns_servername in ns_servers:
        ...
        try:
            ans = resolver.query(ns_servername, ttype, raise_on_no_answer=False)
        except dns.exception.DNSException as err:
            raise MonrunException(ns_servername, err)
        answer.extend((str(i) for i in ans))
    if not answer:
        raise MonrunException("resolve_nameservers", Exception("empty IPS result"))
    return answer
...
def main():
    ...
    try:
        ...
        yandex_ns_servers = resolve_nameservers(CONFIG['YANDEX_NS_SERVERS'], timeout=TIMEOUT)
        dig(CONFIG['COMMON_YANDEX_HOSTNAME'], yandex_ns_servers, timeout=TIMEOUT)
