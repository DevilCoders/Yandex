package Ubic::Service::S3MdsLFWorker;
...
sub new {
    ...
    $params->{bin} = "/usr/bin/s3-kikimr-bucket-lifecycle-worker --config $config";
...
sub start_impl {
    ...
    Ubic::Service::Shared::Dirs::directory_checker( "/var/log/s3/lf/worker/", $self->{daemon_user} );
...
# /etc/ubic/service/s3-lf-worker.json
{
    "module": "Ubic::Service::S3MdsLFWorker",
    "options": {
        "config": "/etc/s3/s3-lifecycle-worker.conf",
        "timeout_options": {
            "start": {
                "step": 1,
                "trials": 3
        ...
        "ubic_log": "/var/log/s3/lf/worker/ubic.log",
        "stdout": "/var/log/s3/lf/worker/ubic-stdout.log",
        "stderr": "/var/log/s3/lf/worker/ubic-stderr.log"
    }
}
=head1 AUTHOR
