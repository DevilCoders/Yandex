package Ubic::Service::S3MdsLFSched;
...
sub new {
    ...
    $params->{bin} = "/usr/bin/s3-kikimr-bucket-lifecycle-scheduler --config $config";
...
sub start_impl {
    ...
    Ubic::Service::Shared::Dirs::directory_checker( "/var/log/s3/lf/scheduler/", $self->{daemon_user} );
...
Ubic::Service::S3MdsLFSched - run S3 LF sched as Ubic service

=head1 SYNOPSIS

# /etc/ubic/service/s3-lf-scheduler.json
{
    "module": "Ubic::Service::S3MdsLFSched",
    "options": {
        "config": "/etc/s3/s3-lifecycle-scheduler.conf",
        "timeout_options": {
            "start": {
                "step": 1,
                "trials": 3
        ...
        "env": {
            "SCHEMAS_XSD_DIR": "/etc/s3-mds-proxy-arc/xsd/"
        },
        "user": "root",
        "daemon_user": "s3proxy",
        "ulimit": {
        ...
        "ubic_log": "/var/log/s3/lf/scheduler/ubic.log",
        "stdout": "/var/log/s3/lf/scheduler/ubic-stdout.log",
        "stderr": "/var/log/s3/lf/scheduler/ubic-stderr.log"
    }
}
=head1 AUTHOR
