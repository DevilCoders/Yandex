...
{#- Heap sizes are forced temporarily to help in mitigation 
   of CLOUD-12543 "contrail-named is killed by OOM on PROD" #}
...
# CLOUD-5129: Add jolokia - gate to JMX
JOLOKIA_FILENAME="jolokia.jar"
JOLOKIA_PORT="10000"
if [ -e "$CASSANDRA_HOME/lib/$JOLOKIA_FILENAME" ]; then
  JVM_OPTS="$JVM_OPTS -javaagent:$CASSANDRA_HOME/lib/$JOLOKIA_FILENAME=port=$JOLOKIA_PORT"
fi
