...
def get_argparser():
    ...
    saint -ch <host_fqdn>                                   show compute host info
    saint -ch <host_fqdn> -o                                list compute host operations
    saint -ch <host_fqdn> -l                                list instances on compute host
    ...
    # Compute host
    compute_hosts_group = parser.add_argument_group('Compute hosts')
    compute_hosts_group.add_argument('--compute-host', '-ch', '--сщьзгеу-рщые', '-ср', type=str,
                             help='compute host resolver')
    compute_hosts_group.add_argument('--list', '-l', '--дшые', '-д', action="store_true")
