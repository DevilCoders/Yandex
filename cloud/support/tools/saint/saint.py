        ...
        if args.verbose_masters:
            for master_instance in instances_by_folder:
                printers.InstancePrinter(args).print(
                    cloud_facade.format_instance_full(master_instance, view='BASIC'))
                disks = cloud_facade.get_instances_disks(master_instance)
...
#ToDo: отсюда и далее. Проверяем, не None ли между получением данных и отправкой их на форматирование
    ...
    if folder:
        ...
        if args.operations:
            ...
            operations = [cloud_facade.format_operation(op) for op in
                          cloud_facade.folder_operations_list(args.folder)]
            if operations:
                printers.OperationListPrinter(args).print(operations)
            else:
                print(f"No recent operations for folder {folder}. Try YQL")
    else:
    ...
    if instance:
        ...
        print(f'The instance {args.instance} not found.')
elif args.disk:
    """
    ...
    if user_roles:
        printers.AccountRolesPrinter(args).print(cloud_facade.format_user_roles(user_roles.access_bindings))
    else:
    ...
    '--compute-host', '-ch'
    """
    compute_host = cloud_facade.get_compute_host(args.compute_host)
    if compute_host:
        printers.NodePrinter(args).print(cloud_facade.format_compute_host_info(compute_host))
        if args.list:
            """
            '--list', '-l'
            """
            instances = cloud_facade.list_compute_host_instances(args.compute_host).instances
            if instances:
                printers.ComputeNodeInstancesListPrinter(args).print(cloud_facade.format_instances_list_full(instances))
        if args.operations:
            """
            '--operations', '-o'
            """
            operations = [cloud_facade.format_operation(op) for op in
                          cloud_facade.list_compute_host_operations(args.compute_host)]
            if operations:
                printers.OperationListPrinter(args).print(operations)
            else:
                print(f"No recent operations for host {compute_host.name}. Try YQL")
    else:
        print(f"Compute node with FQDN {args.compute_host} not found. Check the FQDN and try again.")
    ...
    '--managed-db', '-mdb'
    """
    grpc_clusters = ["kafka", "elasticsearch", "greenplum", "sqlserver"]
    if args.id:
        """
        '--id'
        """
        ...
        cluster = None
    if cluster:
        if cluster.get("type") in grpc_clusters:
            cluster_formatted = cloud_facade.cluster_formatted(cluster)
            printers.MdbClusterPrinter(args).print(cluster_formatted)
            printers.MdbHostsListPrinter(args).print(cluster_formatted.get('hosts', []))
        elif cluster.get("type") not in grpc_clusters:
        ...
        if args.operations:
            ...
            if db_type in grpc_clusters:
                operations = cloud_facade.grpc_cluster_operation_get(cluster_id, db_type=db_type)
                printers.OperationListPrinter(args).print(operations)
            elif db_type not in grpc_clusters:
    ...
    """
    data = cloud_facade.bin_check_dop(args.bin_check)
    if data:
        printers.BankCardPrinter(args).print(data)
    else:
        print(f"Unknown bin {args.bin_check_dop}")
