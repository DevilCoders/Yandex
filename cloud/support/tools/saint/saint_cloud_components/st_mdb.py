...
from cloudfacade import *
import asyncio

class St_MDB:
    async def get_cluster_kafka(self, cluster_id):
        get = self.use_grpc(self.cluster_kafka_channel.Get,
                            cluster_kafka_service_pb2.GetClusterRequest(cluster_id=cluster_id))
        if get:
            check_dict = {"type": "kafka", "get": get, "listhosts": "", "cluster_id": cluster_id}
            if listhosts := self.use_grpc(self.cluster_kafka_channel.ListHosts,
                                      cluster_kafka_service_pb2.GetClusterRequest(cluster_id=cluster_id)):
                check_dict["listhosts"] = listhosts

            return check_dict

    async def get_cluster_elasticsearch(self, cluster_id):
        get = self.use_grpc(self.cluster_elasticsearch_channel.Get,
                            cluster_elasticsearch_service_pb2.GetClusterRequest(cluster_id=cluster_id))
        if get:
            check_dict = {"type": "elasticsearch", "get": get, "listhosts": "", "cluster_id": cluster_id}
            if listhosts := self.use_grpc(self.cluster_elasticsearch_channel.ListHosts,
                                      cluster_elasticsearch_service_pb2.GetClusterRequest(cluster_id=cluster_id)):
                check_dict["listhosts"] = listhosts

            return check_dict

    async def get_cluster_greenplum(self, cluster_id):
        get = self.use_grpc(self.cluster_greenplum_channel.Get,
                            cluster_greenplum_service_pb2.GetClusterRequest(cluster_id=cluster_id))
        if get:
            check_dict = {"type": "greenplum", "get": get, "listmasterhosts": "",
                          "listsegmenthosts": "", "cluster_id": cluster_id}
            if listmasterhosts := self.use_grpc(self.cluster_greenplum_channel.ListMasterHosts,
                                            cluster_greenplum_service_pb2.GetClusterRequest(cluster_id=cluster_id)):
                check_dict["listmasterhosts"] = listmasterhosts
            if listsegmenthosts := self.use_grpc(self.cluster_greenplum_channel.ListSegmentHosts,
                                            cluster_greenplum_service_pb2.GetClusterRequest(cluster_id=cluster_id)):
                check_dict["listsegmenthosts"] = listsegmenthosts

            return check_dict

    async def get_cluster_sqlserver(self, cluster_id):
        get = self.use_grpc(self.cluster_sqlserver_channel.Get,
                            cluster_sqlserver_service_pb2.GetClusterRequest(cluster_id=cluster_id))
        if get:
            check_dict = {"type": "sqlserver", "get": get, "listhosts": "", "cluster_id": cluster_id}
            if listhosts := self.use_grpc(self.cluster_sqlserver_channel.ListHosts,
                        cluster_sqlserver_service_pb2.GetClusterRequest(cluster_id=cluster_id)):
                check_dict["listhosts"] = listhosts
    ...
    def cluster_resolve(self, data, id_type=None):
        ...
        tasks = self.get_cluster_kafka(cluster_id=data), self.get_cluster_elasticsearch(cluster_id=data), self.get_cluster_greenplum(cluster_id=data), self.get_cluster_sqlserver(cluster_id=data)
        kafka, elasticsearch, greenplum, sqlserver = loop.run_until_complete(asyncio.gather(*tasks))
        if kafka:
            return kafka
        elif elasticsearch:
    ...
    def grpc_cluster_operation_get(self, cluster_id, db_type=None):

        if db_type == "kafka":
            listoperations = self.use_grpc(self.cluster_kafka_channel.ListOperations,
                                      cluster_kafka_service_pb2.GetClusterRequest(cluster_id=cluster_id))
        elif db_type == "elasticsearch":
            listoperations = self.use_grpc(self.cluster_elasticsearch_channel.ListOperations,
                                           cluster_elasticsearch_service_pb2.GetClusterRequest(cluster_id=cluster_id))
        elif db_type == "greenplum":
            listoperations = self.use_grpc(self.cluster_greenplum_channel.ListOperations,
                                           cluster_greenplum_service_pb2.GetClusterRequest(cluster_id=cluster_id))
        elif db_type == "sqlserver":
            listoperations = self.use_grpc(self.cluster_sqlserver_channel.ListOperations,
                                           cluster_sqlserver_service_pb2.GetClusterRequest(cluster_id=cluster_id))

        operations_list = list(listoperations.operations)
        final = []
        ...
        for operation in operations_list:
            operation_result = {
                ...
                'created_at': datetime.utcfromtimestamp(int(operation.created_at.seconds)).strftime('%Y-%m-%d %H:%M:%S'),
                'modified_at': datetime.utcfromtimestamp(int(operation.modified_at.seconds)).strftime('%Y-%m-%d %H:%M:%S'),
                'created_by': operation.created_by,
                'done': operation.done,
                'error': ""
            }
            final.append(operation_result)
