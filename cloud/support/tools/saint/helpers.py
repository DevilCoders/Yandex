...
from profiles import Profile, WellKnownProfiles
from endpoints import Endpoints
from printers import Color, get_bytes_size_string
...
def load_profiles_from_config(config: configparser.RawConfigParser):
    ...
    for profile_name in profile_names:
        ...
        if config.has_section(keyauth_section_name):
            profile_static_keys = {
                'id': config.get(keyauth_section_name, 'key_id'),
                'acc_id': config.get(keyauth_section_name, 'account_id'),
                'private_key': config.get(keyauth_section_name, 'account_private_key')
            }
        else:
            profile_static_keys = {}
