...
class ListPrinter(AbstractPrinter):
    ...
    def print_as_yaml(self, data):
        for element in data:
            return f'<pre>{yaml.dump(element, allow_unicode=True, sort_keys=False)}</pre>'

    def print_as_json(self, data):
        for element in data:
            return f'<pre>{json.dumps(element, ensure_ascii=False)}</pre>'
    ...
    def print_as_table(self, data):
        ...
        if not self.args.sort:
            return f'{table.get_string(sortby=self.default_sort_by)}'


        # else sort by specified key
        try:
            return f'{table.get_string(sortby=self.args.sort.upper())}'
        except Exception:
...
class MetadataPrinter(SimplePrinter):
    ...
    def print_as_table(self, metadata):
        super().print_as_table(self._convert(metadata))
