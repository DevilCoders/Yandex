resource "ycp_compute_instance" "mdb-report_hosts" {
  count = 3

  name                      = format(var.name_format, "mdb-report", count.index + 1, var.zone_suffix)
  zone_id                   = var.zone_id
  platform_id               = "standard-v2"
  fqdn                      = format(var.fqdn_format, "mdb-report", count.index + 1, var.zone_suffix, var.fqdn_suffix)
...
resource "ycp_compute_placement_group" "mdb-report_pg" {
  name = "mdb-report-pg"
