resource "ycp_compute_instance" "mdb-secretsdb_hosts" {
  lifecycle {
    prevent_destroy = true
  }
  count = 3

  name                      = format(var.name_format, "mdb-secretsdb", count.index + 1, var.zone_suffix)
  zone_id                   = var.zone_id
  platform_id               = "standard-v2"
  fqdn                      = format(var.fqdn_format, "mdb-secretsdb", count.index + 1, var.zone_suffix, var.fqdn_suffix)
...
resource "ycp_storage_bucket" "secretsdb" {
  bucket = "mdb-backup-secretsdb"
}

resource "ycp_compute_placement_group" "mdb-secretsdb_pg" {
  name = "mdb-secretsdb-pg"
