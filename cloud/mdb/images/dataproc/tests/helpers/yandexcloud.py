...
EVICTION_TIME_FORMAT = '%Y-%m-%d-%H%M%S-%Z'
...
def date_encode(dt):
    """
    Encode datetime to acceptable label format
    """
    return str(dt.strftime(EVICTION_TIME_FORMAT).lower())


def date_decode(dt):
    """
    Decode datetime from acceptable label format
    """
    return datetime.strptime(dt.upper(), EVICTION_TIME_FORMAT).replace(tzinfo=timezone.utc)
...
def setup_labels(ctx):
    ...
    """
    ctx.labels = {
        'owner': os.environ.get('OWNER', getpass.getuser()),
    }
    ctx.labels['commit'] = os.environ.get('COMMIT', 'null-commit')
    ctx.labels['branch'] = os.environ.get('BRANCH', 'null-branch')
    ctx.labels['pr'] = os.environ.get('ARCANUMID', 'null')
    ctx.labels['created_at'] = date_encode(datetime.now(tz=timezone.utc))


def is_evictable(ctx, resource, ttl=timedelta(hours=3)):
    """
    ...
    if created_at:
        try:
            now = date_decode(ctx.labels['created_at'])
            event = date_decode(created_at)
            if now - event >= ttl:
                # resource have ttl mark and should be evicted
                return True
            # resource have ttl mark, but ttl isn't over
            return False
        except ValueError:
            LOG.exception(f'Failed to parse datetime {created_at}, for resource {resource}')
            return False
    return True


def delete_resources(ctx, ttl=None):
    """
    ...
    for instance in compute.instances_list(ctx):
        if is_evictable(ctx, instance, ttl=ttl):
    ...
    for cluster in mdb_clickhouse.clusters_list(ctx):
        if is_evictable(ctx, cluster, ttl=ttl):
    ...
    for log_group in cloud_logging.log_groups_list(ctx):
        if is_evictable(ctx, log_group, ttl=ttl):
    ...
    if ctx.state.get('s3'):
        for bucket in s3.bucket_list(ctx):
            if is_evictable(ctx, bucket, ttl=ttl):
    ...
    for service_account in iam.service_accounts_list(ctx):
        if is_evictable(ctx, service_account, ttl=ttl):
...
def setup_environment(ctx):
    ...
    ctx.state['render'] = {
        ...
        'pr': ctx.labels['pr'],
        'created_at': ctx.labels['created_at'],
        'conf': ctx.conf,
...
def teardown_environment(ctx):
    ...
    ssh.clean_key(ctx)
    ttl_minutes = os.environ.get('RESOURCES_SWEEP_TTL_MINUTES')
    delete_resources(ctx, ttl=ttl_minutes and timedelta(minutes=float(ttl_minutes)))
