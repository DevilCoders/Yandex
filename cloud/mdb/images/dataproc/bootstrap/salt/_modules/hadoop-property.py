...
def config_site_properties_present(config_path, properties, dry_run=False, splitter=',', joiner=','):
    """
    ...
    for child_node in root.iter('property'):
        ...
        if name_node.text in new_props:
            ...
            if p_append:
                ...
                current_values = [x.strip() for x in value_node.text.split(splitter)]
                new_values = []
                for p_value_item in p_value.split(splitter):
                    if p_value_item not in current_values:
                        new_values.append(p_value_item)
                if new_values:
                    value_node.text = str(joiner.join(current_values + new_values))
                    has_changes = True
    ...
    difference = difflib.unified_diff(config_before.splitlines(), config_after.splitlines())
