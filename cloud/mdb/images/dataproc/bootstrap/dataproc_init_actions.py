...
logger = logging.getLogger("dataproc_init_actions")
...
@retry(retry_on_exception=retry_if_request_exception, wait_fixed=10000)
def send_state(state, fqdn, masternode):
    headers = {'Content-Type': 'application/json'}
    data = {"host": fqdn, "status": state}
    addr = f'http://{masternode}:5555/initacts'
...
def get_environment():
    env_vars = {
        ...
        "ROLE": " ".join(_get_metadata("ydputils.roles")),
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        for init_action in init_actions_list:
            ...
            try:
                ...
                return "FAILED"
            retcode = _execute_initialization_action(tmpfile, args, timeout, env)
            if retcode != 0:
                return "FAILED"
...
def _execute_initialization_action(userfile, args, timeout, env):
    ...
    if retcode != 0:
        ...
        logger.info("Successful execution of %s", userfile)
...
def main():
    ...
    state = _download_and_execute(init_actions_list)

    if _get_metadata("ydputils.get_send_states_activated"):
        fqdn = _get_metadata("ydputils.get_current_fqdn")
        masternode = _get_metadata("ydputils.get_masternodes")[0]
        send_state(state, fqdn, masternode)
