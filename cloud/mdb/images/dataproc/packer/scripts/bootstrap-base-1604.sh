#!/bin/bash
# Script for installing base components for Yandex Cloud Dataproc image.
...
setup_apt_mirrors() {
    ...
    REPO="http://mirror.yandex.ru/mirrors/repo.saltstack.com/apt/ubuntu/16.04/amd64/archive/${SALT_VERSION}/"
    cat <<EOF > "/etc/apt/sources.list.d/saltstack.list"
deb ${REPO} xenial main
EOF
...
# Currently our dataproc distribution doesn't support  systemd units, so, use sysvinit
setup_sysvinit() {
    DEBIAN_FRONTEND=noninteractive apt-get install -y upstart-sysv sysvinit-utils || log_fatal "Can't install sysvinit"
}

setup_salt() {
    DEBIAN_FRONTEND=noninteractive apt-get install -y salt-minion salt-common virt-what python-retrying || log_fatal "Can't install salt"
    update-rc.d -f salt-minion remove || log_fatal "Failed to remove  salt-minion from autostart"
}
...
setup_conda() {
    ...
    # Increase overall timeout, because some of packages are really large
    $CONDA_BIN_PATH/conda config --set remote_read_timeout_secs 600.0
}

setup_conda_packages () {
    $CONDA_BIN_PATH/conda install python=3.7 \
        pyarrow=0.13.0 \
        ipykernel=5.1.3 \
        tensorflow=1.15.0 \
        conda-forge::catboost=0.20 \
        conda-forge::lightgbm=2.3.0 \
        conda-forge::xgboost=0.90 \
        scikit-learn=0.21.3 \
        pandas=0.25.3  \
        ipython=7.9.0 \
        matplotlib=3.1.1 \
        ...
        py4j=0.10.7 \
        pyspark=2.4.6 \
        pyhive=0.6.1 || log_fatal "Failed to install conda packages"
}
