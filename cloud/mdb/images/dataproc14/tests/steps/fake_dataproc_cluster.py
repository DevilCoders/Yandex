...
def set_property_to_value(ctx, name, value):
    ...
    if not props.get(section):
        props[section] = {}
    props[section][property_name] = value
...
def step_execute_command(ctx, command=None):
    """
    Execute command on masternode
    """
    if not command:
        if not ctx.text:
...
def step_command_output_contains(ctx):
    ...
    ctx.text = utils.render_template(ctx.text, ctx)
    assert_that(command['stdout'].decode('utf-8'), contains_string(textwrap.dedent(ctx.text)))
...
def step_command_err_contains(ctx):
    ...
    ctx.text = utils.render_template(ctx.text, ctx)
    assert_that(command['stderr'].decode('utf-8'), contains_string(textwrap.dedent(ctx.text)))
