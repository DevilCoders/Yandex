...
BOOTPATH = '/var/lib/cloud/scripts/per-boot/dataproc-start.sh'
...
def sync_bootstrap(ctx, names):
    ...
    for name, p in tasks:
        try:
            ...
            raise Exception(f'Failed to sync {name}', exc)


def bootstrap(ctx, names, timeout=900):
    """
    ...
    def _bootstrap(user, addr, timeout):
        ...
        return sh.ssh(*utils.ssh_options(ctx),
                      f'{user}@{addr}',
                      f'sudo FORCE=true {BOOTPATH} 2>&1 > /var/log/dataproc_image_test_bootstrap.log',
                      _bg=True,
                      _no_out=True,
                      _no_err=True,
                      _timeout=timeout)
    ...
    for name, run in tasks:
        try:
            run.wait()
        except (sh.TimeoutException, sh.ErrorReturnCode) as exception:
            raise Exception(f'name={name} run={run} exception={exception}')
...
def setup_firewall(ctx, names):
    ...
    for name in names:
        ...
        try:
            ...
            raise Exception('Failed to setup firewall', exc)
...
def service_is_running(ctx, instances, service):
    ...
    for instance, command in tasks:
        try:
            ...
            LOG.warn(f'service {service} on instance {instance} is broken, code: {code}, out: {out}, err: {err}')
            ret[instance] = False
    return ret
...
def package_installed(ctx, instances, package):
    ...
    for instance, command in tasks:
        ...
        try:
            ...
            code, out, err = str(command.exit_code), str(command.stdout), str(command.stderr)
            raise Exception(f'Failed package_installed on {instance}, code: {code}, out: {out}, err: {err}',
                            exc_info=True)
    return ret
...
def dataproc_create(ctx,
    ...
    if not topology:
        ...
        topology = (1, 1, 1)
    masternodes, datanodes, computenodes = generate_instance_names(f'{ctx.id}-{name}', topology)
    ...
    ssh_keys = [ctx.state['ssh_public_key']]
    ...
    create_dataproc_instances(ctx, userdata, masternodes + datanodes + computenodes, timeout)
...
def dataproc_restart(ctx,
    ...
    for name in instances:
        compute.wait_ssh_available(ctx, ctx.conf['user'], compute.get_public_ip_address(ctx, name))
    bootstrap(ctx, instances, timeout=300)
    ...
    with tempfile.NamedTemporaryFile(mode='w+t') as f:
        ...
        for name, run in tasks:
            try:
                ...
                raise Exception(f'Failed to create file {name}:{file_path}, out: {out}, err: {err}', exc_info=True)
