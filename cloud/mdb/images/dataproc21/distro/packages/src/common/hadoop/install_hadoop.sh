...
cp ${BUILD_DIR}/share/hadoop/common/lib/hadoop-shaded-*.jar ${HADOOP_DIR}/
...
chmod 644 ${YARN_DIR}/timelineservice/*.jar
...
cp -a ${BUILD_DIR}/bin/yarn ${YARN_DIR}/bin
...
for library in `cd ${BUILD_DIR}/lib/native ; ls libzstd.so.1.* 2>/dev/null`; do
  cp ${BUILD_DIR}/lib/native/${library} ${HADOOP_NATIVE_LIB_DIR}/
  ldconfig -vlN ${HADOOP_NATIVE_LIB_DIR}/${library}
done
