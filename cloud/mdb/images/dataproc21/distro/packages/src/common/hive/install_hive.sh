...
DEFAULT_DIR=${DEFAULT_DIR:-$PREFIX/etc/default}
...
for jar in `ls ${HIVE_DIR}/lib/hive-*.jar | grep -v 'standalone.jar' | grep -v -F 'tests.jar'`; do
    base=`basename $jar`
    (cd ${HIVE_DIR}/lib && echo ln -sf $base ${base/-[0-9].*/.jar})
done
...
for v in hive-metastore hive-hiveserver2 hive-hcatalog-server hive-webhcat-server; do
    cp ${DISTRO_DIR}/$v.default ${DEFAULT_DIR}/$v
done
...
for template in hive-exec-log4j2.properties hive-log4j2.properties beeline-log4j2.properties llap-cli-log4j2.properties llap-daemon-log4j2.properties
do
...
cp hive-env.sh ${PREFIX}${CONF_DIST_DIR}
