...
def get_exposed_port(container, port):
    ...
    if machine_name:
        host = subprocess.check_output(['docker-machine', 'ip', machine_name]).decode('utf-8').rstrip()  # nosec
    else:
        host = 'localhost'
...
def exec_run(instance, cmd, tty=None, user=None):
    """
    Run command inside container and return exit_code and decoded output
    """
    exec_result = instance.exec_run(cmd, tty=tty, user=user)
    # exit_code, raw_output = instance.exec_run(cmd, tty=tty, user=user)
    output = str(exec_result.output.decode())
    return exec_result.exit_code, output


def pidof(context, node, service):
    """
    Get pid of given service
    """
    instance = get_container(context, node)
    cmd = '/bin/pidof {service}'.format(service=service)
    exit_code, pid = exec_run(instance, cmd)
    if exit_code:
        raise RuntimeError('Exit code of "{cmd}" was {code}'.format(cmd=cmd, code=exit_code))
    return int(pid)
...
def prep_network(state, conf):
    ...
    net_name = conf.get('network_name', 'testnet%s' % bridge_name)
    net_opts = {
...
def shutdown_network(conf, **_extra):
    ...
    """
    nets = DOCKER_API.networks.list(names=conf.get('network_name', '^testnet'))
    for net in nets:
        net.remove()
