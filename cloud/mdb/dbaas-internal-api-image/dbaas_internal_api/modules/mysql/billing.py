# -*- coding: utf-8 -*-
"""
DBaaS Internal API mysql cluster billing estimate helpers
"""
...
def billing_create_mysql_cluster(config_spec: dict, host_specs: List[dict], host_group_ids: List[str] = None, **_):
    """
    Returns billing metrics for cost estimator
    """
    try:
        # Load and validate config spec
        mysql_config_spec = MysqlConfigSpec(config_spec, fill_default=True)
    except ValueError as err:
        raise errors.ParseConfigError(err)

    resources = mysql_config_spec.get_resources()
