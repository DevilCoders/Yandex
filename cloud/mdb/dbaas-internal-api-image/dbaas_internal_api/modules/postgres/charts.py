# -*- coding: utf-8 -*-
"""
DBaaS Internal API PostgreSQL charts
"""

from flask import g

from ...utils import config
from ...utils.register import DbaasOperation, Resource, register_request_handler
from .constants import MY_CLUSTER_TYPE
from .pillar import get_cluster_pillar


@register_request_handler(MY_CLUSTER_TYPE, Resource.CLUSTER, DbaasOperation.CHARTS)
def postgresql_cluster_charts(cluster, _):
    """
    Returns cluster charts
    """
    cluster_pillar = get_cluster_pillar(cluster)
    charts = []
    for dbname in cluster_pillar.databases.get_names():
        new_charts = [
            {
                'name': chart[0],
                'description': chart[1],
                'link': chart[2].format(
                    cid=cluster['cid'],
                    dbname=dbname,
                    folderId=g.folder['folder_ext_id'],
                    cluster_type=MY_CLUSTER_TYPE,
                    console=config.get_console_address(),
                ),
            }
            for chart in config.cluster_charts(MY_CLUSTER_TYPE)
        ]
        charts.extend([chart for chart in new_charts if chart not in charts])
    return charts
