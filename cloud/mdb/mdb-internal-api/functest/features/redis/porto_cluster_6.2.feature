Feature: Create/Modify Porto Redis 6.2 Cluster
  ...
  @grpc
  Scenario Outline: Adding host with invalid params fails [grpc]
    When we "AddHosts" via gRPC at "yandex.cloud.priv.mdb.redis.v1.ClusterService" with data
    """
    {
        "cluster_id": "cid1",
    ...
    Examples:
      ...
      | [{"zone_id": "man"}, {"zone_id": "man"}] | UNIMPLEMENTED    | adding multiple hosts at once is not supported yet    |
      | [{"zone_id": "nodc"}]                    | INVALID_ARGUMENT | invalid zone nodc, valid are: iva, man, myt, sas, vla |

  @grpc
  Scenario Outline: Adding host without cid fails
    When we "AddHosts" via gRPC at "yandex.cloud.priv.mdb.redis.v1.ClusterService" with data
    """
    {
        "host_specs": <hosts>
    ...
    Examples:
      | hosts                                    | code             | message                               |
      | [{"zone_id": "man"}]                     | INVALID_ARGUMENT | cluster id is not set                        |
  ...
  Scenario: Scaling cluster up to upper min disk range fails
    When we PATCH "/mdb/redis/1.0/clusters/cid1" with data
    """
    {
        "configSpec": {
  ...
  Scenario: Cluster move works
    When we POST "/mdb/redis/1.0/clusters/cid1:move" with data
    """
    {
  ...
  @decommission @geo @grpc @grpc_api
  Scenario: Create cluster host in decommissioning geo [grpc]
    Given feature flags
    """
