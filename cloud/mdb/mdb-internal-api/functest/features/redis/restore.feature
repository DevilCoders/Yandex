Feature: Restore Redis cluster from backup
  ...
  @events
  Scenario: Restoring to 6.0 works
    Given feature flags
    """
  ...
  @events
  Scenario: Restoring from backup to tls mode ON fails
    When we POST "/mdb/redis/1.0/clusters:restore" with data
    """
    {
        "name": "test_restored",
    ...
    {
        "code": 9,
        "message": "TLS could not be turned on for Redis version 5.0"
    }
    """

  Scenario: Restoring to 6.2 fails
    When we POST "/mdb/redis/1.0/clusters:restore" with data
    """
    {
        "name": "test_restored",
  ...
  Scenario: Restoring from backup to original folder with security group works
    ...
    And worker set "sg_id1,sg_id2" security groups on "cid2"
    When we GET "/mdb/redis/1.0/clusters/cid2"
    Then we get response with status 200 and body contains
    """
    {
