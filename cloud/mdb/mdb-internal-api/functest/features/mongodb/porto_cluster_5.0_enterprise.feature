Feature: Create/Modify Porto MongoDB Cluster 5.0 Enterprise

  Background:
    Given feature flags
    """
    ["MDB_MONGODB_ENTERPRISE"]
    """
    And default headers
    And "create" data
    """
  ...
  Scenario: Setting FCV to 4.2 fails
    ...
    {
        "code": 3,
        "message": "Error parsing configSpec: Invalid feature compatibility version for version 5.0-enterprise, allowed values: 4.4, 5.0"
    }
    """
  ...
  @events
  Scenario: Allow access to Web SQL
    When we PATCH "/mdb/mongodb/1.0/clusters/cid1" with data
    """
    {
        "configSpec": {
  ...
  @enterprise
  Scenario: Change auditlog settings works
    When we PATCH "/mdb/mongodb/1.0/clusters/cid1" with data
    """
    {
        "configSpec": {
  ...
  @enterprise
  Scenario: Change auditlog.runtimeConfiguration settings fails
    When we PATCH "/mdb/mongodb/1.0/clusters/cid1" with data
    """
    {
        "configSpec": {
    ...
    {
        "code": 12,
        "message": "Runtime audit configuration is not supported yet"
    }
    """
  ...
  Scenario: Enterprise MongoDB API-Adapter hack works
    When we POST "/mdb/mongodb/1.0/clusters?folderId=folder1" with data
    """
    {
        "name": "test-adapter",
  ...
  Scenario: Enterprise MongoDB KMIP configuration works
    When we POST "/mdb/mongodb/1.0/clusters?folderId=folder1" with data
    """
    {
        "name": "test-kmip",
