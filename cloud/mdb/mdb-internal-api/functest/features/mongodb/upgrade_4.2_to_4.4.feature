Feature: Porto MongoDB Cluster version upgrade 4.2 to 4.4
  ...
  Scenario: Upgrade 4.2 to 4.4 works
  ...
  Scenario: Upgrade 4.2 to unsupported version fails
    When we PATCH "/mdb/mongodb/1.0/clusters/cid1" with data
    """
    {
        "configSpec": {
  ...
  Scenario: Upgrade 4.2 to 4.4 with bad fcv fails
    When we PATCH "/mdb/mongodb/1.0/clusters/cid1" with data
    """
    {
        "configSpec": {
    ...
    {
        "code": 9,
        "message": "Current feature compatibility version 4.0 is invalid for upgrade to version 4.4, expected: 4.2"
    }
    """

  Scenario: Downgrade 4.4 to 4.2 fails
    When we PATCH "/mdb/mongodb/1.0/clusters/cid1" with data
    """
    {
        "configSpec": {
  ...
  Scenario: Upgrade 4.2 to 4.4 mixed with another changes fails
    When we PATCH "/mdb/mongodb/1.0/clusters/cid1" with data
    """
    {
        "configSpec": {
