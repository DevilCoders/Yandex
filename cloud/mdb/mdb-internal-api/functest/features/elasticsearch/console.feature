@elasticsearch
@grpc_api
Feature: ElasticSearch console API
  Background:
    Given default headers

  Scenario: Cluster config handler works
    When we "Get" via gRPC at "yandex.cloud.priv.mdb.elasticsearch.v1.console.ClusterService"
  ...
  Scenario: Cluster config handler works with feature flags
    ...
    """
    And we "Get" via gRPC at "yandex.cloud.priv.mdb.elasticsearch.v1.console.ClusterService" with data
    """
    {
        "folder_id": "folder1"
    }
    ...
    """
    And we add feature flag "MDB_ELASTICSEARCH_ALLOW_UNLIMITED_HOSTS" for cloud "cloud1"
    And we "Get" via gRPC at "yandex.cloud.priv.mdb.elasticsearch.v1.console.ClusterService" with data
    """
    {
        "folder_id": "folder1"
  ...
  Scenario: Billing cost estimation works
    When we "EstimateCreate" via gRPC at "yandex.cloud.priv.mdb.elasticsearch.v1.console.ClusterService" with data
    """
    {
        "folder_id": "folder1",
        "name": "test",
...
# TODO: test resource presets output
  ...
  Scenario: Cluster create config handler works
    When we "GetCreateConfig" via gRPC at "yandex.cloud.priv.mdb.elasticsearch.v1.console.ClusterService"
    Then we get response with status 200
