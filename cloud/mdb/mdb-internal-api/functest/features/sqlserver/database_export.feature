@sqlserver
@grpc_api
Feature: Export single SQLServer database
  ...
  Scenario: Export single database works
    When we "ExportBackup" via gRPC at "yandex.cloud.priv.mdb.sqlserver.v1.DatabaseService" with data
    """
    {
      "cluster_id": "cid1",
      "database_name": "testdb",
    ...
    {
      "created_by": "user",
      "description": "Export database from Microsoft SQLServer cluster to external storage",
      "done": false,
      "id": "worker_task_id2",
      "metadata": {
        "@type": "type.googleapis.com/yandex.cloud.priv.mdb.sqlserver.v1.ExportDatabaseBackupMetadata",
        "cluster_id": "cid1",
        "database_name": "testdb",
        "s3_bucket": "userbucketname",
        "s3_path": "/userpath"
  ...
  Scenario: Export single database with default path and prefix works
    When we "ExportBackup" via gRPC at "yandex.cloud.priv.mdb.sqlserver.v1.DatabaseService" with data
    """
    {
      "cluster_id": "cid1",
  ...
  Scenario: Export not existing database fails
    When we "ExportBackup" via gRPC at "yandex.cloud.priv.mdb.sqlserver.v1.DatabaseService" with data
    """
    {
      "cluster_id": "cid1",
      "database_name": "notdatabase",
      "s3_bucket": "userbucketname",
      "s3_path": "/userpath",
      "prefix": "testdb"      
    }
    """
    Then we get gRPC response error with code NOT_FOUND and message "database "notdatabase" not found"

  Scenario: Export database without service account fails
  ...
  Scenario Outline: Export with invalid bucket name fails
    When we "ExportBackup" via gRPC at "yandex.cloud.priv.mdb.sqlserver.v1.DatabaseService" with data
    """
    {
      "cluster_id": "cid1",
  ...
  Scenario Outline: Export with invalid path fails
    When we "ExportBackup" via gRPC at "yandex.cloud.priv.mdb.sqlserver.v1.DatabaseService" with data
    """
    {
      "cluster_id": "cid1",
  ...
  Scenario Outline: Export with invalid prefix fails
    When we "ExportBackup" via gRPC at "yandex.cloud.priv.mdb.sqlserver.v1.DatabaseService" with data
    """
    {
      "cluster_id": "cid1",
    ...
    """
    Then we get gRPC response error with code INVALID_ARGUMENT and message "prefix name "<prefix>" has invalid symbols"
    Examples:
    | prefix           |
    | with spaces      |
    | with'specials    |
