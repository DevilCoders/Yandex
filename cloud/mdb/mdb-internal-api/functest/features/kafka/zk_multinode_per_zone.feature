#  This feature could be used to test cluster creation with multiple ZK nodes on one zone.
#  To enable ZK multi-node per zone, patch ../../testcontext.go :
#  @@ -461,8 +461,7 @@ func initInternalAPIImpl(licenseMock stub.LicenseMock, mockdata *MockData, logsd
#  -       cfg.Logic.Kafka.ZooKeeperZones = []string{"myt", "sas", "vla"}
#  +       cfg.Logic.Kafka.ZooKeeperZones = []string{"sas", "sas", "vla"}
@kafka
@grpc_api
@skip
Feature: ZK-Multinode Create Compute Apache Kafka Cluster
  Background:
    Given default headers
    And we add default feature flag "MDB_KAFKA_CLUSTER"

  Scenario: Cluster creation with multiple ZK hosts on single zone works
