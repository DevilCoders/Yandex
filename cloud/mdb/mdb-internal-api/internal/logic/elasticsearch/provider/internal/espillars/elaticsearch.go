...
func NewCluster() *Cluster {
	pillar := &Cluster{}
	pillar.Data.ElasticSearch.Users = make(map[string]UserData)
	return pillar
}
...
func (p *Cluster) MarshalPillar() (json.RawMessage, error) {
	...
	if err != nil {
		return nil, xerrors.Errorf("failed to marshal elasticsearch cluster pillar: %w", err)
	}
...
func (p *Cluster) UnmarshalPillar(raw json.RawMessage) error {
	if err := json.Unmarshal(raw, p); err != nil {
		return xerrors.Errorf("failed to unmarshal elasticsearch cluster pillar: %w", err)
	}
	return nil
}

func (p *Cluster) AddUser(u UserData) error {
	_, ok := p.Data.ElasticSearch.Users[u.Name]
	if ok {
		return semerr.AlreadyExistsf("user %q already exists", u.Name)
	}
	p.Data.ElasticSearch.Users[u.Name] = u
	return nil
}

func (p *Cluster) GetUser(name string) (UserData, bool) {
	u, ok := p.Data.ElasticSearch.Users[name]
	return u, ok
}
...
func (p *Cluster) DeleteUser(name string) error {
	_, ok := p.Data.ElasticSearch.Users[name]
	if !ok {
		return semerr.NotFoundf("user %q not found", name)
	}
	delete(p.Data.ElasticSearch.Users, name)
	return nil
}

func (p *Cluster) SetPlugins(plugins ...string) error {
	if err := esmodels.ValidatePlugins(plugins); err != nil {
		return err
	}
	p.Data.ElasticSearch.Plugins = plugins
	return nil
}

func (p *Cluster) Version() esmodels.Version {
	return p.Data.ElasticSearch.Version
}

func (p *Cluster) Edition() esmodels.Edition {
	return p.Data.ElasticSearch.Edition
}

func (p *Cluster) SetAuthProviders(ap *esmodels.AuthProviders) error {
	return p.Data.ElasticSearch.Auth.setAuthProviders(ap, p.Data.ElasticSearch.Edition)
}

func (p *Cluster) AuthProviders() (*esmodels.AuthProviders, error) {
	return p.Data.ElasticSearch.Auth.authProviders()
}

func (p *Cluster) Extensions() *esmodels.Extensions {
	return esmodels.NewExtensions(p.Data.ElasticSearch.Extensions)
}

func (p *Cluster) SetExtensions(e *esmodels.Extensions) {
	p.Data.ElasticSearch.Extensions = e.GetList()
}

func (p *Cluster) SetAccess(a esmodels.OptionalAccess) {
...
type ClusterData struct {
	...
	ElasticSearch             CommonClusterData       `json:"elasticsearch"`
...
func NewDataNodeSubCluster() *DataNodeSubCluster {
	...
	pillar.Data.ElasticSearch.IsData = true
	pillar.Data.ElasticSearch.Kibana.Enabled = true
	return pillar
}
...
func (p *DataNodeSubCluster) MarshalPillar() (json.RawMessage, error) {
	raw, err := json.Marshal(p)
	if err != nil {
		return nil, xerrors.Errorf("failed to marshal elasticsearch data node pillar: %w", err)
	}
...
func (p *DataNodeSubCluster) UnmarshalPillar(raw json.RawMessage) error {
	if err := json.Unmarshal(raw, p); err != nil {
		return xerrors.Errorf("failed to unmarshal elasticsearch data node pillar: %w", err)
	}

	return nil
}

func (p *DataNodeSubCluster) UpdateConfig(config esmodels.DataNodeConfig) bool {
	updated := false
	pillarConfig := &p.Data.ElasticSearch.Config.DataNode
...
func NewMasterNodeSubCluster() *MasterNodeSubCluster {
	...
	pillar.Data.ElasticSearch.IsMaster = true
	return pillar
}
...
func (p *MasterNodeSubCluster) MarshalPillar() (json.RawMessage, error) {
	raw, err := json.Marshal(p)
	if err != nil {
		return nil, xerrors.Errorf("failed to marshal elasticsearch master node subcluster pillar: %w", err)
	}
...
func (p *MasterNodeSubCluster) UnmarshalPillar(raw json.RawMessage) error {
	if err := json.Unmarshal(raw, &p); err != nil {
		return xerrors.Errorf("failed to unmarshal elasticsearch master node subcluster pillar: %w", err)
	}
...
type CommonClusterData struct {
	Version esmodels.Version    `json:"version"`
	Edition esmodels.Edition    `json:"edition"`
	...
	Extensions []esmodels.Extension `json:"extensions,omitempty"`
}

type DataNodeSubClusterData struct {
	ElasticSearch DataNodeElasticSearch `json:"elasticsearch"`
}

type MasterNodeSubClusterData struct {
	ElasticSearch MasterNodeElasticSearch `json:"elasticsearch"`
}

type DataNodeElasticSearch struct {
...
type MasterNodeElasticSearch struct {
...
type DataNodeConfig struct {
	DataNode esmodels.DataNodeConfig `json:"data_node,omitempty"`
}
