...
func setupDeleteHost(t *testing.T, excludes ...option) (context.Context, *elasticsearchFixture) {
	ctx, f := newElasticsearchFixture(t)

	f.Operator.EXPECT().ModifyOnCluster(ctx, ClusterID, clusters.TypeElasticSearch, gomock.Any(), gomock.Any()).
...
func TestElasticsearch_DeleteHosts(t *testing.T) {
	ctx, f := setupDeleteHost(t)
	defer f.Finish()

	_, err := f.Elasticsearch.DeleteHosts(ctx, ClusterID, []string{"d1"})
	require.NoError(t, err, "should successfully delete data node")
}

func TestElasticsearch_DeleteHosts_ListPaging(t *testing.T) {
...
func setupAddHosts(t *testing.T, excludes ...option) (context.Context, *elasticsearchFixture) {
	ctx, f := newElasticsearchFixture(t)
...
func TestElasticsearch_AddHosts(t *testing.T) {
	ctx, f := setupAddHosts(t)
	defer f.Finish()

	_, err := f.Elasticsearch.AddHosts(ctx, ClusterID, []esmodels.Host{{Role: hosts.RoleElasticSearchDataNode}})
	require.NoError(t, err, "should successfully add data node")
}

func TestElasticsearch_AddHosts_ListPaging(t *testing.T) {
