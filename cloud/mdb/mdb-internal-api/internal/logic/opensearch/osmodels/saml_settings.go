...
func (s SamlSettings) validate() error {
	...
	if err := s.validateDashboardsURL(); err != nil {
		return err
	}
...
func (s SamlSettings) validateDashboardsURL() error {
	ku := s.DashboardsURL

	if ku == "" {
		return semerr.InvalidInput("dashboards_url is required")
	}

	u, err := url.Parse(ku)
	if err != nil || u.Scheme == "" || u.Host == "" {
		return semerr.InvalidInput("dashboards_url isn't a url")
	}
