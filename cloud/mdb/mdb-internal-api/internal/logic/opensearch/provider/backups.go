...
func backupMetaFromS3(ctx context.Context, client s3.Client, bucket, key string) (osmodels.BackupMeta, error) {
	r, err := client.GetObject(ctx, bucket, key)
	if err != nil {
		return osmodels.BackupMeta{}, err
	}
	defer r.Close()

	var backupMeta osmodels.BackupMeta
...
func (es *OpenSearch) listClusterBackupsImpl(ctx context.Context, reader clusterslogic.Reader, client s3.Client, cid string) ([]bmodels.Backup, error) {
	cluster, err := reader.ClusterByClusterID(ctx, cid, clusters.TypeOpenSearch, models.VisibilityVisibleOrDeleted)
	if err != nil {
		return nil, err
	}
	...
	metadata, err := backupMetaFromS3(ctx, client, bucket, osmodels.S3MetaKey)
...
func (es *OpenSearch) Backup(ctx context.Context, globalBackupID string) (bmodels.Backup, error) {
...
func (es *OpenSearch) FolderBackups(ctx context.Context, fid string, pageToken bmodels.BackupsPageToken, pageSize int64) ([]bmodels.Backup, bmodels.BackupsPageToken, error) {
	...
	err := es.operator.ReadOnFolder(ctx, fid,
		func(ctx context.Context, session sessions.Session, reader clusterslogic.Reader) error {
			var err2 error
			backups, nextPageToken, err2 = es.backups.BackupsByFolderID(ctx, session.FolderCoords.FolderID, clusters.TypeOpenSearch,
				func(ctx context.Context, client s3.Client, cid string) ([]bmodels.Backup, error) {
					return es.listClusterBackupsImpl(ctx, reader, client, cid)
				},
				pageToken, pageSize)
...
func (es *OpenSearch) ClusterBackups(ctx context.Context, cid string, pageToken bmodels.BackupsPageToken, pageSize int64) ([]bmodels.Backup, bmodels.BackupsPageToken, error) {
	var backups []bmodels.Backup
	var nextPageToken bmodels.BackupsPageToken
	pageSize = pagination.SanePageSize(pageSize)

	var err = es.operator.ReadOnCluster(ctx, cid, clusters.TypeOpenSearch,
