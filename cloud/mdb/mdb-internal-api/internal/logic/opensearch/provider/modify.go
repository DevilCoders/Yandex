...
func (es *OpenSearch) ModifyCluster(ctx context.Context, args opensearch.ModifyClusterArgs) (operations.Operation, error) {
	return es.operator.ModifyOnCluster(ctx, args.ClusterID, clusters.TypeOpenSearch,
		func(ctx context.Context, session sessions.Session, reader clusterslogic.Reader, modifier clusterslogic.Modifier, cluster clusterslogic.Cluster) (operations.Operation, error) {
			...
			upillar := ospillars.NewPillarUpdater(clusterPillar)
			...
			if err := upillar.UpdatePlugins(args.ConfigSpec.OpenSearch.Plugins); err != nil {
				return operations.Operation{}, err
			}
			...
			if args.ConfigSpec.Version.Valid && !args.ConfigSpec.Version.Value.Equal(clusterPillar.Data.OpenSearch.Version) {
				if resChanges {
			...
			if args.ConfigSpec.OpenSearch.DataNode.Valid {
				dataNode := args.ConfigSpec.OpenSearch.DataNode.Value
				err = dataNode.Validate(false)
				if err != nil {
					return operations.Operation{}, semerr.InvalidInputf("invalid data node config: %s", err)
				}
				dataPillar := &ospillars.DataNodeSubCluster{}
				subCluster, err := reader.SubClusterByRole(ctx, cluster.ClusterID, hosts.RoleOpenSearchDataNode, dataPillar)
				if err != nil {
					return operations.Operation{}, err
				}
...
func (es *OpenSearch) modifyResources(ctx context.Context, reader clusterslogic.Reader, modifier clusterslogic.Modifier, cluster clusters.Cluster, configSpec *osmodels.ConfigSpecUpdate, session sessions.Session) (bool, time.Duration, workerFlags, error) {
	var hasChanges bool
	var timeout time.Duration
	var flags workerFlags

	if configSpec.OpenSearch.DataNode.Valid {
		dataNode := configSpec.OpenSearch.DataNode.Value
		if dataNode.Resources.IsSet() {
			changes, extraTimeout, dataResChanges, err := es.modifyResourcesForRole(ctx, reader, modifier, cluster, hosts.RoleOpenSearchDataNode, dataNode.Resources, session)
			if err != nil {
	...
	if configSpec.OpenSearch.MasterNode.Valid {
		masterNode := configSpec.OpenSearch.MasterNode.Value
		changes, extraTimeout, masterResChanges, err := es.modifyResourcesForRole(ctx, reader, modifier, cluster, hosts.RoleOpenSearchMasterNode, masterNode.Resources, session)
		if err != nil {
			return false, timeout, workerFlags{}, err
		}
...
func (es *OpenSearch) modifyResourcesForRole(ctx context.Context, reader clusterslogic.Reader, modifier clusterslogic.Modifier, cluster clusters.Cluster, role hosts.Role, targetResources models.ClusterResourcesSpec, session sessions.Session) (bool, time.Duration, resourceChanges, error) {
	...
	hostGroup := clusterslogic.HostGroup{
		...
		SkipValidations: clusterslogic.SkipValidations{
			MaxHosts: role == hosts.RoleOpenSearchDataNode,
		},
	}
...
func getTaskCreationInfo() common.TaskCreationInfo {
	return common.TaskCreationInfo{
		ClusterUpgradeTask:     osmodels.TaskTypeClusterUpgrade,
		ClusterModifyTask:      osmodels.TaskTypeClusterModify,
		ClusterModifyOperation: osmodels.OperationTypeClusterModify,

		MetadataUpdateTask:      osmodels.TaskTypeMetadataUpdate,
		MetadataUpdateOperation: osmodels.OperationTypeMetadataUpdate,
