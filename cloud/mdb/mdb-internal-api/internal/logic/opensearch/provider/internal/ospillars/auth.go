...
func (a *Auth) authProviders() (*osmodels.AuthProviders, error) {
	var res = make([]*osmodels.AuthProvider, 0, osmodels.MaxAuthProviders)

	for _, realm := range a.Realms.Native {
		p := osmodels.NewAuthProvider(osmodels.AuthProviderTypeNative, realm.Name)
	...
	for _, r := range a.Realms.Saml {
		p := osmodels.NewAuthProvider(osmodels.AuthProviderTypeSaml, r.Name)
		...
		s.DashboardsURL = strings.TrimSuffix(r.Settings.SPLogout, "/logout")
...
func (a *Auth) fromProvider(kp KibanaProvider, p *osmodels.AuthProvider) {
...
func (a *Auth) addProvider(p *osmodels.AuthProvider) {
	...
	if p.Type == osmodels.AuthProviderTypeSaml {
...
func (a *Auth) addNativeProvider(p *osmodels.AuthProvider) {
	var kp KibanaProvider
...
func (a *Auth) setAuthProviders(ap *osmodels.AuthProviders) error {
	a.clear()
	...
	for _, p := range ap.Providers() {
		...
		case osmodels.AuthProviderTypeSaml:
			...
			r.Settings.SPAcs = strings.TrimSuffix(s.DashboardsURL, "/") + "/api/security/saml/callback"
			r.Settings.SPLogout = strings.TrimSuffix(s.DashboardsURL, "/") + "/logout"
