...
type ResourcePresetService struct {
	protov1.UnimplementedResourcePresetServiceServer
...
func NewResourcePresetService(es opensearch.OpenSearch, console console.Console, l log.Logger) *ResourcePresetService {
	return &ResourcePresetService{es: es, console: console, l: l}
}
...
func consoleResourcePresetsToGRPC(resourcePresets []consolemodels.ResourcePreset) *protov1.ResourcePreset {
	zones := getZonesFromPresets(resourcePresets)
	preset := resourcePresets[0]
	return &protov1.ResourcePreset{
		Id:      preset.ExtID,
...
func (rps *ResourcePresetService) Get(ctx context.Context, req *protov1.GetResourcePresetRequest) (*protov1.ResourcePreset, error) {
	resourcePresetsAll, err := rps.console.GetResourcePresetsByClusterType(ctx, clusters.TypeOpenSearch, "", false)
	if err != nil {
		return nil, err
	}
...
func (rps *ResourcePresetService) List(ctx context.Context, req *protov1.ListResourcePresetsRequest) (*protov1.ListResourcePresetsResponse, error) {
	consoleResourcePresets, err := rps.console.GetResourcePresetsByClusterType(ctx, clusters.TypeOpenSearch, "", false)
	if err != nil {
		return nil, err
	}
	...
	grpcResourcePresets := make([]*protov1.ResourcePreset, len(resourcePresetIDsToList))
	...
	res := protov1.ListResourcePresetsResponse{
