...
type ResourcePresetService struct {
	esv1.UnimplementedResourcePresetServiceServer
...
func NewResourcePresetService(es elasticsearch.ElasticSearch, console console.Console, l log.Logger) *ResourcePresetService {
...
func consoleResourcePresetsToGRPC(resourcePresets []consolemodels.ResourcePreset) *esv1.ResourcePreset {
	zones := getZonesFromPresets(resourcePresets)
	preset := resourcePresets[0]
	return &esv1.ResourcePreset{
		Id:      preset.ExtID,
...
func (rps *ResourcePresetService) Get(ctx context.Context, req *esv1.GetResourcePresetRequest) (*esv1.ResourcePreset, error) {
	resourcePresetsAll, err := rps.console.GetResourcePresetsByClusterType(ctx, clusters.TypeElasticSearch, "", false)
	if err != nil {
		return nil, err
	}
...
func (rps *ResourcePresetService) List(ctx context.Context, req *esv1.ListResourcePresetsRequest) (*esv1.ListResourcePresetsResponse, error) {
	consoleResourcePresets, err := rps.console.GetResourcePresetsByClusterType(ctx, clusters.TypeElasticSearch, "", false)
	if err != nil {
		return nil, err
	}
	...
	grpcResourcePresets := make([]*esv1.ResourcePreset, len(resourcePresetIDsToList))
	...
	res := esv1.ListResourcePresetsResponse{
