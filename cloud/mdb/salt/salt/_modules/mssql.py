...
def __virtual__():
    ...
    if HAS_ALL_IMPORTS:
        return True
    return (False, 'The mssql execution module cannot be loaded: the pyodbc python library is not available.')


def _get_connection(**kwargs):
    connection_args = {}
    if 'pwd' not in kwargs and 'connection_default_file' not in kwargs:
        raise KeyError('Pass "connection_default_file" or "pwd" argument to establish connection')
    if 'connection_default_file' in kwargs:
        config = ConfigParser.ConfigParser()
        config.read(kwargs['connection_default_file'])
        kwargs['pwd'] = config.get('Administrator', 'pwd')
        kwargs['uid'] = config.get('Administrator', 'uid')
        kwargs['driver'] = config.get('Administrator', 'driver')
        kwargs['server'] = config.get('Administrator', 'server')
    if 'database' not in kwargs and 'connection_default_file' in kwargs:
        kwargs['database'] = 'master'
    for arg in ('driver', 'server', 'uid', 'pwd', 'database'):
        if arg in kwargs:
            connection_args[arg] = kwargs[arg]
    return pyodbc.connect(**connection_args)


class _MssqlEncoder(salt.utils.json.json.JSONEncoder):
...
def db_exists(database_name, **kwargs):
    ...
    return (
        len(tsql_query("SELECT database_id FROM sys.databases WHERE NAME='{0}'".format(database_name), **kwargs)) == 1
    )
...
def user_list(**kwargs):
    ...
    return [
        row[0]
        for row in tsql_query("SELECT name FROM sysusers where issqluser=1 or isntuser=1", as_dict=False, **kwargs)
    ]
...
def user_create(username, login=None, domain='', database=None, roles=None, options=None, **kwargs):
    ...
    if user_exists(username, domain, database, **kwargs):
