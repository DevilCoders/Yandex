...
def __virtual__():
    ...
    if not HAS_CSV:
        return False
    if not salt.utils.path.which('psql'):
...
def _run_psql(cmd, runas=None, password=None, host=None, port=None, user=None):
    ...
    if password and not __salt__['file.remove'](pgpassfile):
        log.warning('Remove PGPASSFILE failed')

    return ret


def _run_initdb(name, auth='password', user=None, password=None, encoding='UTF8', locale=None, runas=None):
    '''
    ...
    if user is None:
        user = runas
    _INITDB_BIN = salt.utils.path.which('initdb')
    if not _INITDB_BIN:
    ...
    kwargs = dict(runas=runas, group=runas, clean_env=True)
...
def version(user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
    ...
    cmd = _psql_cmd(
        '-c', query, '-t', host=host, user=user, port=port, maintenance_db=maintenance_db, password=password
    )
    ret = _run_psql(cmd, runas=runas, group=runas, password=password, host=host, port=port, user=user)
...
def get_default_password_encryption():
    return _DEFAULT_PASSWORDS_ENCRYPTION
...
def _psql_cmd(*args, **kwargs):
    ...
    _PSQL_BIN = salt.utils.path.which('psql')
    dsn = []
    cmd = [
        _PSQL_BIN,
        '--no-align',
    ...
    if user:
        dsn.append('user={user}'.format(user=user))
    if host:
        dsn.append('host={host}'.format(host=host))
    if port:
        dsn.append('port={port}'.format(port=port))
    if not maintenance_db:
        maintenance_db = 'postgres'
    dsn.append('dbname={dbname}'.format(dbname=maintenance_db))
    dsn.append(_LOG_SKIP_OPTIONS)
    cmd.append(' '.join(dsn))
    cmd.extend(args)
    return cmd


def _psql_prepare_and_run(cmd, host=None, port=None, maintenance_db=None, password=None, runas=None, user=None):
    rcmd = _psql_cmd(host=host, user=user, port=port, maintenance_db=maintenance_db, *cmd)
    cmdret = _run_psql(rcmd, runas=runas, password=password, host=host, port=port, user=user)
    return cmdret


def psql_query(query, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None, write=False):
    '''
    ...
    for row in csv.reader(
        csv_file, delimiter=salt.utils.stringutils.to_str(','), quotechar=salt.utils.stringutils.to_str('"')
    ):
        if not row:
            continue
        if not header:
...
def psql_exec(query, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
    Run an SQL-Command.
    ...
    return cmdret['retcode'], cmdret['stdout'], cmdret['stderr']


def psql_file(path, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
    Run SQL-file.
    ...
    .. code-block:: bash
        ...
        salt '*' postgres.psql_file '/my/path.sql'
    '''
    cmdret = _psql_prepare_and_run(
        ['-1', '-f', path],
        runas=runas,
...
def psql_compose(cmd, **kwargs):
    '''
    Wrapper for _psql_cmd
    '''
    psql = ' '.join([pipes.quote(x) for x in _psql_cmd(**kwargs)])
    return '{psql} {cmd}'.format(psql=psql, cmd=cmd)
...
def db_list(user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    ...
    rows = psql_query(
        query, runas=runas, host=host, user=user, port=port, maintenance_db=maintenance_db, password=password
    )
...
def _quote_ddl_value(value, quote="'"):
    ...
    if quote in value:  # detect trivial sqli
        raise SaltInvocationError('Unsupported character {0} in value: {1}'.format(quote, value))
...
def db_create(
    ...
    ret = _psql_prepare_and_run(
        ['-c', query], user=user, host=host, port=port, maintenance_db=maintenance_db, password=password, runas=runas
    )
...
def db_alter(
    ...
    if owner and owner_recurse:
        ...
        if tablespace:
            queries.append('ALTER DATABASE "{0}" SET TABLESPACE "{1}"'.format(name, tablespace))
...
def db_remove(name, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
    ...
    ret = _psql_prepare_and_run(
        ['-c', query], user=user, host=host, port=port, runas=runas, maintenance_db=maintenance_db, password=password
    )
    return ret['retcode'] == 0
...
def tablespace_exists(name, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    ...
    tablespaces = tablespace_list(
        user=user, host=host, port=port, maintenance_db=maintenance_db, password=password, runas=runas
    )
...
def tablespace_create(
    ...
    query = 'CREATE TABLESPACE "{0}" {1} LOCATION \'{2}\' {3}'.format(name, owner_query, location, options_query)
...
def tablespace_alter(
    ...
    if set_option:
        queries.append(
            'ALTER TABLESPACE "{0}" SET ({1} = {2})'.format(name, set_option.keys()[0], set_option.values()[0])
        )
    if reset_option:
...
def tablespace_remove(name, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
...
def user_list(user=None, host=None, port=None, maintenance_db=None, password=None, runas=None, return_password=False):
    '''
    ...
    for row in rows:
        ...
        for date_key in ('expiry time',):
            try:
                retrow[date_key] = datetime.datetime.strptime(row[date_key], '%Y-%m-%d %H:%M:%S')
            except ValueError:
...
def role_get(
    name, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None, return_password=False
):
    '''
    Return a dict with information about users of a Postgres server.
...
def user_exists(name, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
...
def is_password_already_encrypted(password):
    return password and six.text_type(password).startswith('md5')
...
def _maybe_encrypt_password(role, password, encrypted=_DEFAULT_PASSWORDS_ENCRYPTION):
    ...
    if encrypted and password and not password.startswith('md5'):
        password = "md5{0}".format(
            hashlib.md5(salt.utils.stringutils.to_bytes('{0}{1}'.format(password, role), encoding='UTF-8')).hexdigest()
        )
    return password


def maybe_encrypt_password(role, password, encrypted=_DEFAULT_PASSWORDS_ENCRYPTION):
    '''
    Public version of _maybe_encrypt_password
    '''
    return _maybe_encrypt_password(role, password, encrypted)
...
def _role_cmd_args(
    ...
    if isinstance(rolepassword, six.string_types) and bool(rolepassword):
        escaped_password = '\'{0}\''.format(
            _maybe_encrypt_password(name, rolepassword.replace('\'', '\'\''), encrypted=encrypted)
        )
    ...
    flags = (
        ...
        {'flag': 'PASSWORD', 'test': bool(rolepassword), 'skip': skip_passwd, 'addtxt': escaped_password},
...
def _role_create(
    ...
    ret = _psql_prepare_and_run(
        ['-c', sub_cmd], runas=runas, host=host, user=user, port=port, maintenance_db=maintenance_db, password=password
    )
...
def _role_remove(name, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
...
def available_extensions(user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
    ...
    ret = psql_query(
        query, user=user, host=host, port=port, maintenance_db=maintenance_db, password=password, runas=runas
    )
    exts = {}
    for row in ret:
...
def installed_extensions(user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
    ...
    '''
    exts = []
    query = 'select a.*, b.nspname as schema_name ' 'from pg_extension a,  pg_namespace b where a.extnamespace = b.oid;'
    ret = psql_query(
        query, user=user, host=host, port=port, maintenance_db=maintenance_db, password=password, runas=runas
    )
    exts = {}
...
def get_installed_extension(name, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
...
def is_available_extension(name, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
...
def create_metadata(
    name, ext_version=None, schema=None, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None
):
    '''
...
def drop_extension(
    ...
    if is_installed_extension(
        ...
        _psql_prepare_and_run(
            ['-c', cmd], runas=runas, host=host, user=user, port=port, maintenance_db=maintenance_db, password=password
        )
    ...
    if installable:
        if not installed:
            ...
            if schema and _EXTENSION_TO_MOVE in mtdata:
                args.append('ALTER EXTENSION "{0}" SET SCHEMA "{1}";'.format(name, schema))
            if ext_version and _EXTENSION_TO_UPGRADE in mtdata:
                args.append('ALTER EXTENSION "{0}" UPDATE TO {1};'.format(name, ext_version))
            cmd = ' '.join(args).strip()
        if cmd:
            _psql_prepare_and_run(
                ['-c', cmd],
...
def user_remove(username, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    '''
    ...
    '''
    return _role_remove(
        username, user=user, host=host, port=port, maintenance_db=maintenance_db, password=password, runas=runas
    )
...
def group_remove(groupname, user=None, host=None, port=None, maintenance_db=None, password=None, runas=None):
    ...
    '''
    return _role_remove(
        groupname, user=user, host=host, port=port, maintenance_db=maintenance_db, password=password, runas=runas
    )
...
def owner_to(dbname, ownername, user=None, host=None, port=None, password=None, runas=None):
    ...
    cmdret = _psql_prepare_and_run(
        ['-f', sqlfile.name], user=user, runas=runas, host=host, port=port, password=password, maintenance_db=dbname
    )
    return cmdret
...
def schema_create(dbname, name, owner=None, user=None, db_user=None, db_password=None, db_host=None, db_port=None):
    ...
    if schema_exists(
        dbname, name, user=user, db_user=db_user, db_password=db_password, db_host=db_host, db_port=db_port
    ):
...
def schema_remove(dbname, name, user=None, db_user=None, db_password=None, db_host=None, db_port=None):
    '''
    ...
    if not schema_exists(
        dbname, name, user=None, db_user=db_user, db_password=db_password, db_host=db_host, db_port=db_port
    ):
    ...
    if not schema_exists(
        dbname, name, user, db_user=db_user, db_password=db_password, db_host=db_host, db_port=db_port
    ):
        return True
    else:
...
def schema_exists(dbname, name, user=None, db_user=None, db_password=None, db_host=None, db_port=None):
    '''
    ...
    '''
    return bool(
        schema_get(dbname, name, user=user, db_user=db_user, db_host=db_host, db_port=db_port, db_password=db_password)
    )


def schema_get(dbname, name, user=None, db_user=None, db_password=None, db_host=None, db_port=None):
    '''
    ...
    all_schemas = schema_list(
        dbname, user=user, db_user=db_user, db_host=db_host, db_port=db_port, db_password=db_password
    )
    try:
...
def schema_list(dbname, user=None, db_user=None, db_password=None, db_host=None, db_port=None):
    ...
    rows = psql_query(
        query, runas=user, host=db_host, user=db_user, port=db_port, maintenance_db=dbname, password=db_password
    )
...
def language_exists(name, maintenance_db, user=None, host=None, port=None, password=None, runas=None):
    '''
    .. versionadded:: 2016.3.0
    ...
    languages = language_list(maintenance_db, user=user, host=host, port=port, password=password, runas=runas)

    return name in languages


def language_create(name, maintenance_db, user=None, host=None, port=None, password=None, runas=None):
    '''
    .. versionadded:: 2016.3.0
...
def language_remove(name, maintenance_db, user=None, host=None, port=None, password=None, runas=None):
    '''
    .. versionadded:: 2016.3.0
...
def _get_object_owner(
    name, object_type, prepend='public', maintenance_db=None, user=None, host=None, port=None, password=None, runas=None
):
    '''
    ...
    if object_type == 'table':
        query = (
            ' '.join(
                ['SELECT tableowner AS name', 'FROM pg_tables', "WHERE schemaname = '{0}'", "AND tablename = '{1}'"]
            )
        ).format(prepend, name)
    elif object_type == 'sequence':
        query = (
...
def _validate_privileges(object_type, privs, privileges):
    ...
    if object_type != 'group':
        ...
        if object_type not in _PRIVILEGES_OBJECTS:
            raise SaltInvocationError('Invalid object_type: {0} provided'.format(object_type))

        if not set(privs).issubset(set(_perms)):
            raise SaltInvocationError(
                'Invalid privilege(s): {0} provided for object {1}'.format(privileges, object_type)
            )
    else:
        if privileges:
            raise SaltInvocationError('The privileges option should not ' 'be set for object_type group')
...
def privileges_grant(
    ...
    if has_privileges(
        ...
        log.info('The object: %s of type: %s already has privileges: %s set', object_name, object_type, privileges)
        return False
    ...
    if grant_option:
        if object_type == 'group':
            ...
            query = 'GRANT {0} ON ALL {1}S IN SCHEMA {2} TO ' '"{3}" WITH GRANT OPTION'.format(
        ...
        if object_type == 'group':
            ...
            query = 'GRANT {0} ON ALL {1}S IN SCHEMA {2} TO "{3}"'.format(_grants, object_type.upper(), prepend, name)
        else:
            query = 'GRANT {0} ON {1} {2} TO "{3}"'.format(_grants, object_type.upper(), on_part, name)
...
def privileges_revoke(
    ...
    if not has_privileges(
        ...
        log.info('The object: %s of type: %s does not' ' have privileges: %s set', object_name, object_type, privileges)
        return False
    ...
    if object_type == 'group':
        ...
        query = 'REVOKE {0} ON {1} {2} FROM {3}'.format(_grants, object_type.upper(), on_part, name)
...
def datadir_init(name, auth='password', user=None, password=None, encoding='UTF8', locale=None, runas=None):
    '''
    .. versionadded:: 2016.3.0
...
def find_master(hosts, user=None, port=None, password=None, timeout=60):
    """
    ...
    while time.time() < deadline:
        for host in hosts:
            ret = psql_query(
                # MDB-4668: basebackup use repl user for replica setup
                query="SELECT pg_is_in_recovery() FROM pg_roles WHERE rolname='repl'",
                host=host,
                user=user,
                port=port,
                password=password,
            ...
            if ret and ret[0]['pg_is_in_recovery'] == 'f':
                return host
        time.sleep(1)
