...
DATA_NODE_ROLE = 'elasticsearch_cluster.datanode'
MASTER_NODE_ROLE = 'elasticsearch_cluster.masternode'
...
def cluster_name():
    ...
    Return cluster name.
    """
    return pillar('data:elasticsearch:cluster_name', None) or pillar('data:dbaas:cluster_id')
...
def is_master_node():
    ...
    """
    return pillar('data:elasticsearch:is_master', False)
...
def is_data_node():
    ...
    """
    return pillar('data:elasticsearch:is_data', False)
...
def licensed_for(license):
    ...
    try:
        return LICENSE_LEVEL[pillar('data:elasticsearch:edition', 'oss')] >= LICENSE_LEVEL[license]
    except KeyError:
        return False
...
def edition():
    ...
    return pillar('data:elasticsearch:edition', 'oss')
...
def version():
    ...
    """
    return pillar('data:elasticsearch:version')
...
def render_htpasswd():
    ...
    user_list = pillar('data:elasticsearch:users')
    strings = []
...
def jvm_mem_share():
    if is_master_node():
        return pillar('data:elasticsearch:config:master_node:jvm_mem_share', 60)
    # if is_ml_node():
    #     return pillar('data:elasticsearch:config:ml_node:jvm_mem_share', 25)
    return pillar('data:elasticsearch:config:data_node:jvm_mem_share', 50)


def jvm_mem_max():
    if is_master_node():
        return pillar('data:elasticsearch:config:master_node:jvm_mem_max', 31 * _GIGABYTES)
    # if is_ml_node():
    #     return pillar('data:elasticsearch:config:ml_node:jvm_mem_max', 2 * _GIGABYTES)
    return pillar('data:elasticsearch:config:data_node:jvm_mem_max', 31 * _GIGABYTES)
...
def data_node_settings():
    ...
    settings = pillar('data:elasticsearch:config:data_node', {})
...
def pillar_users():
    return pillar('data:elasticsearch:users', {})
...
def install_plugin(plugin):
    plugin_path = "file:///opt/yandex/mdb-elasticsearch-plugins/{0}.zip".format(plugin)
    cmd = ['/usr/share/elasticsearch/bin/elasticsearch-plugin', 'install', '-b', plugin_path]
    return _run_cmd(cmd)


def remove_plugin(plugin):
    cmd = ['/usr/share/elasticsearch/bin/elasticsearch-plugin', 'remove', plugin]
    return _run_cmd(cmd)
...
def pillar_plugins():
    return set(pillar('data:elasticsearch:plugins', [])) | set(['repository-s3'])
...
def s3client_settings():
    ...
    if not has_repository_s3():
        return result
    s3clients = pillar('data:elasticsearch:s3:client', {})
...
def keystore_settings():
    settings = pillar('data:elasticsearch:keystore', {})
...
def keystore_add(key, value):
    cmd = ['/usr/share/elasticsearch/bin/elasticsearch-keystore', 'add', '--stdin', '--force', key]
    _run_cmd(cmd, stdin=value)  # no output


def keystore_remove(key):
    cmd = ['/usr/share/elasticsearch/bin/elasticsearch-keystore', 'remove', key]
...
def auth_chain_kibana():
    providers = pillar('data:elasticsearch:auth:providers', [])
    result = {}
    has_native = False
    max_order = 0
    for p in providers:
...
def auth_chain_elastic():
    realms = pillar('data:elasticsearch:auth:realms', {})
...
def _request(api, method='GET', retry=3, **kwargs):
    ...
    r = http.request(
        ...
        'https://' + fqdn() + ':9200/' + api,
        auth=(
            pillar('data:elasticsearch:users:mdb_admin:name'),
            pillar('data:elasticsearch:users:mdb_admin:password'),
        ),
        **kwargs
    )
    r.raise_for_status()
...
def auto_backups_enabled():
    return pillar('data:elasticsearch:auto_backups:enabled', False)
...
def active_extension_ids():
    extensions = pillar('data:elasticsearch:extensions', [])
...
def validate_extension(extname, archive):
    ...
    total_size_limit = pillar('data:elasticsearch:extension_size_limit', 1 * _GIGABYTES)
    files_limit = pillar('data:elasticsearch:extension_files_limit', 100)
    total = 0
