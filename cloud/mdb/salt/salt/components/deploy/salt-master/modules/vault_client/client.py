...
class VaultClient(object):
    ...
    def validate_response(self, response):
        ...
        if data.get('status') == 'warning':
            ...
            logger.warn('Warning: %s' % data.get('warning_message', default_message))
        return data
    ...
    def _build_rsa_auth(self, rsa_auth=False, rsa_login=None):
        ...
        if isinstance(rsa_auth, BaseRSAAuth):
            ...
            if not rsa_login:
                logger.warn('WARNING! RSA private key passed without explicit passing rsa_login. '
                            'Your system login `%s` will be used for request signing' % getpass.getuser())  # pragma: no cover
    ...
    def list_tokens(self, secret_uuid, with_revoked=False, return_raw=False):
        ...
        request_data = noneless_dict({'with_revoked': with_revoked})
    ...
    def send_tokenized_requests(self, tokenized_requests, return_raw=False):
    ...
    def send_tokenized_revoke_requests(self, tokenized_requests, return_raw=False):
