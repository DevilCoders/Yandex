...
def main():
    ...
    for port in args.PAIRS:
        ...
        if res.get(port) and res[port][0] != res[args.PAIRS[port]][0] and \
                get_file_mtime(args.PAIRS[port]) + args.mtime_threshold < time.time():
...
def get_file_mtime(filename):
    cmd = ['sudo', '/usr/bin/stat', '-c', '%Y', filename]
    stdout = subprocess.check_output(cmd, shell=False)
    return int(stdout)
...
def get_args():
    ...
    parser.add_argument(
        ...
        help='MongoDB certificate port:file')
    parser.add_argument(
        '-w', '--warn', type=int, default=60, help='Warning limit (days)')
    parser.add_argument(
        '-c', '--crit', type=int, default=30, help='Critical limit (days)')
    parser.add_argument(
        '-t', '--mtime-threshold', type=int, default=14400,
        help='Do not emit file vs port diff error if file is modified less than N seconds ago')
