...
/etc/dbaas-cron/modules/mdbdns_clickhouse.py:
    file.managed:
        - source: salt://components/dbaas-cron/conf/mdbdns_clickhouse.py
        - mode: '0640'
        - user: root
        - group: monitor
        - require:
...
/etc/dbaas-cron/conf.d/mdbdns_upper.conf:
    file.absent:
        - require_in:
            - file: /etc/dbaas-cron/conf.d/mdbdns_clickhouse.conf

/etc/dbaas-cron/conf.d/mdbdns_clickhouse.conf:
    file.managed:
        - template: jinja
        ...
        - context:
            ...
            args:
                ...
                params:
                    ...
                    shard_id: "{{ salt.pillar.get('data:dbaas:shard_id') }}"
                    shard_name: "{{ salt.pillar.get('data:dbaas:shard_name') }}"
                    ...
                    has_zookeeper: {{ salt.mdb_clickhouse.has_zookeeper() }}
                    ch_version: "{{ salt.mdb_clickhouse.version() }}"
        - require:
            - pkg: dbaas-cron
            - file: {{ log_dir }}
