...
raw_bloat AS (
    ...
    sub.index_name,
        bs*(sub.relpages)::bigint AS usedbytes,
        pg_relation_size(sub.index_oid) AS totalbytes,
        CASE
            WHEN sub.relpages <= otta THEN 0
            ELSE pg_relation_size(sub.index_oid) - (bs*otta)::bigint END
            AS wastedbytes,
        CASE
            WHEN sub.relpages <= otta
            THEN 0
        ELSE 100 * (1 - bs*otta / pg_relation_size(sub.index_oid))
        END
        AS realbloat,
...
SELECT
    ...
    r.table_name,
    r.index_name,
        round(realbloat::numeric, 1) AS bloat_pct,
        wastedbytes AS bloat_bytes,
    pg_size_pretty(wastedbytes::bigint) AS bloat_size,
        usedbytes AS used_index_bytes,
    pg_size_pretty(usedbytes::bigint) AS used_index_size,
