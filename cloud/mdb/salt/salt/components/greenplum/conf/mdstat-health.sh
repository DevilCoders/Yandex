...
while IFS= read -d $'\0' -r MD_SYS_FOLDER ; do

    eval $(grep DEVNAME= ${MD_SYS_FOLDER}/uevent)

    MD_DEV=${DEVNAME}
    [ -z "${MD_DEV}" ] && continue

    MISMATCH_CNT=$(cat ${MD_SYS_FOLDER}/md/mismatch_cnt)
    STATE=$(cat ${MD_SYS_FOLDER}/md/array_state)
    DEGRADED_DISKS=$(cat ${MD_SYS_FOLDER}/md/degraded)
    TOTAL_DISKS=$(cat ${MD_SYS_FOLDER}/md/raid_disks)
    ACTIVE_DISKS=$(expr $TOTAL_DISKS - $DEGRADED_DISKS)

    echo "mdstat,host=${HOST},dev=${MD_DEV} mismatch_cnt=${MISMATCH_CNT}i,array_state=\"${STATE}\",active_disks=${ACTIVE_DISKS}i,degraded_disks=${DEGRADED_DISKS}i,total_disks=${TOTAL_DISKS}i"
done < <(find /sys/block -mindepth 1 -maxdepth 1 -name "md*" -print0)
