...
USAGE="Usage: $(basename $0) [-c <conf-dir>] [-d <dbname>] [-t <timeout-minutes>] [-z <zk-lockfile>] [-w <zk-lock-wait-seconds>] [-s <sleep-seconds>] {backup_create [--permanent] [<backup_id>]| backup_delete <backup_name> |purge <before>|backup_restore <backup_name>}"
...
default_walg='wal-g-redis'
cmd=${default_walg}
if [ -n "${dbname}" ]; then
    cmd="wal-g-{$dbname}"
fi
default_cmd="${cmd} --config ${default_confdir}/${default_config}"
export default_cmd
...
create_backup () {
    ...
    if [ -n "${backup_id}" ]; then
      if ${default_cmd} backup-list --detail | grep "${backup_id}"; then
...
delete_backup () {
    ...
    if ! (${default_cmd} backup-list --detail | grep -q "${backup_name}") ; then
...
action=$1
case "$action" in
    backup_create | backup-create | backup-push)
        permanent=""
        backup_id=""
    ...
    purge)
        ...
        cmd="${default_cmd} delete --confirm  --retain-count ${days} --retain-after ${before}"
        shift 2;;
