{% set dbname = 'deploydb' %}
{% set target = salt['pillar.get']('data:deploydb:target', 'latest') %}
{% set path = salt['pillar.get']('data:dbfiles_path', '/usr/local/yandex/') %}

include:
    - components.monrun2.deploydb

{{ path + dbname }}:
    file.recurse:
        - source: salt://components/pg-code/deploydb
        - user: postgres
        - dir_mode: 755
        - file_mode: 744
        - makedirs: True
...
change_db_owner_{{ dbname }}:
    cmd.run:
        - name: /opt/yandex/pg_change_owner/bin/pg_change_owner --user deploydb_admin --database deploydb -s deploy -s code -c $DSN
        - env:
            - DSN: "postgresql://localhost/deploydb?sslrootcert=/etc/postgresql/ssl/allCAs.pem&target_session_attrs=read-write"
        - runas: postgres
        - group: postgres
        - onchanges:
            - postgres_database: create_db_{{ dbname }}
...
{% endif %}
        - callbacks:
            afterAll:
                - {{ path + dbname }}/code/defs
                - {{ path + dbname }}/code/private
                - {{ path + dbname }}/code/api
                - {{ path + dbname }}/grants
        - require:
            - postgres_database: create_db_{{ dbname }}
            - pkg: pgmigrate-pkg
