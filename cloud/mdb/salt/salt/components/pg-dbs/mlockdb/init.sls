{% set dbname = 'mlockdb' %}
{% set target = salt['pillar.get']('data:mlockdb:target', 'latest') %}
{% set path = salt['pillar.get']('data:dbfiles_path', '/usr/local/yandex/') %}

{{ path + dbname }}:
    file.recurse:
        - source: salt://components/pg-code/mlockdb
        - user: postgres
        - dir_mode: 755
        - file_mode: 744
        - makedirs: True
...
change_db_owner_{{ dbname }}:
    cmd.run:
        - name: /opt/yandex/pg_change_owner/bin/pg_change_owner --user mlockdb_admin --database mlockdb -s mlock -s code -c $DSN
        - env:
            - DSN: "postgresql://localhost/mlockdb?sslrootcert=/etc/postgresql/ssl/allCAs.pem"
        - runas: postgres
        - group: postgres
        - onchanges:
            - postgres_database: create_db_{{ dbname }}
