...
HACKED for Python 2.7

TODO: replace it with mdb_sqlserver states
...
def _normalize_options(options):
    if type(options) in [dict, collections.OrderedDict, salt.utils.odict.OrderedDict]:
        return ['{0}={1}'.format(k, v) for k, v in options.items()]
    if type(options) is list and (not len(options) or isinstance(options[0], six.string_types)):
        return options
    # Invalid options
