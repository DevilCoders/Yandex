...
def ensure_plugins(name):
    ...
    installed_plugins, update_plugins = __salt__['mdb_opensearch.installed_and_update_plugins']()
    expected_plugins = __salt__['mdb_opensearch.pillar_plugins']()
    ...
    for plugin in to_remove:
        __salt__["mdb_opensearch.remove_plugin"](plugin)
        ret["changes"]["plugins_removed"].append(plugin)

    for plugin in to_install:
        __salt__["mdb_opensearch.install_plugin"](plugin)
...
def ensure_keystore(name, settings):
    ...
    installed = __salt__['mdb_opensearch.keystore_keys']()
    ...
    for key in to_remove:
        __salt__["mdb_opensearch.keystore_remove"](key)
        ret["changes"]["keystore_removed"].append(key)

    for key in to_install:
        __salt__["mdb_opensearch.keystore_add"](key, settings[key])
...
def ensure_license(name, edition):
    ...
    installed = __salt__['mdb_opensearch.get_license']()['license']
    ...
    if not data:  # remove non basic license
        ...
        try:
            __salt__['mdb_opensearch.delete_license']()
    ...
    try:
        __salt__['mdb_opensearch.update_license'](data)
...
def ensure_repository(name, reponame, settings):
    ...
    """
    ret = {'name': name, 'changes': {}, 'result': False, 'comment': ''}
    if not __salt__['mdb_opensearch.auto_backups_enabled']():
        ret['result'] = True
        ret['comment'] = 'Repositories check disabled'
        return ret

    existing, ok = __salt__['mdb_opensearch.get_repository'](reponame)
    ...
    try:
        __salt__['mdb_opensearch.update_repository'](reponame, settings)
...
def ensure_snapshot_policy(name, policy_name, settings):
    ...
    existing, ok = __salt__['mdb_opensearch.get_snapshot_policy'](policy_name)
    ...
    try:
        __salt__['mdb_opensearch.update_snapshot_policy'](policy_name, settings)
