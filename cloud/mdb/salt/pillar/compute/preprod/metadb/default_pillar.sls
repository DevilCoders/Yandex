data:
  dbaas_metadb:
    default_pillar:
      - id: 1
        value:
          cert.ca: |
            -----BEGIN CERTIFICATE-----
            
            -----END CERTIFICATE-----
            -----BEGIN CERTIFICATE-----
            
            -----END CERTIFICATE-----
          data:
            access:
              user_hosts:
              - fqdn: s3-private.mds.yandex.net
                port: 443
              management_hosts:
              - fqdn: s3-private.mds.yandex.net
                port: 443
              data_lens_nets:
                - '2a02:6b8:c02:901:0:fc1f:1c8c:0/112'
                - '2a02:6b8:c0e:501:0:fc1f:1c8c:0/112'
                - '2a02:6b8:c03:501:0:fc1f:1c8c:0/112'
              web_sql_nets: []
              data_transfer_nets:
                - '2a02:6b8:c02:901:0:fc11:55ce:0/112'
                - '2a02:6b8:c02:901:0:fc11:2:0/112'
                - '2a02:6b8:c02:901:0:fc11::/112'
                - '2a02:6b8:c02:901:0:fc11:1:0/112'
                - '2a02:6b8:c0e:501:0:fc11:1:0/112'
                - '2a02:6b8:c0e:501:0:fc11:55ce:0/112'
                - '2a02:6b8:c0e:501:0:fc11::/112'
                - '2a02:6b8:c0e:501:0:fc11:2:0/112'
                - '2a02:6b8:c03:501:0:fc11:1:0/112'
                - '2a02:6b8:c03:501:0:fc11:2:0/112'
                - '2a02:6b8:c03:501:0:fc11:55ce:0/112'
                - '2a02:6b8:c03:501:0:fc11::/112'
            dist:
              bionic:
                secure: True
              pgdg:
                absent: True
            allow_salt_version_update: False
            external_project_ids:
              - '0x453e'
              - '0xf807'
            linux_kernel:
              version: '4.19.143-37'
            token_service:
              address: ts.private-api.cloud-preprod.yandex.net:4282
            solomon:
              agent: https://solomon.cloud.yandex-team.ru/push/json
              push_url: https://solomon.cloud.yandex-team.ru/api/v2/push
              project: yandexcloud
              cluster: 'mdb_{ctype}'
              service: yandexcloud_dbaas
              ca_path: /opt/yandex/allCAs.pem
              # solomon-prod token
              sa_id: {{ salt.yav.get('ver-01ep4q27cj2a6dx9xm03wf1dgc[service_account_id]') }}
              sa_private_key: {{ salt.yav.get('ver-01ep4q27cj2a6dx9xm03wf1dgc[private_key]') |  yaml_dquote }}
              sa_key_id: {{ salt.yav.get('ver-01ep4q27cj2a6dx9xm03wf1dgc[key_id]') }}
            solomon_cloud:
              push_url: https://solomon.cloud-preprod.yandex-team.ru/api/v2/push
              token: {{ salt.yav.get('ver-01e9rg41wq0c2kqg2y40en6v41[token]') }}
              ca_path: /opt/yandex/allCAs.pem
              sa_id: {{ salt.yav.get('ver-01enr0vfcwj3shtzrgdg0nx8he[service_account_id]') }}
              sa_private_key: {{ salt.yav.get('ver-01enr0vfcwj3shtzrgdg0nx8he[private_key]') | yaml_dquote }}
              sa_key_id: {{ salt.yav.get('ver-01enr0vfcwj3shtzrgdg0nx8he[key_id]') }}
            mdb_health:
              host: mdb-health.private-api.cloud-preprod.yandex.net
              ca_path: /opt/yandex/allCAs.pem
            mdb_int_api:
              address: mdb-internal-api.private-api.cloud-preprod.yandex.net
            certs:
              readonly: True
            use_mdbsecrets: true
            monrun2: true
            system:
              journald:
                system_max_use: '50M'
            use_yasmagent: false
            ship_logs: true
            s3:
              access_key_id:
                data: {{ salt.yav.get('ver-01dws7cactgphpm6sjvvrsv7ke[id]') }}
                encryption_version: 1
              access_secret_key:
                data: {{ salt.yav.get('ver-01dws7cactgphpm6sjvvrsv7ke[secret]') }}
                encryption_version: 1
              endpoint: https://s3-private.mds.yandex.net
              path_addressing_style: false
              virtual_addressing_style: true
            object_storage:
              endpoint: https://storage.cloud-preprod.yandex.net
            mdb_dns:
              host: mdb-dns.private-api.cloud-preprod.yandex.net
            billing:
              tvm_secret:
                data: {{ salt.yav.get('ver-01dws7gs48s10x7t1zq6ebkwxh[token]') }}
                encryption_version: 1
              topic: billing-mdb-instance
              ident: yc-mdb-pre
              tvm_client_id: 2002006
              tvm_server_id: 2001059
              use_cloud_logbroker: True
              yc_logbroker:
                topic: '/yc.billing.service-cloud/billing-mdb-instance'
                iam:
                  service_account:
                    id: yc.mdb.billing
                    key_id: {{ salt.yav.get('ver-01ex2br7bfpd72fygamnnvc84w[key_id]') }}
                    private_key: {{ salt.yav.get('ver-01ex2br7bfpd72fygamnnvc84w[private_key]') | yaml_encode }}
                    public_key: {{ salt.yav.get('ver-01ex2br7bfpd72fygamnnvc84w[public_key]') | yaml_encode }}
            billing2:
              logbroker:
                endpoint: 'lb.cc8035oc71oh9um52mv3.ydb.mdb.cloud-preprod.yandex.net'
                database: '/pre-prod_global/aoeb66ftj1tbt1b2eimn/cc8035oc71oh9um52mv3'
                topic: '/yc.billing.service-cloud/billing-mdb-mssql'
                iam:
                  token_service:
                    endpoint: 'ts.private-api.cloud-preprod.yandex.net:4282'
                  service_account:
                    id: yc.mdb.billing
                    key_id: {{ salt.yav.get('ver-01ex2br7bfpd72fygamnnvc84w[key_id]') }}
                    private_key: |
                      {{ salt.yav.get('ver-01ex2br7bfpd72fygamnnvc84w[private_key]')|indent(22) }}
            logship:
              use_rt: false
              use_cloud_logbroker: true
              topics:
                security_log: aoe9shbqc2v314v7fp3d/dataplane/security-logs
              logbroker_port: 2135
              logbroker_host: lb.cc8035oc71oh9um52mv3.ydb.mdb.cloud-preprod.yandex.net
              database: /pre-prod_global/aoeb66ftj1tbt1b2eimn/cc8035oc71oh9um52mv3
              iam_endpoint: iam.api.cloud-preprod.yandex.net
              lb_producer_key: {{ salt.yav.get('ver-01ewg0wgrbers3c7gz4vmf307w[lb_producer_key]') | tojson }}
            sysctl:
              vm.nr_hugepages: 0
          mine_functions:
            grains.item: [id, role, virtual]
