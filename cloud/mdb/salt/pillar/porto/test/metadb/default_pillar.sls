data:
  dbaas_metadb:
    default_pillar:
      - id: 1
        value:
          cert.ca: |
            -----BEGIN CERTIFICATE-----
            
            -----END CERTIFICATE-----
            -----BEGIN CERTIFICATE-----
            
            -----END CERTIFICATE-----
          data:
            access:
              user_hosts:
              - fqdn: s3-private.mds.yandex.net
                port: 443
              management_hosts:
              - fqdn: s3-private.mds.yandex.net
                port: 443
            dist:
              bionic:
                secure: True
              pgdg:
                absent: True
            allow_salt_version_update: False
            use_mdbsecrets: True
            cauth_use_v2: True
            certs:
              readonly: True
            l3host: True
            selfdns-api:
              token: {{ salt.yav.get('ver-01dz8psx0knrbadd4p97gd2dq1[token]') }}
              plugins-enabled:
              - mdb-porto
            solomon:
              agent: https://solomon.yandex-team.ru/push/json
              push_url: https://solomon.yandex-team.ru/api/v2/push
              project: internal-mdb
              cluster: 'mdb_{ctype}'
              service: mdb
              ca_path: False
              oauth_token: {{ salt.yav.get('ver-01dz8pwb5kcen02rgtbbme6w49[token]') }}
            mdb_health:
              host: mdb-health-test.db.yandex.net
              ca_path: /opt/yandex/allCAs.pem
            mdb_int_api:
              address: mdb-internal-api-test.db.yandex.net
            monrun2: true
            system:
              journald:
                system_max_use: '50M'
            ship_logs: true
            config:
              log_rotate_by_size: true
            s3:
              access_key_id:
                data: {{ salt.yav.get('ver-01fvtrs66pp25nvc2stnys7vf6[encrypted_id]') }}
                encryption_version: 1
              access_secret_key:
                data: {{ salt.yav.get('ver-01fvtrs66pp25nvc2stnys7vf6[encrypted_secret]') }}
                encryption_version: 1
              endpoint: https+path://s3.mds.yandex.net
              proxy_resolver:
                uri: http://s3.mds.yandex.net/hostname
                proxy_port: 4080
            mdb_dns:
              host: mdb-dns-test.db.yandex.net
            billing:
              tvm_secret:
                data: {{ salt.yav.get('ver-01e6grfyh1hgxvnesa853wttzy[token]') }}
                encryption_version: 1
              logbroker_host: lbkx.logbroker.yandex.net.
              topic: /yc/yandex/billing-mdb-porto-test-instance
              tvm_client_id: 2018095
              tvm_server_id: 2001059
            logship:
              use_rt: false
              topics:
                security_log: mdb/porto/test/security-log
              tvm:
                client_id: 2018950
                server_id: 2001059
                secret:
                  data: {{ salt.yav.get('ver-01e61msdsvcyxs9tpfyj7necj2[tvm_secret]') }}
                  encryption_version: 1
          mine_functions:
            grains.item: [id, role, ya, virtual]
