...
data:
    ...
    zk:
        ...
        jvm_xmx: '4G'
        nodes:
            zk-s3-compute01f.svc.cloud.yandex.net: 1
            zk-s3-compute01h.svc.cloud.yandex.net: 2
            zk-s3-compute01k.svc.cloud.yandex.net: 3
        config:
            snapCount: 1000000
...
firewall:
    policy: ACCEPT
    ACCEPT:
      - net: _C_MDB_S3_ZK_COMPUTE_PROD_
        type: macro
        ports:
          - '2888'
          - '3888'
      - net: _C_MDB_S3_COMPUTE_PROD_
        type: macro
        ports:
          - '2181'
    REJECT:
