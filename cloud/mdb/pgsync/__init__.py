...
def parse_cmd_args():
    ...
    parser.add_argument("-c", "--config", dest="config_file", default='/etc/pgsync.conf')
...
def read_config(filename=None, options=None):
    ...
    defaults = {
        'global': {
            ...
            'append_master_conn_string': 'connect_timeout=1',
            'iteration_timeout': 1.0,
            'zk_hosts': 'localhost:2181',
            'zk_lockpath_prefix': None,
            'plugins_path': '/etc/pgsync/plugins',
        ...
        'replica': {
            'master_unavailability_timeout': 5,
            'start_pgbouncer': 'yes',
            'remaster_checks': 3,
        ...
        'commands': {
            ...
            'bouncer_start': 'sudo service pgbouncer start',
            'bouncer_stop': 'sudo service pgbouncer stop',
            'bouncer_status': 'sudo service pgbouncer status >/dev/null 2>&1',
