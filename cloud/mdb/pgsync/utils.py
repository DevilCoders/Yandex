...
class Switchover(object):
    ...
    def reset(self, force=False):
        ...
        if not self._zk.write(zk.SWITCHOVER_STATE_PATH, 'failed', need_lock=False):
            raise SwitchoverException('unable to reset node %s' % zk.SWITCHOVER_STATE_PATH)
        return True
    ...
    def _wait_for_master(self, timeout=None):
        ...
        for _ in range(timeout):
            ...
            if holder is not None and holder != self._plan['master']:
                self._log.info('master is now %s', holder)
                return holder
            self._log.debug('current holder %s, waiting for new master to acquire lock...', holder)
