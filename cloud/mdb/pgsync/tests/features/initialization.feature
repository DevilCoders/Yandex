Feature: Test that tests infrastructure works correctly.
    ...
    @init
    Scenario: Pgsync container check configuration
        Given a "pgsync" container common config
        """
            pgsync.conf:
                replica:
                    remaster_checks: 100500
            pgbouncer.ini:
                pgbouncer:
        ...
        Given a "pgsync" container "postgresql1"
        Given a replication slot "pgsync_postgresql2_1_pgsync_pgsync_net" in container "postgresql1"
        Given a "pgsync" container "postgresql2" with following config
        """
            pgsync.conf:
                replica:
            ...
            postgresql.conf:
                fsync: 'off'
                restore_command: 'rsync -a --password-file=/etc/archive.passwd rsync://archive@pgsync_backup1_1.pgsync_pgsync_net:/archive/%f %p'
            recovery.conf:
                recovery_target_timeline: 'latest'
                primary_conninfo: 'host=pgsync_postgresql1_1.pgsync_pgsync_net'
        """
