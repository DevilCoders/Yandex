...
def get_oldest_replica(replics_info):
    ...
    if len(replics):
        return replics[0]['client_hostname']
    return None
...
def make_current_replics_quorum(replics_info, alive_hosts):
    ...
    alive_replics = set(map(operator.itemgetter('client_hostname'), streaming_replics))
    return set(alive_hosts) & alive_replics
...
def write_status_file(db_state, zk_state, path):
    ...
    try:
        ...
        fname = os.path.join(path, 'pgsync.status')
