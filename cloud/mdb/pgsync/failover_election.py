...
class FailoverElection(object):
    ...
    def __init__(
        self, _zk, timeout, replics_info, replication_manager, allow_data_loss, host_priority, host_lsn, quorum_size
    ):
        self._zk = _zk
    ...
    def _get_host_vote(self, hostname):
        lsn = self._zk.get(zk.get_election_vote_path(hostname) + '/lsn', preproc=int)
        if lsn is None:
            logging.error("Failed to get '%s' lsn for elections." % hostname)
            return None
        priority = self._zk.get(zk.get_election_vote_path(hostname) + '/prio', preproc=int)
        if priority is None:
    ...
    def _collect_votes(self):
        ...
        for info in self._replica_infos:
            replica = info['client_hostname']
            vote = self._get_host_vote(replica)
            if vote is not None:
                votes[replica] = vote
        logging.info('Collected votes are: %s' % str(votes))
        return votes
    ...
    def _vote_in_election(self):
        if not self._zk.ensure_path(zk.get_election_vote_path()):
            raise VoteFailError
        if not self._zk.write(zk.get_election_vote_path() + '/lsn', self._host_lsn, need_lock=False):
            raise VoteFailError
        if not self._zk.write(zk.get_election_vote_path() + '/prio', self._host_priority, need_lock=False):
            raise VoteFailError
    ...
    def _cleanup_votes(self):
        for info in self._replica_infos:
            replica = info['client_hostname']
            if not self._zk.delete(zk.get_election_vote_path(replica), recursive=True):
                raise CleanupError

    def _await_election_status(self, status):
        if not helpers.await_for(
            lambda: self._zk.get(zk.ELECTION_STATUS_PATH) == status, self._timeout, f'election status {status}'
        ):
    ...
    def _write_election_status(self, status):
        ...
        if not self._zk.write(zk.ELECTION_STATUS_PATH, status, need_lock=False):
    ...
    def _participate_in_election(self):
        ...
        if self._zk.get(zk.ELECTION_WINNER_PATH) == helpers.get_hostname():
            if not self._zk.try_acquire_lock(zk.MASTER_LOCK_PATH, timeout=self._timeout):
                return False
            if not self._await_lock_holder_fits(
                zk.ELECTION_MANAGER_LOCK_PATH,
                lambda holder: holder is None,
                f'lock {zk.ELECTION_MANAGER_LOCK_PATH} is empty',
            ):
                raise ElectionTimeout
            if self._zk.get(zk.ELECTION_STATUS_PATH) == STATUS_FAILED:
                self._zk.release_lock(zk.MASTER_LOCK_PATH)
                return False
            return True
        return False
    ...
    def _manage_election(self):
        ...
        if not self._zk.write(zk.ELECTION_WINNER_PATH, winner_host, need_lock=False):
            return False
        self._write_election_status(STATUS_DONE)
        if winner_host == helpers.get_hostname():
            return self._zk.try_acquire_lock(zk.MASTER_LOCK_PATH, timeout=self._timeout)
        if not self._await_lock_holder_fits(
            zk.MASTER_LOCK_PATH, lambda holder: holder is not None, f'lock {zk.MASTER_LOCK_PATH} is not empty'
        ):
    ...
    def make_election(self):
        ...
        # The order of actions inside this function is very important and was validated to avoid race conditions.
        #
        if not self._zk.try_acquire_lock(zk.ELECTION_ENTER_LOCK_PATH, allow_queue=True, timeout=self._timeout):
            return False
        if self._zk.get_current_lock_holder(zk.ELECTION_MANAGER_LOCK_PATH):
            self._zk.release_lock(zk.ELECTION_ENTER_LOCK_PATH)
            return self._participate_in_election()
        if self._zk.get_current_lock_holder(zk.MASTER_LOCK_PATH):
            return False
        self._write_election_status(STATUS_CLEANUP)
        if not self._zk.try_acquire_lock(zk.ELECTION_MANAGER_LOCK_PATH, timeout=self._timeout):
            return False
        try:
            self._zk.release_lock(zk.ELECTION_ENTER_LOCK_PATH)
            is_winner = self._manage_election()
        finally:
            self._zk.release_lock(zk.ELECTION_MANAGER_LOCK_PATH)
        return is_winner
