job("arcadia-infra-test"){
    description("""
<h2>Test time</h2>
<br/>
<img src='https://jenkins.db.yandex-team.ru/job/arcadia-infra-test/buildTimeGraph/png' />""")
    logRotator(7, 30)
    label('dbaas')
    concurrentBuild()
    ...
    steps{
        ...
        shell('cd arcadia/cloud/mdb/dbaas_infra_tests && make lint')
        systemGroovyCommand('''
import jenkins.*
...
properties_path = new FilePath(Jenkins.getInstance().getComputer(vars['NODE_NAME']).getChannel(), build.workspace.toString() + "/infra.properties")
properties = new Properties()
properties.load(properties_path.read())
