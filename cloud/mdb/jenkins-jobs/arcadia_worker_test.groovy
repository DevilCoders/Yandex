job("arcadia-worker-test"){
    description("""
<h2>Test time</h2>
<br/>
<img src='https://jenkins.db.yandex-team.ru/job/arcadia-worker-test/buildTimeGraph/png' />""")
    logRotator(7, 30)
    label('built-in')
    concurrentBuild()
...
for (job_name in job_names.split(',')) {
  ...
  if (job_name.contains("dataproc") && !job_name.contains("dataproc_image")) {
    if (!properties.SUMMARY.contains("[dataproc]")) {
      println("No trigger for ${job_name} (Buildong only with [dataproc] flag)")
      continue
    } else {
      found_filter = true
    }
  } else {
    if (properties.SUMMARY.contains("[noci]")) {
        throw new AbortException("noci in worker jobs is forbidden (use deploy_v2 instead)")
    }
