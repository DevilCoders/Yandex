job("arcadia-salt-test"){
    description("""
<h2>Test time</h2>
<br/>
<img src='https://jenkins.db.yandex-team.ru/job/arcadia-salt-test/buildTimeGraph/png' />""")
    logRotator(7, 30)
    label('built-in')
    concurrentBuild()
    ...
    steps{
        shell('FORCE_TAG=diff-check /usr/local/bin/arcadia_pass_context.py --add-summary diff-check')
        systemGroovyCommand('''
import jenkins.*
