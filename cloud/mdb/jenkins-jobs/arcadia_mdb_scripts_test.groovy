job("arcadia-mdb-scripts-test"){
    description("""
<h2>Test time</h2>
<br/>
<img src='https://jenkins.db.yandex-team.ru/job/arcadia-mdb-scripts-test/buildTimeGraph/png' />""")
    ...
    steps {
        ...
        shell('cd arcadia && retry timeout 600 svn up && retry timeout 600 ./ya make -j0 --checkout cloud/mdb/scripts')
        ...
        shell('cd arcadia && retry timeout 600 ./ya make -j0 --checkout cloud/mdb/scripts')
        shell('cd arcadia/cloud/mdb/scripts && PATH=$WORKSPACE/arcadia:$PATH make all')
    }
