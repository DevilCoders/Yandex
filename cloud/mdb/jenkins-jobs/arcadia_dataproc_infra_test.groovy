job("arcadia-dataproc-infra-test"){
    description("""
<h2>Test time</h2>
<br/>
<img src='https://jenkins.db.yandex-team.ru/job/arcadia-dataproc-infra-test/buildTimeGraph/png' />""")
    logRotator(7, 30)
    label('built-in')
    concurrentBuild()
...
def infra_job_names = 'arcadia_infrastructure_dataproc,arcadia_infrastructure_kafka,arcadia_infrastructure_greenplum_compute,arcadia_infrastructure_redis_compute'
def job_names = []

for (job_name in infra_job_names.split(',')) {
    job_names << job_name
}

// we need to add some auto-generated jobs
def all_job_names = '%ARCADIA_DOWNSTREAM_PROJECTS%'
for (job_name in all_job_names.split(',')) {
  if (job_name.contains("arcadia_infrastructure_sqlserver")) {
    job_names << job_name
  }
}
...
for (job_name in job_names) {
  ...
  for (filter in ["dataproc", "kafka", "greenplum", "sqlserver", "redis"]) {
