job("arcadia-idm-service-test"){
    description("""
<h2>Test time</h2>
<br/>
<img src='https://jenkins.db.yandex-team.ru/job/arcadia-idm-service-test/buildTimeGraph/png' />""")
    logRotator(7, 30)
    label('built-in')
    concurrentBuild()
    ...
    steps{
        shell('FORCE_TAG=postgresql /usr/local/bin/arcadia_pass_context.py')
        systemGroovyCommand('''
import jenkins.*
...
for (job_name in job_names.split(',')) {
  ...
  if (!job_name.contains("postgresql")) {
    println("No trigger for ${job_name} (Need to run only postgresql jobs)")
    continue
  }
