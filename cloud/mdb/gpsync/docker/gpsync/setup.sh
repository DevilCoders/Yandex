...
then
    ...
    do
        ...
        supervisorctl start gpsync 2>/dev/null >/dev/null || supervisorctl status gpsync
    ...
    do
        ...
        if [ -f /tmp/gpsync_init ]
        then
            echo "trying to start gpsync"
            supervisorctl start gpsync 2>/dev/null >/dev/null || supervisorctl status gpsync
        else
            echo "starting setup"
            rm -rf ${PGDATA}/* && \
