...
class Postgres(object):
    ...
    def _create_cursor(self):
        try:
            ...
            replica_state_before_reconnect = self.replica_state
            self.reconnect()
            if self.replica_state:
                return
            if replica_state_before_reconnect and self.conn_local and not self.conn_local.closed:
    ...
    def _exec_query(self, query, **kwargs):
        ...
        if not cur:
            raise RuntimeError('Local conn is dead')
        # Reset replica in recovery state since connection is alive
        self.replica_state = dict()
    ...
    def reconnect(self):
        ...
        try:
            ...
            if "FATAL:  the database system is in recovery mode" in e.args[0]:
                self._handle_recovery_mode_exception(e)
                return
            self.replica_state = dict()
    ...
    def get_state(self):
        ...
        """
        fname = '%s/.gpsync_db_state.cache' % self.config.get('global', 'working_dir')
        try:
        ...
        try:
            ...
            if data['role'] == 'master':
                ...
                data['master_dbids'] = self.get_master_dbids()
    ...
    def is_alive(self):
        ...
        try:
            ...
            if (
                ...
                and "FATAL:  the database system is in recovery mode" in e.args[0]
            ):
                self._handle_recovery_mode_exception(e)
                return True
            if isinstance(e, RuntimeError) and e.args[0] == 'Local conn is dead' and self.replica_state:
                return True
            for line in traceback.format_exc().split('\n'):
                logging.debug(line.rstrip())
    ...
    def _handle_recovery_mode_exception(self, err):
        match = re.search(
            r"DETAIL:  last replayed record at (\S+)\n"
            r"- VERSION: PostgreSQL (\d+\.\d+(\.\d+)?) \(Greenplum Database",
            err.args[0],
        )
        last_xlog_replay_location = match.group(1)
        version = match.group(2).split('.')
        version_num = int(version[0]) * 10000 + int(version[1]) * 100
        version_num += int(version[2]) if len(version) == 2 else 0

        self.replica_state = {'last_xlog_replay_location': last_xlog_replay_location, 'version': version_num}
    ...
    def _get_pg_version(self):
        ...
        """
        if self.replica_state:
            return self.replica_state['version']
    ...
    def _get_pgdata_path(self):
        ...
        Get local pg_data
        """
        return os.environ.get('MASTER_DATA_DIRECTORY')
    ...
    def _get_wal_receiver_info(self):
        ...
        TODO: get information without connecting to standby master
        """
        return None
    ...
    @helpers.return_none_on_error
    def get_master_dbids(self):
        """
        Get information about master segment hosts and roles
        """
        return self._get("SELECT dbid, role, hostname FROM gp_segment_configuration WHERE content=-1")

    def remove_standby_master(self):
        """
        Remove standby master from catalog
        """
        self._exec_without_result('SELECT gp_remove_master_standby()')

    def create_standby_master(self, standby_hostname):
        """
        Create standby master in catalog
        """
        self._exec_query(
            "SELECT gp_add_master_standby(%(hostname)s, %(address)s, %(datadir)s)",
            hostname=standby_hostname,
            address=standby_hostname,
            datadir=self.pgdata,
        )
    ...
    @staticmethod
    def lsn_to_bigint(lsn):
        hi, lo = lsn.split("/")
        return int(hi, 16) << 32 + int(lo, 16)

    @helpers.return_none_on_error
    def get_wal_receive_lsn(self):
        return self.lsn_to_bigint(self.replica_state["last_xlog_replay_location"])
    ...
    def check_walreceiver(self, replics_info):
        ...
        TODO: rewrite to avoid connections to standby master
        """
        return True
        try:
    ...
    def create_internal_auto_conf(self):
        """
        Create internal.auto.conf with dbid of replica
        """
        internal_filepath = os.path.join(self.pgdata, 'internal.auto.conf')

        with open(internal_filepath, 'w') as f:
            f.write(f"gp_dbid={self.dbid}\n")
    ...
    def do_rewind(self, master_host):
        ...
        if self.config.getboolean('global', 'use_replication_slots'):
            ...
            #
            try:
                helpers.backup_dir('%s/pg_replslot' % self.pgdata, '/tmp/gpsync_replslots_backup')
            except Exception:
        ...
        if self.config.getboolean('global', 'use_replication_slots') and res == 0:
            if os.path.exists('/tmp/gpsync_replslots_backup'):
                try:
                    helpers.backup_dir('/tmp/gpsync_replslots_backup', '%s/pg_replslot' % self.pgdata)
                except Exception:
    ...
    def stop_archiving_wal(self):
        if self.replica_state:
            return self.stop_archiving_wal_stopped()
