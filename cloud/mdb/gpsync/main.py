...
class Pgsync(object):
    ...
    def startup_checks(self):
        ...
        fname = '%s/.gpsync_rewind_fail.flag' % work_dir
    ...
    def update_maintenance_status(self, role):
    ...
    def run_iteration(self, my_prio):
        ...
        try:
            ...
            self.update_maintenance_status(role)
    ...
    def single_node_master_iter(self, db_state, zk_state):
        ...
        if self._manage_dbid_request(db_state['master_dbids']):
            db_state['master_dbids'] = self.db.get_master_dbids()

        self.zk.write(zk.DBID_INFO_PATH, db_state['master_dbids'], preproc=json.dumps)
    ...
    def non_ha_replica_iter(self, db_state, zk_state):
        try:
            ...
            self.checks['failover'] = 0
            streaming = any(
                [i for i in replics_info if i['state'] == 'streaming' and i['client_hostname'] == my_hostname]
            )
    ...
    def _accept_switchover(self):
        ...
        # writing last switchover timestamp to ZK.
        #
    ...
    def dead_iter(self, db_state, zk_state):
        ...
        if holder and holder != helpers.get_hostname():
            ...
            #
            # First check if this instance is a standby master, and if it is,
            # check for published dbid info in ZK
            if not self._ensure_dbid():
                return None
    ...
    def _save_my_dbid(self, zk_state):
        if zk_state['master_dbids'] is None:
            logging.debug('No dbid info exists in ZK, resetting saved dbid')
            self.db.dbid = None
            return

        for dbid_info in zk_state['master_dbids']:
            if dbid_info['hostname'] == helpers.get_hostname():
                self.db.dbid = dbid_info['dbid']
                return
        self.db.dbid = None
        self._request_new_dbid()

    def _ensure_dbid(self):
        master = self.db.recovery_conf('get_master')

        if master is None:
            return True
        if self.db.dbid is None:
            logging.error('Dbid info for my hostname not found in ZK. Refusing to start standby master')
            self._request_new_dbid()
            return False
        self.db.create_internal_auto_conf()
        return True

    def _request_new_dbid(self):
        hostname = helpers.get_hostname()
        existing_request = self.zk.get(zk.CREATE_DBID_FOR_STANDBY_REQUEST_PATH)
        if existing_request is None:
            self.zk.write(zk.CREATE_DBID_FOR_STANDBY_REQUEST_PATH, hostname, need_lock=False)
            logging.debug(f'Created request for dbid for standby master for {hostname}')

    def _manage_dbid_request(self, master_dbids):
        request = self.zk.get(zk.CREATE_DBID_FOR_STANDBY_REQUEST_PATH)
        if request is None:
            return False
        for dbid_info in master_dbids:
            if request == dbid_info['hostname']:
                logging.debug(f'Requested standby dbid for hostname {request} alredy satisfied, removing request')
                self.zk.delete(zk.CREATE_DBID_FOR_STANDBY_REQUEST_PATH)
                return False
            if dbid_info['role'] == 'm':
                logging.debug(
                    f'Standby master already exists for other hostname {dbid_info["hostname"]}'
                    'removing it and recreating standby master'
                )
                self.db.remove_standby_master()
        self.db.create_standby_master(request)
        logging.debug(f'Created standby master for host {request}')
        self.zk.delete(zk.CREATE_DBID_FOR_STANDBY_REQUEST_PATH)
        return True
    ...
    def _verify_timeline(self, db_state, zk_state):
        ...
        if tli_res:
            if zk_state['replics_info_written'] is False:
                ...
                # but checking it just in case.
                #
                if self.db.pgbouncer('status'):
                    logging.warning('Error with ZK. Here we should close from ' 'load.')
                    self.db.pgbouncer('stop')
                return None
    ...
    def _simple_remaster(self, limit, new_master, is_dead):
        ...
        stream_from = self.config.get('global', 'stream_from')
        if stream_from:
            logging.warning('Return replica to stream_from %s ', stream_from)
            new_master = stream_from
        ...
        if self._wait_for_recovery(limit) and self._check_archive_recovery(limit):
            ...
            #
            if self._wait_for_streaming(limit):
                #
    ...
    def _return_to_cluster(self, new_master, role, is_dead=False):
        ...
        try:
            ...
            if not self.config.getboolean('global', 'enable_rewind'):
                logging.error('We need to rewind but rewind is not enabled. Exiting.')
                self._set_rewind_flag_and_exit(limit)
    ...
    def _set_rewind_flag_and_exit(self, timeout):
        self.db.pgbouncer('stop')
        self.db.stop_postgresql(timeout=timeout)
        work_dir = self.config.get('global', 'working_dir')
        fname = '%s/.gpsync_rewind_fail.flag' % work_dir
    ...
    def _can_do_failover(self):
        ...
        election = FailoverElection(
            ...
            len(zk.get_alive_hosts(self.zk, election_timeout / 2)),
        )
        try:
    ...
    def _get_replics_info_from_zk(self):
        stream_from = self.config.get('global', 'stream_from')
        if stream_from:
            replics_info_path = '{member_path}/{hostname}/replics_info'.format(
                member_path=zk.MEMBERS_PATH, hostname=stream_from
    ...
    def _check_postgresql_streaming(self):
        if not self.db.is_alive():
        ...
        try:
            replica_infos = self._get_replics_info_from_zk()
        except ZookeeperException:
            logging.error("Can't get replics_info from ZK. Won't wait for timeout.")
            return False

        if replica_infos is not None and (
            Pgsync._is_caught_up(replica_infos) or self.db.check_walreceiver(replica_infos)
        ):
    ...
    def _wait_for_streaming(self, limit=-1):
        """
        Stop until postgresql start streaming from master.
        With limit=-1 the loop here can be infinite.
        """
        return helpers.await_for_value(
            self._check_postgresql_streaming, limit, 'PostgreSQL started streaming from master'
        )
    ...
    def _fix_hostname(self, replics_info):
        ...
        try:
            ...
            for host in alive_hosts:
                wal_receiver_info = self.zk.get('{}/{}/wal_receiver'.format(zk.MEMBERS_PATH, host), preproc=json.loads)
