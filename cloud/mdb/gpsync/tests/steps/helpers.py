...
PGDATA = '/var/lib/greenplum/data1/master/gpseg-1'

CONFIG_ENVS = {
    'gpsync.conf': '/etc/gpsync.conf',
    ...
    'recovery.conf': '{pgdata}/recovery.conf'.format(pgdata=PGDATA),
    'standby.signal': '{pgdata}/standby.signal'.format(pgdata=PGDATA),
    'odyssey.conf': '/etc/odyssey/odyssey.conf',
}

CONTAINER_PORTS = {'gpsync': ['5432', '6432'], 'zookeeper': ['2181'], 'backup': ['873'], 'greenplum': []}
SERVICE_PORTS = {'greenplum': 5432, 'pgbouncer': 6432}
...
def container_get_health_status(container):
    container.reload()
...
def build_config_get_path(build):
    if isinstance(build, dict):
        if 'dockerfile' in build:
            return build['dockerfile'][: -len('/Dockerfile')]
...
def set_switchover(container, params):
    return container.exec_run('gpsync-util switchover -y {params}'.format(params=params))


def get_zk(context, name):
    container = context.containers[name]
    return KazooClient('{host}:{port}'.format(host=container_get_host(), port=container_get_tcp_port(container, 2181)))
...
def time_to_nanoseconds(value):
    parsed, flag = parsedatetime.Calendar().parse(value, sourceTime=time.gmtime(0))
    if flag == 0:
        return None
    return int(1000 * 1000 * 1000 * calendar.timegm(parsed))
