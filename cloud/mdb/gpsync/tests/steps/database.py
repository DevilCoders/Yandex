...
class Postgres(object):
    def __init__(
        self, host='localhost', dbname='postgres', user='postgres', port='5432', autocommit=True, async_=False, **kwargs
    ):
        self.replica_in_recovery_mode = False
        self.conn = None
        try:
            self.conn = psycopg2.connect(
                user=user, host=host, port=port, dbname=dbname, async_=async_, connect_timeout=1, **kwargs
            )
            ...
            if "FATAL:  the database system is in recovery mode" in error.args[0]:
                self.replica_in_recovery_mode = True
                return
            assert False, error
    ...
    def is_master(self):
        if self.replica_in_recovery_mode:
            return False
        self.cursor.execute(
            """
...
class Greenplum(Postgres):
    def __init__(self, *args, user='gpadmin', options='-c gp_session_role=utility', **kwargs):
        super().__init__(*args, user=user, options=options, **kwargs)

    def all_segments_are_up(self):
        self.cursor.execute(
            """
            SELECT COUNT(*) AS down_segments FROM gp_segment_configuration
            WHERE status != 'u'
            """
        )
        return deepcopy(self.cursor.fetchone())['down_segments'] == 0
