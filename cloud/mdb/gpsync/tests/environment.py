...
def before_all(context):
    ...
    context.timeout = float(os.environ.get('TIMEOUT', 120))
    ...
    with open('cloud/mdb/gpsync/docker-compose.yml', 'r') as compose_file:
...
def after_step(context, step):
    if step.status == 'failed':
        if step.filename == '<string>':
            ...
            return
        base_dir = 'cloud/mdb/gpsync/logs'
        ...
        for container in context.containers.values():
            ...
            log_files = [
                ('/var/log/', 'gpsync.log'),
                ('/var/log/', 'greenplum.log'),
                ('/var/lib/greenplum/data1/master/gpseg-1/pg_log', 'greenplum-6-data.csv'),
