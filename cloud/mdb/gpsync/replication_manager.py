...
class QuorumReplicationManager:
    ...
    def is_promote_safe(self, host_group, **kwargs):
        ...
        logging.info(f'Host group was: {host_group}')
        hosts_in_quorum = len(set(sync_quorum) & set(host_group))
        # In case of 2 hosts common formula doesn't works
        if hosts_in_quorum == 1 and len(sync_quorum) == 2:
            return True
        # For all other cases
