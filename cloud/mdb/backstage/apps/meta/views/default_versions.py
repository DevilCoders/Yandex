...
class DefaultVersionListSkelView(django.views.generic.View):
    def get(self, request):
        ctx = {
            'menu': 'Meta',
            'get_params': request.GET.urlencode(),
            'filters': mod_filters.get_default_versions_filters(request),
            'onload_url': '/ui/meta/ajax/default_versions',
            'title': 'Default versions',
        }
...
@django.utils.decorators.method_decorator(iam_decorators.auth_required, name='dispatch')
class DefaultVersionListView(django.views.generic.View):
    def get(self, request):
        ...
        ctx = {
            'filters': filters,
            'js': {
                'identifier': 'meta_default_versions',
                'array_name': 'meta_default_versions_selected',
        ...
        if not filters.errors:
            ctx = mod_helpers.paginator_setup_set_context(
                queryset=mod_models.DefaultVersion.ui_list.filter(**filters.queryset),
                request=request,
                context=ctx,
                items_per_page=200,
            )

        response.html = mod_helpers.render_html('meta/default_versions/default_versions.html', ctx, request)
