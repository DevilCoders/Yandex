job("graphene-master-storage-test-thread"){
    description("""
<h2>Test time</h2>
<br/>
<img src='https://jenkins.db.yandex-team.ru/job/graphene-master-storage-test-thread/buildTimeGraph/png' />""")
    ...
    steps{
        shell('''|git init
                 ...
                 |    exit 0
                 |fi
                 |CC=/usr/bin/clang-14 CXX=/usr/bin/clang++-14 CFLAGS="-fno-omit-frame-pointer" PATH="/usr/lib/llvm-14/bin:${PATH}" ./configure --enable-debug --enable-cassert
                 |make PATH="/usr/lib/llvm-14/bin:${PATH}" -j 32
                 |cd storage
                 |CC=/usr/bin/clang-14 CXX=/usr/bin/clang++-14 PATH="/usr/lib/llvm-14/bin:${PATH}" cmake -Bbuild -DUSE_SANITIZER='Thread' -DBUILD_SERVER=OFF -DBUILD_TEST=ON
                 |cd build
