job("graphene-master-storage-benchmark"){
    description("""
<h2>Test time</h2>
<br/>
<img src='https://jenkins.db.yandex-team.ru/job/graphene-master-storage-benchmark/buildTimeGraph/png' />""")
    ...
    steps{
        shell('''|git init
                 ...
                 |cd storage
                 |CC=/usr/bin/clang-14 CXX=/usr/bin/clang++-14 PATH="/usr/lib/llvm-14/bin:${PATH}" cmake -Bbuild -DBUILD_SERVER=OFF -DBUILD_BENCHMARK=ON -DCMAKE_BUILD_TYPE=Release
                 |cd build
                 |make storage_benchmark -j 32
                 |./storage_benchmark --benchmark_repetitions=10'''.stripMargin('|'))
    }
