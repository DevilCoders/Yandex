...
if [ -z ${cms_explanation+x} ]
then
    echo 'set $cms_explanation var, for example: export cms_explanation="this host cannot be managed"'
    exit 1
fi
...
cms_login=$USER cat ${cms_script_path}/manage_reject.sql | $cms_ssh_cmd "psql -U postgres --variable=your_login='`echo $cms_login`' --variable=explanation='`echo $cms_explanation`' --variable=request_id='`echo $cms_req_id`' --quiet -d cmsdb <&0"
