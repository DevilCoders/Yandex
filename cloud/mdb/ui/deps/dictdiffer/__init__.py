...
from ._compat import PY2, Iterable, MutableMapping, MutableSequence, MutableSet, string_types, text_type  # noqa
...
def diff(first, second, node=None, ignore=None, path_limit=None, expand=False, tolerance=EPSILON, dot_notation=True):
    ...
    def dotted(node, default_type=list):
        ...
        if dot_notation and all(map(lambda x: isinstance(x, string_types) and '.' not in x, node)):
            return '.'.join(node)
        else:
        ...
        if isinstance(_first, DICT_TYPES) and isinstance(_second, DICT_TYPES):
            ...
            def check(key):
                ...
                return ignore is None or (
                    dotted(_node + [key], default_type=tuple) not in ignore and tuple(_node + [key]) not in ignore
                )
        ...
        if differ:
            ...
            for key in intersection:
                ...
                if path_limit and path_limit.path_is_limit(_node + [key]):
                    yield CHANGE, _node + [key], (deepcopy(_first[key]), deepcopy(_second[key]))
                else:
            ...
            if addition:
                if path_limit:
                    ...
                    collect_recurred = []
                    for key in addition:
                        if not isinstance(_second[key], SET_TYPES + LIST_TYPES + DICT_TYPES):
