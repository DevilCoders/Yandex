...
class ServerMock(Flask):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.add_url_rule(
            '/mock/invocations', view_func=self._get_invocations_handler)

        self.add_url_rule(
            '/mock/rules', view_func=self._get_rules, methods=['GET'])

        self.add_url_rule(
            '/mock/rules', view_func=self._post_rules, methods=['POST'])

        self.add_url_rule(
            '/mock/reset', view_func=self._reset_handler, methods=['POST'])
    ...
    def _config_handler(self):
        ...
        """
        return Response(
            json.dumps(self.config, default=repr), mimetype='application/json')
