...
func TestDBSpecificElasticsearch(t *testing.T) {

	tests := elasticsearch.GetTestServices()
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%s status %d", test.Expected, i), func(t *testing.T) {
			...
			topology := generateClusterTopologyByHealth(metadb.ElasticSearchCluster, cid, roles, health)
			...
			require.NoError(t, doProceedCycle(ctx, ds, metadb.ElasticSearchCluster), "process update cycle")
