...
func TestDBSpecificKafka(t *testing.T) {

	tests := kafka.GetTestServices()
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%s status %d", test.Expected, i), func(t *testing.T) {
			...
			topology := generateClusterTopologyByHealth(metadb.KafkaCluster, cid, roles, health)
			...
			require.NoError(t, doProceedCycle(ctx, ds, metadb.KafkaCluster), "process update cycle")
