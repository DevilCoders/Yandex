...
func TestDBSpecificRedis(t *testing.T) {

	tests := redis.GetTestServices()
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%s status %d", test.Expected, i), func(t *testing.T) {
			...
			cid, _, roles, health := dbspecifictests.GenerateOneHostWithAllServices(test.Input)
			topology := generateClusterTopologyByHealth(metadb.RedisCluster, cid, roles, health)
			...
			require.NoError(t, doProceedCycle(ctx, ds, metadb.RedisCluster), "process update cycle")
			...
			for fqdn, sh := range health {
				res, err := ds.LoadHostsHealth(ctx, []string{fqdn})
				require.NoError(t, err)
				loadHealth := res[0]
				expected := types.NewHostHealthWithStatus(cid, fqdn, sh, test.ExpectHost)
				require.Equal(t, expected.ClusterID(), loadHealth.ClusterID())
				require.Equal(t, expected.FQDN(), loadHealth.FQDN())
				require.Equal(t, expected.Status(), loadHealth.Status())
				compareServices(t, expected.Services(), loadHealth.Services())
			}
		})
	}
}
