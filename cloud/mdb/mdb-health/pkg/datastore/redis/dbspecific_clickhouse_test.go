...
func TestDBSpecificClickhouse(t *testing.T) {

	tests := clickhouse.GetTestServices()
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%s status %d", test.Expected, i), func(t *testing.T) {
			...
			topology := generateClusterTopologyByHealth(metadb.ClickhouseCluster, cid, roles, health)
			...
			require.NoError(t, doProceedCycle(ctx, ds, metadb.ClickhouseCluster), "process update cycle")
