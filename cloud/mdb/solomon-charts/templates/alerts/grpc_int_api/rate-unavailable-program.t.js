...
let s_unavailable = drop_empty_lines(group_lines(
    'sum',
    {
        project='<< project_id >>',
        cluster='<< grpc_int_api.get("cluster") >>',
        ...
        DC!='total', 
        name='prometheus.grpc_server_handled_total',
        grpc_code='Unavailable|DeadlineExceeded'
    }
));

no_data_if(size(s_total) == 0);
no_data_if(size(s_unavailable) == 0);

let rate = avg(100 * s_unavailable/s_total);
let count = sum(s_unavailable);

let str_rate = to_fixed(rate, 2);
let str_count = to_fixed(count, 0);

// Don't alarm on single Unknown (1-2 in a window)
alarm_if(rate >= 1.0 && count >= 3.0);
warn_if(rate >= 0.5);
