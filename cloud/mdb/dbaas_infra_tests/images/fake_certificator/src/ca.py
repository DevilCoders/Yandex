...
class PKI:
    ...
    def _gen_cert(self, name, alt_names=None, key_len=2048):
        """
        ...
        if not alt_names:
            alt_names = [name]
        alt_names = ['DNS:{name}'.format(name=alt_name) for alt_name in alt_names]
        alt_names = ', '.join(alt_names).encode('utf-8')
        cert.add_extensions([
    ...
    def get_cert(self, name, alt_names=None):
        """
        Return cert in PEM format
        """
        if name not in self.certs:
            self._gen_cert(name, alt_names)
