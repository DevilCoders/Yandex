@dependent-scenarios
Feature: Internal API Redis Sharded Cluster lifecycle
  ...
  Background: Wait until internal api is ready
    ...
    And we are working with sharded Redis cluster

  @setup
  Scenario: Redis sharded cluster creation works
    When we enable feature flag "MDB_REDIS_SHARDING"
    And we enable feature flag "MDB_REDIS_ALLOW_DEPRECATED_5"
  ...
  Scenario: Upgrade Sharded Redis cluster to 6.0
    Given we are able to log in to "sharded_redis" with following parameters
    """
    password: newpassword
    """
