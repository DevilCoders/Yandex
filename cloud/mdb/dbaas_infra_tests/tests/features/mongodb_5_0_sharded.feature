...
Feature: Internal API MongoDB Cluster lifecycle
  ...
  Background: Wait until internal api is ready
    ...
    And we are working with standard_replicaset_5_0 MongoDB cluster
  ...
  Scenario: MongoDB cluster creation works
    ...
    And mongodb major version is "5.0"
    And mongodb featureCompatibilityVersion is "5.0"
  ...
  Scenario: Sharding role works properly
    ...
    And user_testsh_db inserts 100 test mongodb docs into "testsh_db.test" MONGOS
  ...
  Scenario: MongoDB backups
    ...
    And user_testdb2 inserts 10 test mongodb docs into "testdb2.test" MONGOD "shard2"
    Then user_testdb2 has 25 test mongodb docs in "testdb2.test" MONGOD "shard2"
    When we save shard "shard2" last oplog timestamp to "after_second_load"
    And user_testdb2 inserts 100 test mongodb docs into "testdb2.test" MONGOD "shard2"
    Then user_testdb2 has 125 test mongodb docs in "testdb2.test" MONGOD "shard2"
    When we wait until shard "shard2" timestamp next to "after_second_load" appears in storage
    And we restore MongoDB using latest "backups" timestamp "after_second_load" containing "shard2" and config
    """
    ...
    Then user_testdb2 has 25 test mongodb docs in "testdb2.test" MONGOD "rs01"
    And cluster has no pending changes
