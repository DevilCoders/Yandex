...
Feature: Internal API PostgreSQL Cluster lifecycle
  ...
  Background: Wait until internal api is ready
    ...
    And we are working with standard 14 PostgreSQL cluster
 ...
 @extensions
  ...
  Scenario: Change PostgreSQL shared_preload_libraries
    ...
    And following SQL request on master in "test_cluster" succeeds
    """
    SELECT 'ok' result FROM pg_settings WHERE name='shared_preload_libraries' AND setting~'auto_explain|pg_hint_plan|timescaledb|pg_qualstats'
    """
    And query result is like
    """
    - result: ok
  ...
  Scenario: Remove PostgreSQL database from cluster
    Given cluster "test_cluster" is up and running
    When we attempt to remove database "testdb2" in cluster
    Then response should have status 200
    And generated task is finished within "15 minutes"
    And cluster has no pending changes
  ...
  Scenario: Remove cluster
    Given cluster "test_cluster" is up and running
    When we create backup for PostgreSQL "test_cluster"
    Then response should have status 200
    And generated task is finished within "15 minutes"
    And we get PostgreSQL backups for "test_cluster"
    Then response should have status 200
    And message with "backups" list is larger than "0"
    When we remember current response as "backups_before_remove_cluster"
  ...
  @backups
  Scenario: Restore deleted cluster to latest restore point works
    # restore cluster with smaller instanceType - MDB-2722
    # restore cluster with time in future - MDB-12564
    Then we remember current timestamp as "pitr_moment"
    When we restore PostgreSQL using latest "backups_before_remove_cluster" timestamp "pitr_moment" and config
    """
    name: test_cluster_at_latest_restore_point
    environment: PRESTABLE
    hostSpecs:
      - zoneId: man
      - zoneId: vla
    ...
    configSpec:
      postgresqlConfig_14:
        sharedPreloadLibraries: ["SHARED_PRELOAD_LIBRARIES_AUTO_EXPLAIN", "SHARED_PRELOAD_LIBRARIES_PG_HINT_PLAN", "SHARED_PRELOAD_LIBRARIES_TIMESCALEDB", "SHARED_PRELOAD_LIBRARIES_PG_QUALSTATS"]
      resources:
        resourcePresetId: db1.nano
        diskSize: 20000000000
        diskTypeId: local-ssd
    ...
    And following SQL request on master in "test_cluster_at_latest_restore_point" succeeds
    """
    SELECT message FROM mail.box
    """
    And query result is exactly
    """
    []
