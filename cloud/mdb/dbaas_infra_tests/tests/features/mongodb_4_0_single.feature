@dependent-scenarios
Feature: Internal API MongoDB Single lifecycle
  ...
  Background: Wait until internal api is ready
    ...
    And we are working with one_node_replicaset_4_0 MongoDB cluster
  ...
  Scenario: MongoDB cluster creation works
    ...
    And mongodb MONGOD "rs01" shard replset in cluster "test_cluster" has 1 members
  ...
  Scenario: Add MongoDB host to cluster
    ...
    And cluster "test_cluster" is up and running
    And cluster has no pending changes
    And mongodb MONGOD "rs01" shard replset in cluster "test_cluster" has 2 members
  ...
  Scenario: MongoDB backups
    ...
    And we restore MongoDB using latest "backups" and config
    """
    ...
    Then another_test_user has 5 test mongodb docs in "testdb1.test"
    And cluster has no pending changes
