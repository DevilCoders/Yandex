...
Feature: Internal API MySQL Cluster lifecycle
  ...
  Background: Wait until internal api is ready
    ...
    And we are working with standard MySQL cluster

  @setup
  Scenario: MySQL cluster creation works
    When we try to create cluster "test_mysql_renamed"
    Then response should have status 200
    And generated task is finished within "15 minutes"
    And all hosts in mysql "test_mysql_renamed" are online
    And cluster "test_mysql_renamed" is up and running
  ...
  Scenario: Rename MySQL cluster
    ...
    When we attempt to modify cluster "test_mysql_renamed" with following parameters
    """
    name: test_mysql
    """
    Then response should have status 200
    ...
    """
    grep -q "test_mysql" /etc/dbaas.conf
    """
  ...
  Scenario: Backup priority works
    Given cluster "test_mysql" is up and running
    When we attempt to modify host in "vla" in cluster "test_mysql" with backup_priority "10"
    Then response should have status 200
    ...
    Then response should have status 200
    When we create backup for MySQL "test_mysql"
    Then response should have status 200
    And generated task is finished within "5 minutes"
    ...
    | sas         |
    | vla         |
    When we attempt to modify host in "sas" in cluster "test_mysql" with backup_priority "15"
    Then response should have status 200
    And generated task is finished within "5 minutes"
    When we create backup for MySQL "test_mysql"
    Then response should have status 200
  ...
  Scenario: Change MySQL cluster option without restart
    Given cluster "test_mysql" is up and running
    When we attempt to modify cluster "test_mysql" with following parameters
    """
    configSpec:
      mysqlConfig_5_7:
        maxConnections: 30
        innodbBufferPoolSize: 535822337
        slowQueryLog: ON
        longQueryTime: 1.5
        generalLog: ON
  ...
  Scenario: Change MySQL cluster options with restart
    ...
    And following SQL request in "test_mysql" succeeds
    """
    SELECT @@global.default_authentication_plugin AS defaultAuthenticationPlugin
    """
    And query result is like
    """
     - defaultAuthenticationPlugin: sha256_password
  ...
  Scenario: Add user via IDM service
    ...
    Then generated task is finished within "15 minutes"
    And cluster has no pending changes
    When we attempt to add grant "reader" to "some_user" on cluster "test_mysql"
    Then generated task is finished within "15 minutes"
    And cluster has no pending changes
    And user "some_user" has grant "reader" on cluster "test_mysql"
  ...
  Scenario: Rotate passwords via IDM service
    ...
    And all IDM users on cluster "test_mysql" have new passwords
    And cluster has no pending changes

  @idm
  Scenario: Remove role from user via IDM service
    Given cluster "test_mysql" is up and running
    When we attempt to revoke grant "reader" from "some_user" on cluster "test_mysql"
    Then generated task is finished within "15 minutes"
    And user "some_user" doesn't have grant "reader" on cluster "test_mysql"
    And cluster has no pending changes
  ...
  Scenario: Get list of IDM clusters
    ...
    Then IDM response OK and body has "test_mysql"
