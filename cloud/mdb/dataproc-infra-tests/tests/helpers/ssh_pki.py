...
def gen_pair(state: dict, name: str) -> None:
    """
    Retrieve a pair of private and public keys for given name
    """
    if not state.get('ssh_pki'):
        raise KeyError('ssh_pki isn\'t initialized')
    return state['ssh_pki'].gen_pair(name)
