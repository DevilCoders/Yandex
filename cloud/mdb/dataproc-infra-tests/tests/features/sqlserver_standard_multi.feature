Feature: Create Compute SQLServer cluster
    ...
    Background: Wait until go internal api is ready
        Given we are working with standard SQLServer cluster

    @create
    Scenario: SQL Server SE HADR creation works
        When we try to create sqlserver cluster "multi_node_cluster" with following config overrides
        """
        { 
            "hostSpecs": [{
    ...
    @backup
    Scenario: SQL Server SE HADR backup creation works
        Given sqlserver cluster "multi_node_cluster" is up and running
    ...
    @backup
    Scenario: Restoring SQL Server SE HADR from backup to original folder works
        Given sqlserver cluster "multi_node_cluster" is up and running
        ...
        hostSpecs:
            - { zoneId: ru-central1-b, subnetId: bltnas5si736upc8ucq9 }
            - { zoneId: ru-central1-b, subnetId: bltnas5si736upc8ucq9}
        """
        Then grpc response should have status OK
        And generated task is finished within "30 minutes" via GRPC
  ...
  @delete
  Scenario: Remove SQL Server SE HADR cluster
        Given sqlserver cluster "multi_node_cluster" is up and running
        When we try to remove SQLServer cluster
        Then grpc response should have status OK
        And generated task is finished within "5 minutes" via GRPC
