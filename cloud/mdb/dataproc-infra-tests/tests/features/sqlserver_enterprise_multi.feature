Feature: Create Compute SQLServer cluster
    ...
    Scenario: SQL Server EE creation works
        When we try to create sqlserver cluster "multi_node_cluster"
        Then grpc response should have status OK
        And generated task is finished within "30 minutes" via GRPC
        And sqlserver cluster "multi_node_cluster" is up and running
        And hadr replication on cluster "multi_node_cluster" is running
        And sqlserver database "testdb" exists
        And sqlserver user "test" can connect to "testdb"
        When we add test data "foo" on cluster "multi_node_cluster" in "testdb"
        Then test data "foo" exists on cluster "multi_node_cluster" in "testdb"
   
    Scenario: Database management
        Given sqlserver cluster "multi_node_cluster" is up and running
        When we try to create database "betadb"
        Then grpc response should have status OK
        And generated task is finished within "4 minutes" via GRPC
    ...
    Scenario: Backup export and import works
        ...
        Then grpc response should have status OK
        And generated task is finished within "4 minutes" via GRPC
        And test data "foo" exists on cluster "multi_node_cluster" in "testdb_reimported"
    ...
    Scenario: Restore existing database
        ...
        Then grpc response should have status OK
        And generated task is finished within "4 minutes" via GRPC
        And test data "foo" exists on cluster "multi_node_cluster" in "testdb_copy"
        ...
        Then grpc response should have status OK
        And generated task is finished within "4 minutes" via GRPC
        And test data "foo" exists on cluster "multi_node_cluster" in "testdb_copy2"

    @backup
    Scenario: Restoring SQL Server EE from backup to original folder works
        Given sqlserver cluster "multi_node_cluster" is up and running
        When we restore sqlserver cluster from latest backup from cluster "multi_node_cluster" via GRPC
        """
        ...
        And hadr replication on cluster "restored_sqlserver" is running
        When we try to remove SQLServer cluster
        Then grpc response should have status OK
        And generated task is finished within "5 minutes" via GRPC

    Scenario: SQL Server EE failover works
        Given sqlserver cluster "multi_node_cluster" is up and running
        When we attempt to failover sqlserver cluster "multi_node_cluster" to host in geo "ru-central1-a" via GRPC
        Then grpc response should have status OK
        And generated task is finished within "5 minutes" via GRPC
        Then following SQL request in SQLServer cluster "multi_node_cluster" in geo "ru-central1-a" succeeds
        """
        exec msdb.dbo.mdb_is_replica
        """
        And query result is exactly
        """
        - False
        """
        Then hadr replication on cluster "multi_node_cluster" is running within "2 minutes"

        When we attempt to failover sqlserver cluster "multi_node_cluster" to host in geo "ru-central1-c" via GRPC
        Then grpc response should have status OK
        And generated task is finished within "5 minutes" via GRPC
        Then following SQL request in SQLServer cluster "multi_node_cluster" in geo "ru-central1-c" succeeds
        """
        exec msdb.dbo.mdb_is_replica
        """
        And query result is exactly
    ...
    Scenario: scale flavor up
        ...
        Then grpc response should have status OK
        And generated task is finished within "30 minutes" via GRPC
        And test data "foo" exists on cluster "multi_node_cluster" in "testdb"
        And hadr replication on cluster "multi_node_cluster" is running
