Feature: Create Compute SQLServer cluster
    ...
    @create
    Scenario: SQL Server SE creation works
        When we try to create sqlserver cluster "single_node_cluster"
    ...
    @backup
    Scenario: SQL Server SE backup creation works
        Given sqlserver cluster "single_node_cluster" is up and running
    ...
    @restore
    Scenario: Restoring SQL Server SE from backup to original folder works
        Given sqlserver cluster "single_node_cluster" is up and running
    ...
    @delete
    Scenario: Remove SQL Server SE cluster
        Given sqlserver cluster "single_node_cluster" is up and running
        When we try to remove SQLServer cluster
        Then grpc response should have status OK
        And generated task is finished within "5 minutes" via GRPC
