...
func NewApp(configFile, logLevel string, interactive bool) (*App, error) {
	...
	if !interactive {
		...
		logPath = config.Log
		if logPath != stderr {
			err := logrotate.RegisterLogrotateSink(syscall.SIGUSR2)
			if err != nil {
				return nil, err
			}
			logPath = "logrotate://" + logPath
		}
	}
	logLevelN, err := log.ParseLevel(logLevel)
	if err != nil {
		return nil, err
	}
	loggerConfig := zap.KVConfig(logLevelN)
...
func (app *App) baseContext() context.Context {
	return signals.WithCancelOnSignal(context.Background())
}
...
func (app *App) stateLost() appState {
	...
	if localNodeState.IsMaster {
		err = node.SetReadOnlyWithForce(app.config.ExcludeUsers)
		...
		err = node.SetReadOnlyWithExtraForce(app.config.ExcludeUsers)
		if err != nil {
			app.logger.Errorf("failed to set node %s read-only: %v", node.Host(), err)
			return stateLost
		}
	} else {
		err = node.SetReadOnly()
	}
	if err != nil {
...
func (app *App) stateManager() appState {
	...
	if err != nil {
		...
		if xerrors.Is(err, ErrManyMasters) {
	...
	if !clusterStateDcs[master].PingOk {
...
func (app *App) calcActiveNodes(clusterState, clusterStateDcs map[string]*NodeState, oldActiveNodes []string, master string) (activeNodes []string, err error) {
	...
	for host, node := range clusterState {
		...
		if hostsOnRecovery != nil && slices.ContainsString(hostsOnRecovery, host) {
			continue
		}
		if !node.PingOk {
			if node.PingDubious || clusterStateDcs[host].PingOk {
				// we can't rely on ping and slave status if ping was dubios
				if slices.ContainsString(oldActiveNodes, host) {
...
func (app *App) performSwitchover(clusterState map[string]*NodeState, activeNodes []string, switchover *Switchover, oldMaster string) error {
	if switchover.To != "" {
		if !slices.ContainsString(activeNodes, switchover.To) {
	...
	errs := util.RunParallel(func(host string) error {
		...
		if err != nil || app.emulateError("freeze_ro") {
			...
			if err := node.SetReadOnlyWithForce(app.config.ExcludeUsers); err != nil {
...
func (app *App) repairReadOnlyOnMaster(masterNode *mysql.Node, state *NodeState, clusterStateDcs map[string]*NodeState) {
	needRo := false
	...
	if replicasRunning > 0 {
		if replicasRunning == replicasLow {
	...
	if needRo {
		if state.IsReadOnly && (app.config.KeepSuperWritableOnCriticalDiskUsage != state.IsSuperReadOnly) {
			app.logger.Infof("diskusage: master is already read-only")
			return
		}
		var err error
		if app.config.KeepSuperWritableOnCriticalDiskUsage {
			err = masterNode.SetReadOnlyNoSuper()
		} else {
			err = masterNode.SetReadOnly()
		}
		if err != nil {
...
func (app *App) repairMasterNode(node *mysql.Node, clusterState, clusterStateDcs map[string]*NodeState) {
	host := node.Host()
	state := clusterState[host]

	// enter read-only if disk is full
	app.repairReadOnlyOnMaster(node, state, clusterStateDcs)

	events, err := node.ReenableEvents()
	if err != nil {
...
func (app *App) repairSlaveNode(node *mysql.Node, clusterState, clusterStateDcs map[string]*NodeState, master string) {
	...
	if state.IsMaster {
		...
		err := node.SetOffline()
		if err != nil {
			app.logger.Errorf("repair: cannot set stale master %s offline: %s", host, err)
		}
		app.logger.Infof("repair: disable semi-sync on stale master %s", host)
		err = node.SemiSyncDisable()
		if err != nil {
			app.logger.Errorf("repair: failed to disable semi-sync on stale master %s: %v", host, err)
		}
...
func (app *App) findBestStreamFrom(node *mysql.Node, clusterState map[string]*NodeState, master string, cascadeTopology map[string]mysql.CascadeNodeConfiguration) string {
	...
	for {
		...
		if slices.ContainsString(loopDetector, streamFrom) {
...
func (app *App) leaveMaintenance() error {
	...
	if err != nil {
		if xerrors.Is(err, ErrNoMaster) || xerrors.Is(err, ErrManyMasters) {
			app.writeEmergeFile(err.Error())
		}
		return err
	}
...
func (app *App) setRecovery(host string) error {
	err := app.dcs.Delete(dcs.JoinPath(pathActiveNodes, host))
	if err != nil {
		return err
	}
