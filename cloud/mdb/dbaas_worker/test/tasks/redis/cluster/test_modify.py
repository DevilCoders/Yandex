...
def test_porto_redis_cluster_modify_interrupt_consistency(mocker):
    """
...
def test_compute_redis_cluster_modify_interrupt_consistency(mocker):
    """
...
def test_redis_cluster_modify_alert_sync(mocker):
    """
...
def test_redis_cluster_modify_mlock_usage(mocker):
    """
    Check modify mlock usage
    """
    args = {
...
def test_redis_cluster_modify_alt_conductor_group(mocker):
    """
    Check that cluster correctly changes alt conductor group on modify
    """
    args = {
        'hosts': {
            'host1': get_redis_porto_host(geo='geo1', flavor='alt-test-flavor'),
            'host2': get_redis_porto_host(geo='geo2', flavor='alt-test-flavor'),
            'host3': get_redis_porto_host(geo='geo3', flavor='alt-test-flavor'),
        },
    }
    *_, state = checked_run_task_with_mocks(mocker, 'redis_cluster_create', dict(**args, s3_bucket='test-s3-bucket'))
    ...
    *_, state = checked_run_task_with_mocks(mocker, 'redis_cluster_modify', args, state=state)

    assert (
        state['conductor']['groups']['db_cid_test']['parent_ids'][0]
        == state['conductor']['groups']['mdb_test_redis']['id']
