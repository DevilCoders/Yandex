"""
Kafka zookeeper host create tests
"""

from test.mocks import checked_run_task_with_mocks
from test.tasks.kafka.utils import (
    get_kafka_porto_host,
    get_kafka_compute_host,
    get_zookeeper_porto_host,
    get_zookeeper_compute_host,
)
from test.tasks.utils import (
    check_mlock_usage,
    check_task_interrupt_consistency,
    check_rejected,
)


def test_porto_kafka_zookeeper_host_create_interrupt_consistency(mocker):
    """
    Check porto zookeeper host create interruptions
    """
    args = {
        'hosts': {
            'host1': get_kafka_porto_host(geo='geo1'),
            'host2': get_kafka_porto_host(geo='geo2'),
            'host3': get_zookeeper_porto_host(geo='geo1'),
            'host4': get_zookeeper_porto_host(geo='geo2'),
            'host5': get_zookeeper_porto_host(geo='geo3'),
        },
    }

    *_, state = checked_run_task_with_mocks(mocker, 'kafka_cluster_create', dict(**args, s3_bucket='test-s3-bucket'))

    args['hosts']['host6'] = get_zookeeper_porto_host(geo='geo1')
    args['host'] = 'host6'
    args['zid_added'] = 4

    args['zk_hosts'] = ''

    check_task_interrupt_consistency(
        mocker,
        'kafka_zookeeper_host_create',
        args,
        state,
    )


def test_compute_kafka_zookeeper_host_create_interrupt_consistency(mocker):
    """
    Check compute zookeeper host create interruptions
    """
    args = {
        'hosts': {
            'host1': get_kafka_compute_host(geo='geo1'),
            'host2': get_kafka_compute_host(geo='geo2'),
            'host3': get_zookeeper_compute_host(geo='geo1'),
            'host4': get_zookeeper_compute_host(geo='geo2'),
            'host5': get_zookeeper_compute_host(geo='geo3'),
        },
    }

    *_, state = checked_run_task_with_mocks(mocker, 'kafka_cluster_create', dict(**args, s3_bucket='test-s3-bucket'))

    args['hosts']['host6'] = get_zookeeper_compute_host(geo='geo1')
    args['host'] = 'host6'
    args['zid_added'] = 4

    args['zk_hosts'] = ''

    check_task_interrupt_consistency(
        mocker,
        'kafka_zookeeper_host_create',
        args,
        state,
    )


def test_kafka_zookeeper_host_create_mlock_usage(mocker):
    """
    Check zookeeper host create mlock usage
    """
    args = {
        'hosts': {
            'host1': get_kafka_porto_host(geo='geo1'),
            'host2': get_kafka_porto_host(geo='geo2'),
            'host3': get_zookeeper_porto_host(geo='geo1'),
            'host4': get_zookeeper_porto_host(geo='geo2'),
            'host5': get_zookeeper_porto_host(geo='geo3'),
        },
    }

    *_, state = checked_run_task_with_mocks(mocker, 'kafka_cluster_create', dict(**args, s3_bucket='test-s3-bucket'))

    args['hosts']['host6'] = get_zookeeper_porto_host(geo='geo1')
    args['host'] = 'host6'
    args['zid_added'] = 4

    args['zk_hosts'] = ''

    check_mlock_usage(
        mocker,
        'kafka_zookeeper_host_create',
        args,
        state,
    )


def test_porto_kafka_zookeeper_host_create_revertable(mocker):
    """
    Check porto zookeeper host create revertability
    """
    args = {
        'hosts': {
            'host1': get_kafka_porto_host(geo='geo1'),
            'host2': get_kafka_porto_host(geo='geo2'),
            'host3': get_zookeeper_porto_host(geo='geo1'),
            'host4': get_zookeeper_porto_host(geo='geo2'),
            'host5': get_zookeeper_porto_host(geo='geo3'),
        },
    }

    *_, state = checked_run_task_with_mocks(mocker, 'kafka_cluster_create', dict(**args, s3_bucket='test-s3-bucket'))

    args['hosts']['host6'] = get_zookeeper_porto_host(geo='geo1')
    args['host'] = 'host6'
    args['zid_added'] = 4
    state['fail_actions'].add('deploy-v2-shipment-create-host1-state.sls')

    check_rejected(
        mocker,
        'kafka_zookeeper_host_create',
        args,
        state,
    )


def test_compute_kafka_zookeeper_host_create_revertable(mocker):
    """
    Check compute zookeeper host create revertability
    """
    args = {
        'hosts': {
            'host1': get_kafka_compute_host(geo='geo1'),
            'host2': get_kafka_compute_host(geo='geo2'),
            'host3': get_zookeeper_compute_host(geo='geo1'),
            'host4': get_zookeeper_compute_host(geo='geo2'),
            'host5': get_zookeeper_compute_host(geo='geo3'),
        },
    }

    *_, state = checked_run_task_with_mocks(mocker, 'kafka_cluster_create', dict(**args, s3_bucket='test-s3-bucket'))

    args['hosts']['host6'] = get_zookeeper_compute_host(geo='geo1')
    args['host'] = 'host6'
    args['zid_added'] = 4
    state['fail_actions'].add('deploy-v2-shipment-create-host1-state.sls')

    check_rejected(
        mocker,
        'kafka_zookeeper_host_create',
        args,
        state,
    )
