...
def test_porto_mongodb_cluster_modify_interrupt_consistency(mocker):
    """
...
def test_compute_mongodb_cluster_modify_interrupt_consistency(mocker):
    """
    Check compute modify interruptions
    """
    args = {
        'hosts': {
            'host1': get_mongod_compute_host(geo='geo1', disk_type_id='local-ssd', space_limit=107374182400),
...
def test_mongodb_cluster_modify_mlock_usage(mocker):
    """
...
def test_mongodb_cluster_modify_alert_sync(mocker):
    """
    Check modify mlock usage
    """
    args = {
...
def test_mongodb_cluster_modify_alt_conductor_group(mocker):
    """
    Check that cluster correctly changes alt conductor group on modify
    """
    args = {
        'hosts': {
            'host1': get_mongod_porto_host(geo='geo1', flavor='alt-test-flavor'),
            'host2': get_mongod_porto_host(geo='geo2', flavor='alt-test-flavor'),
            'host3': get_mongod_porto_host(geo='geo3', flavor='alt-test-flavor'),
        },
    }
    *_, state = checked_run_task_with_mocks(mocker, 'mongodb_cluster_create', dict(**args, s3_bucket='test-s3-bucket'))

    assert (
        state['conductor']['groups']['db_subcid_test']['parent_ids'][0]
    ...
    *_, state = checked_run_task_with_mocks(mocker, 'mongodb_cluster_modify', args, state=state)

    assert (
        state['conductor']['groups']['db_subcid_test']['parent_ids'][0]
        == state['conductor']['groups']['mdb_test_mongod']['id']
