"""
Elasticsearch cluster modify tests
"""
...
def test_elasticsearch_cluster_modify_data_only(mocker):
    """
...
def test_elasticsearch_cluster_modify_no_restart(mocker):
    """
    Check successfully modify with data only nodes
    """
    args = {
...
def test_elasticsearch_cluster_modify_not_healthy(mocker):
    """
...
def test_porto_elasticsearch_cluster_modify_interrupt_consistency(mocker):
    """
...
def test_compute_elasticsearch_cluster_modify_interrupt_consistency(mocker):
    """
...
def test_elasticsearch_cluster_modify_mlock_usage(mocker):
    """
...
def test_elasticsearch_cluster_modify_alert_sync(mocker):
    """
    Check modify mlock usage
    """
    args = {
...
def test_elasticsearch_cluster_modify_alt_conductor_group(mocker):
    """
    Check that cluster correctly changes alt conductor group on modify
    """
    args = {
        'hosts': {
            'host1': get_elasticsearch_datanode_porto_host(geo='geo1', flavor='alt-test-flavor', subcid='dn-subcid'),
            'host2': get_elasticsearch_datanode_porto_host(geo='geo2', flavor='alt-test-flavor', subcid='dn-subcid'),
            'host3': get_elasticsearch_masternode_porto_host(geo='geo1', flavor='alt-test-flavor', subcid='mn-subcid'),
            'host4': get_elasticsearch_masternode_porto_host(geo='geo2', flavor='alt-test-flavor', subcid='mn-subcid'),
            'host5': get_elasticsearch_masternode_porto_host(geo='geo3', flavor='alt-test-flavor', subcid='mn-subcid'),
        },
    }
    *_, state = checked_run_task_with_mocks(
        mocker, 'elasticsearch_cluster_create', dict(**args, s3_bucket='test-s3-bucket')
    ...
    *_, state = checked_run_task_with_mocks(mocker, 'elasticsearch_cluster_modify', args, state=state)

    assert (
        state['conductor']['groups']['db_dn_subcid']['parent_ids'][0]
        == state['conductor']['groups']['mdb_test_elasticsearch_data']['id']
    ), 'Unexpected datanode group id after modify'

    assert (
        state['conductor']['groups']['db_mn_subcid']['parent_ids'][0]
        == state['conductor']['groups']['mdb_test_elasticsearch_master']['id']
