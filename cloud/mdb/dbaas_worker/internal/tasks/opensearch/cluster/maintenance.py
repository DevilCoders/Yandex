"""
Opensearch maintenance cluster executor
"""
...
@register_executor('opensearch_cluster_maintenance')
class OpensearchClusterMaintenance(ClusterMaintenanceExecutor, OpensearchExecutorTrait):
    """
    ...
    Currently, there are 3 main ways of using this executor:
        ...
        3. Upgrade cluster's opensearch patch version - provide 'run_highstate', 'restart' flags
        4. Upgrade cluster's openseach minor version - provide 'upgrade_to_major_version' flag
        5. Upgrade cluster's opensearch major version - Todo: Support cluster's major version when needed
    ...
    def _restart_cluster(self, master_group, data_group):
        self._os_cluster_health(master_group, data_group)
