"""
OpenSearch host delete executor.
"""
...
@register_executor('opensearch_host_delete')
class OpensearchHostDelete(HostDeleteExecutor):
    """
    Delete OpenSearch host in dbm or compute.
    """

    UNIQUE_DATA_CHECK = 'os_unique_data'

    def __init__(self, config, task, queue, args):
        super().__init__(config, task, queue, args)
        self.properties = config.opensearch_data
    ...
    def run(self):
        ...
        master_group = build_host_group(self.config.opensearch_master, master_hosts)
        data_group = build_host_group(self.config.opensearch_data, data_hosts)
