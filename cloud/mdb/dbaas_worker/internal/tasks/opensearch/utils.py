"""
Utilities specific to OpenSearch clusters.
"""
...
DATA_ROLE_TYPE = 'opensearch_cluster.datanode'
MASTER_ROLE_TYPE = 'opensearch_cluster.masternode'
...
class OpensearchExecutorTrait:
    """
    OpensearchExecutorTrait contains helper methods useful in different tasks
    """

    def _os_cluster_health(self, master_group, data_group):
        try:
    ...
    def _os_run_highstate(self, group, config):
    ...
    def _os_upgrade_to_major_version(self, master_group, data_group):
        self._os_cluster_health(data_group, master_group)

        # We need to stop dashboards instances so that no 2 different versions are running at the same time
        ...
        self._os_run_highstate(data_group, self.config.opensearch_data)

        for opts in master_group.hosts.values():
            opts['deploy'] = {'pillar': {'service-restart': True}}
        self._os_run_highstate(master_group, self.config.opensearch_master)

        # Don't forget to start the dashboards service
