...
CREATE OR REPLACE FUNCTION code.task_type_action_map()
 RETURNS TABLE(task_type text, action dbaas.action)
 LANGUAGE sql
 IMMUTABLE
AS $function$
SELECT task_type::text,
  ...
  FROM (
    VALUES
        ...
        ('greenplum_cluster_online_resetup', 'cluster-resetup'),
        ('greenplum_cluster_offline_resetup', 'cluster-offline-resetup'),
        ('noop', 'noop')
