...
def _int8range_adapt(value):
    return NumericRange(value[0], value[1], bounds='[]')
...
def main():
    ...
    parser.add_argument('-d', '--dsn', type=str, help='Connection DSN', default='dbname=dbaas_metadb')
    ...
    with conn as txn:
        cursor = txn.cursor()
        populate(cursor, args.table, args.input_file, args.key, args.refill)
