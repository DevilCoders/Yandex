...
def get_cids():
    with closing(psycopg2.connect(database="dbaas_metadb", user="postgres", host="localhost", port=5432)) as conn:
        cur = conn.cursor()
        cur.execute("SELECT cid " "FROM dbaas.clusters " "WHERE dbaas.visible_cluster_status(status)")
        res = cur.fetchall()
...
def change_pillars(cids, box):
    with closing(psycopg2.connect(database="dbaas_metadb", user="postgres", host="localhost", port=5432)) as conn:
        ...
        for cid in cids:
            ...
            for fqdn in fqdns:
                cur.execute(
                    'select value from dbaas.pillar where fqdn=%(fqdn)s and value->\'cert.expiration\' is null;',
                    {'fqdn': fqdn},
                )
                res = cur.fetchone()
                if res is None:
                    continue
                pillar_value = res[0]
                if 'cert.crt' not in pillar_value:
                    continue
