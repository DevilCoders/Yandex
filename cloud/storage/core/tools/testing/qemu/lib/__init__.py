...
def start(argv):
    ...
    virtio_options = _get_virtio_options(virtio)
    enable_kvm = _get_vm_enable_kvm(args)

    if enable_kvm and not qemu.kvm_available():
    ...
    qemu_serial_log = yatest.common.output_path(
        os.path.basename(rootfs) + "_serial.out")
    ...
    cmd = [
        ...
        "-smp", proc,
        "-m", mem,
        "-object", "memory-backend-memfd,id=mem,size={},share=on".format(mem),
        "-numa", "node,memdev=mem",
        "-snapshot",
        "-netdev", "user,id=netdev0,net={},host={},hostfwd=tcp::{}-:{}".format(QEMU_NET, QEMU_HOST, ssh.port, SSH_PORT),
        "-device", "virtio-net-pci,netdev=netdev0,id=net0",
        "-device", "virtio-rng-pci",
        "-serial", "file:{}".format(qemu_serial_log),
        "-nographic",
        "-drive", "format=qcow2,file={},id=hdd0,if=none,aio=native,cache=none".format(rootfs),
        "-device", "virtio-blk-pci,id=vblk0,drive=hdd0,num-queues={},bootindex=1".format(proc),
        "-L", qemu_firmware
    ]
    ...
    for tag, path in _get_mount_paths():
        cmd += ["-virtfs", "local,path={path},mount_tag={tag},security_model=none".format(
            tag=tag, path=path)]
    ...
    qemu_bin = Daemon(
        ...
        timeout=180,
        **daemon_log_files(prefix="qemu-bin", cwd=yatest.common.output_path()))
    qemu_bin.start()

    _wait_ssh(qemu_bin, ssh)
    _prepare_test_environment(ssh, virtio)
    library.python.testing.recipe.set_env(
        "QEMU_KVM_PID", str(qemu_bin.daemon.process.pid))
...
def _get_virtio_options(virtio):
    if virtio == "fs":
        return _get_virtiofs_options()
    elif virtio == "blk":
        return _get_virtioblk_options()
...
def _get_virtiofs_options():
    vhost_socket = os.getenv("NFS_VHOST_SOCKET")
    if vhost_socket is None:
        raise QemuKvmRecipeException("Cannot find nfs vhost socket path")

    cmd = ["-chardev",
           "socket,id=vhost0,path={},reconnect=1".format(vhost_socket)]
    cmd += ["-device", "vhost-user-fs-pci,chardev=vhost0,id=vhost-user-fs0,tag=fs0,queue-size=512"]
    return cmd


def _get_virtioblk_options():
    vhost_socket = os.getenv("NBS_VHOST_SOCKET")
    if vhost_socket is None:
        raise QemuKvmRecipeException("Cannot find nbs vhost socket path")
