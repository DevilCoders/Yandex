...
func createContext(verbose bool) context.Context {
	...
	return logging.SetLogger(
		context.Background(),
		logging.CreateStderrLogger(loggingLevel),
	)
}
...
func getSnapshotInfo(
	...
	storageSize := "0"
	if val, ok := result["storage_size"]; ok {
		storageSize = val.(string)
	}

	return snapshotInfo{
		diskSize:    result["disk_size"].(string),
...
func run(
	...
	ctx := createContext(verbose)
	ycp := createYcpWrapper(profile, folderID)
