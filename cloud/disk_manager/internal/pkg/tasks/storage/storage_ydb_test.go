...
import (
	...
	metrics_mocks "a.yandex-team.ru/cloud/disk_manager/internal/pkg/monitoring/metrics/mocks"
...
func createStorage(
	...
	err := CreateYDBTables(ctx, config, db)
	if err != nil {
		return nil, err
	}

	return CreateStorage(config, metricsRegistry, db)
}
...
func TestStorageYDBCreateTask(t *testing.T) {
	...
	metricsRegistry := metrics_mocks.CreateRegistryMock()

	taskStallingTimeout := "1s"
	storage, err := createStorage(t, ctx, db, &tasks_config.TasksConfig{
...
func TestStorageYDBGetTask(t *testing.T) {
	...
	require.EqualValues(t, MakeStringSet(), taskState.Dependencies)
...
func TestStorageYDBGetTaskWithDependencies(t *testing.T) {
	...
	taskID, err := storage.CreateTask(ctx, TaskState{
		...
		Status:         TaskStatusReadyToRun,
		State:          []byte{1, 2, 3},
		Dependencies:   MakeStringSet(depID1, depID2),
	})
	require.NoError(t, err)
	...
	require.EqualValues(t, []byte{1, 2, 3}, taskState.State)
	require.EqualValues(t, MakeStringSet(depID1, depID2), taskState.Dependencies)
	require.WithinDuration(t, time.Time(createdAt), time.Time(taskState.ChangedStateAt), time.Microsecond)
	metricsRegistry.AssertAllExpectations(t)
}
...
func TestStorageYDBListTasksReadyToRun(t *testing.T) {
	...
	metricsRegistry := metrics.CreateEmptyRegistry()
	...
	_, err = storage.CreateTask(ctx, TaskState{
		...
		Status:         TaskStatusReadyToRun,
		State:          []byte{},
		Dependencies:   MakeStringSet(taskIDFinished, taskIDReadyToRun, taskIDReadyToCancel),
	})
	require.NoError(t, err)
	...
	taskIDReadyToRunWaited, err := storage.CreateTask(ctx, TaskState{
		...
		Status:         TaskStatusReadyToRun,
		State:          []byte{},
		Dependencies:   MakeStringSet(taskIDFinished, taskIDReadyToCancel),
	})
	require.NoError(t, err)
	...
	taskInfos, err := storage.ListTasksReadyToRun(ctx, 100500)
	require.NoError(t, err)
	// TODO: Maybe we need a proper order.
	require.ElementsMatch(t, taskInfos, expectedTaskInfos)
}
...
func TestStorageYDBListTasksReadyToCancel(t *testing.T) {
	...
	taskInfos, err := storage.ListTasksReadyToCancel(ctx, 100500)
	require.NoError(t, err)
	// TODO: Maybe we need a proper order.
	require.ElementsMatch(t, taskInfos, expectedTaskInfos)
}
...
func TestStorageYDBListTasksStallingWhileRunning(t *testing.T) {
	...
	taskInfos, err := storage.ListTasksStallingWhileRunning(ctx, "current_host", 100500)
	require.NoError(t, err)
	// TODO: Maybe we need a proper order.
	require.ElementsMatch(t, taskInfos, expectedTaskInfos)
}
...
func TestStorageYDBListTasksStallingWhileCancelling(t *testing.T) {
	...
	taskInfos, err := storage.ListTasksStallingWhileCancelling(ctx, "current_host", 100500)
	require.NoError(t, err)
	// TODO: Maybe we need a proper order.
	require.ElementsMatch(t, taskInfos, expectedTaskInfos)
}
...
func TestStorageYDBListTasksReadyToRunWithWhitelist(t *testing.T) {
	...
	storage, err := createStorage(t, ctx, db, &tasks_config.TasksConfig{
		TaskStallingTimeout: &taskStallingTimeout,
		TaskTypeWhitelist:   []string{"task2"},
	}, metricsRegistry)
	require.NoError(t, err)
	createTime := time.Now().Add(-time.Hour)
	freshTime := createTime.Add(2 * time.Hour)
...
func TestStorageYDBListTasksReadyToRunWithBlacklist(t *testing.T) {
	ctx, cancelCtx := context.WithCancel(createContext())
	defer cancelCtx()
	...
	storage, err := createStorage(t, ctx, db, &tasks_config.TasksConfig{
		TaskStallingTimeout: &taskStallingTimeout,
		TaskTypeBlacklist:   []string{"task1"},
	}, metricsRegistry)
	require.NoError(t, err)
	createTime := time.Now().Add(-time.Hour)
	freshTime := createTime.Add(2 * time.Hour)
...
func TestStorageYDBListTasksReadyToCancelWithBlacklist(t *testing.T) {
	ctx, cancelCtx := context.WithCancel(createContext())
	defer cancelCtx()
...
func TestStorageYDBListTasksStallingWhileRunningWithBlacklist(t *testing.T) {
	ctx, cancelCtx := context.WithCancel(createContext())
	defer cancelCtx()
...
func TestStorageYDBListTasksStallingWhileCancellingWithBlacklist(t *testing.T) {
	ctx, cancelCtx := context.WithCancel(createContext())
	defer cancelCtx()
...
func TestStorageYDBListTasksReadyToRunInCertainZone(t *testing.T) {
	...
	taskInfos, err := storage.ListTasksReadyToRun(ctx, 100500)
	require.NoError(t, err)
	require.ElementsMatch(t, taskInfos, expectedReadyToRunTaskInfos)
}
...
func TestStorageYDBLockTaskToRunWrongGeneration(t *testing.T) {
	...
	require.Truef(t, errors.Is(err, &errors.WrongGenerationError{}), "Expected WrongGenerationError got %v", err)
	metricsRegistry.AssertAllExpectations(t)
}
...
func TestStorageYDBLockTaskToRunWrongState(t *testing.T) {
	...
	require.Error(t, err)
	require.Truef(t, !errors.Is(err, &errors.WrongGenerationError{}), "Unexpected WrongGenerationError")
	metricsRegistry.AssertAllExpectations(t)
}
...
func TestStorageYDBUpdateTask(t *testing.T) {
	...
	_, err = storage.UpdateTask(ctx, TaskState{
		...
		State:          []byte{1},
		Dependencies:   MakeStringSet(taskIDDependent1, taskIDDependent2),
	})
	require.NoError(t, err)
	metricsRegistry.AssertAllExpectations(t)
...
func TestStorageYDBCheckStallingTimeout(t *testing.T) {
	...
	tasks, err := storage.ListTasksStallingWhileCancelling(ctx, "current_host", 100500)
	require.NoError(t, err)
