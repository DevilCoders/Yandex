...
func createYDB(ctx context.Context) (*YDBClient, error) {
...
func tableV1TableDescription() CreateTableDescription {
	return MakeCreateTableDescription(
...
func TestYDBCreateTableV1(t *testing.T) {
	...
	err = db.CreateOrAlterTable(ctx, folder, table, tableV1TableDescription())
	require.NoError(t, err)
...
func TestYDBMigrateTableV1ToTableV2(t *testing.T) {
	...
	err = db.CreateOrAlterTable(ctx, folder, table, tableV2TableDescription())
	require.NoError(t, err)
...
func TestYDBFailMigrationRemovingColumn(t *testing.T) {
	ctx, cancelCtx := context.WithCancel(createContext())
	defer cancelCtx()
	...
	err = db.CreateOrAlterTable(ctx, folder, table, MakeCreateTableDescription(
		WithColumn("id", ydb_types.Optional(ydb_types.TypeUTF8)),
		WithPrimaryKeyColumn("id"),
	))
	assert.Error(t, err)
