...
func (t *createDiskFromSnapshotTask) Init(
	ctx context.Context,
	request proto.Message,
) error {

	typedRequest, ok := request.(*protos.CreateDiskFromSnapshotRequest)
	if !ok {
...
func (t *createDiskFromSnapshotTask) Save(ctx context.Context) ([]byte, error) {
	return proto.Marshal(t.state)
}

func (t *createDiskFromSnapshotTask) Load(ctx context.Context, state []byte) error {
	t.state = &protos.CreateDiskFromSnapshotTaskState{}
...
func (t *createDiskFromSnapshotTask) Run(
	...
	snapshotMeta, err := t.storage.GetSnapshotMeta(ctx, request.SrcSnapshotId)
	if err != nil {
		return err
	}
	...
	if snapshotMeta != nil && snapshotMeta.UseDataplaneTasks {
		...
		taskID, err = t.transferService.TransferFromSnapshotToDisk(
			headers.SetIncomingIdempotencyKey(ctx, selfTaskID),
...
func (t *createDiskFromSnapshotTask) GetMetadata(
	...
	if len(t.state.TransferTaskId) != 0 {
		...
		transferMetadata, ok := message.(*transfer_protos.TransferFromSnapshotToDiskMetadata)
		if ok {
