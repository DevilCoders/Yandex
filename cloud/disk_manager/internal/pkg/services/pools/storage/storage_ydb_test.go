...
func createStorage(
	...
	return createStorageWithConfig(t, ctx, db, makeDefaultConfig())
}
...
func TestStorageYDBCreateBaseDisksPool(t *testing.T) {
	...
	require.Error(t, err)
	require.True(t, errors.Is(err, &errors.InterruptExecutionError{}))
...
func TestStorageYDBCreatePoolWithImageSize(t *testing.T) {
	...
	storage := createStorageWithConfig(t, ctx, db, config)
