...
func Create(
	...
	logging.Info(ctx, "Creating schema for dataplane snapshot storage in %v", db.AbsolutePath(config.GetStorageFolder()))

	err := db.CreateOrAlterTable(ctx, config.GetStorageFolder(), "snapshots", snapshotStateTableDescription())
	if err != nil {
		return err
	}
	...
	err = db.CreateOrAlterTable(ctx, config.GetStorageFolder(), "chunk_blobs", persistence.MakeCreateTableDescription(
	...
	err = db.CreateOrAlterTable(ctx, config.GetStorageFolder(), "chunk_map", persistence.MakeCreateTableDescription(
		persistence.WithColumn("shard_id", ydb_types.Optional(ydb_types.TypeUint64)),
		persistence.WithColumn("snapshot_id", ydb_types.Optional(ydb_types.TypeUTF8)),
