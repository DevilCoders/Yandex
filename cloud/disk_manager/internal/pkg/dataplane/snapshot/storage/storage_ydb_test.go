...
func getS3Object(f *fixture, chunkID string) s3ChunkObject {
	objBytes, err := f.s3.GetObject(
		f.ctx,
...
func createStorage(
	...
	storage, err := CreateStorage(config, metrics.CreateEmptyRegistry(), db, s3)
	require.NoError(t, err)
...
func createFixture(t *testing.T) *fixture {
	...
	storage := createStorage(t, ctx, config, db, s3)
...
func TestCreateSnapshot(t *testing.T) {
	for _, testCase := range testCases() {
		t.Run(testCase.name, func(t *testing.T) {
			...
			err := f.storage.CreateSnapshot(f.ctx, "snapshot")
			require.NoError(t, err)
			...
			err = f.storage.SnapshotCreated(f.ctx, "snapshot", 0, 0, 0)
			require.NoError(t, err)
...
func TestRewriteDataChunk(t *testing.T) {
	for _, testCase := range testCases() {
		t.Run(testCase.name, func(t *testing.T) {
			f := createFixture(t)
			defer f.teardown()

			chunk1 := makeChunk(0, "data1")
			chunkID1, err := f.storage.WriteChunk(f.ctx, "task1", "snapshot", chunk1, testCase.useS3)
			require.NoError(t, err)

			chunk2 := makeChunk(0, "data2")
			chunkID2, err := f.storage.RewriteChunk(f.ctx, "task2", "snapshot", chunk2, testCase.useS3)
			require.NoError(t, err)

			chunkMapEntries := readChunkMap(f, "snapshot")
			require.Equal(t, sortChunkMapEntries([]chunkMapEntry{
				{makeShardID("snapshot"), "snapshot", 0, chunkID2, testCase.useS3},
			}), chunkMapEntries)

			chunkBlobs := readChunkBlobs(f, testCase.useS3, chunkID1, chunkID2)
			require.Equal(t, sortChunks([]chunkBlob{
				{chunkID2, 1, []byte("data2")},
			}), chunkBlobs)
		})
	}
}

func TestRewriteZeroChunk(t *testing.T) {
	for _, testCase := range testCases() {
		t.Run(testCase.name, func(t *testing.T) {
			f := createFixture(t)
			defer f.teardown()

			chunk1 := makeZeroChunk(0)
			chunkID1, err := f.storage.WriteChunk(f.ctx, "task1", "snapshot", chunk1, testCase.useS3)
			require.NoError(t, err)
...
func TestRewriteShallowCopiedChunk(t *testing.T) {
	for _, testCase := range testCases() {
		t.Run(testCase.name, func(t *testing.T) {
			f := createFixture(t)
			defer f.teardown()
			...
			chunkBlobs := readChunkBlobs(f, testCase.useS3, chunkID1)
			require.Equal(t, sortChunks([]chunkBlob{
				{chunkID1, 2, []byte("data1")},
			}), chunkBlobs)

			chunk2 := makeChunk(0, "data2")
			chunkID2, err := f.storage.RewriteChunk(f.ctx, "", "dst", chunk2, testCase.useS3)
			require.NoError(t, err)

			chunkMapEntries := readChunkMap(f, "dst")
			require.Equal(t, sortChunkMapEntries([]chunkMapEntry{
				{makeShardID("dst"), "dst", 0, chunkID2, testCase.useS3},
			}), chunkMapEntries)

			chunkBlobs = readChunkBlobs(f, testCase.useS3, chunkID1, chunkID2)
			require.Equal(t, sortChunks([]chunkBlob{
				{chunkID1, 1, []byte("data1")},
				{chunkID2, 1, []byte("data2")},
			}), chunkBlobs)
		})
	}
...
func TestCheckSnapshotReady(t *testing.T) {
	for _, testCase := range testCases() {
		t.Run(testCase.name, func(t *testing.T) {
			...
			err = f.storage.SnapshotCreated(f.ctx, "snapshot", diskSize, diskSize, chunkCount)
			require.NoError(t, err)
...
func TestChunkCompression(t *testing.T) {
	for _, testCase := range testCases() {
		t.Run(testCase.name, func(t *testing.T) {
			...
			f.storage = createStorage(t, f.ctx, f.config, f.db, f.s3)
