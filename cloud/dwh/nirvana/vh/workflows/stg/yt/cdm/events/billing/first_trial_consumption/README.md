#### Billing events. First trial consumption

Событие начала первого триального потребления
...
[PROD](https://yt.yandex-team.ru/hahn/navigation?path=//home/cloud-dwh/data/prod/stg/cdm/events/billing/billing_account_first_trial_consumption)
/ [PREPROD](https://yt.yandex-team.ru/hahn/navigation?path=//home/cloud-dwh/data/preprod/stg/cdm/events/billing/billing_account_first_trial_consumption)

* `event_type` - тип события
    * `billing_account_first_trial_consumption` - событие первого триального потребления. Вычисляется как время первого потребления (`record.usage_start`), для которого общая стоимость (`cost`
      + `credit`) равна 0.
