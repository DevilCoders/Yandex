...
$replace_cluster = Re2::Replace("^yandexcloud_[a-z]*?_");

$all_daily_table_paths = (SELECT * FROM $get_all_daily_tables($raw_folder_daily, $cluster));

$daily_result = (
  SELECT
    $replace_cluster(CAST(cluster as String), '') as cluster,
    CAST(media as String) as media,
    CAST(sensor as String) as sensor,
    CAST(subsystem as String) as subsystem,
    CAST(type as String) as type,
    CAST(value as Double) as value,
    CAST(CAST(`timestamp` as Uint64) / 1000 as Datetime) as `created_timestamp`,
   FROM EACH($all_daily_table_paths)
   WHERE value != "NaN"
);
...
$monthly_result = (
  SELECT
    ...
    CAST(type as String) as type,
    CAST(value as Double) as value,
    CAST(`timestamp` / 1000 as Datetime) as `created_timestamp`,
