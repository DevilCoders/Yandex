...
$result = (
    SELECT
        $toString(if(`field_name` in ('description','lost_reason_description'),$decode_utf8(`after_value_string`),`after_value_string`))                                                    
                                                                                            AS after_value_string,
        $toString(if(`field_name` in ('description','lost_reason_description'),$decode_utf8(`after_value_text`),`after_value_text`))                                                    
                                                                                            AS after_value_text,
        $toString(if(`field_name` in ('description','lost_reason_description'),$decode_utf8(`before_value_string`),`before_value_string`))                                                    
                                                                                            AS before_value_string,
        $toString(if(`field_name` in ('description','lost_reason_description'),$decode_utf8(`before_value_text`),`before_value_text`))                                                    
        ...
        $toString(case `field_name` 
            when 'description' then 'crm_opportunity_description' 
            when 'id' then 'crm_opportunity_id' 
            when 'name' then 'crm_opportunity_name' 
