...
$result = (
    SELECT
        $toString(`acl_team_set_id`)                                                        AS acl_team_set_id,
        ...
        $toString(`description`)                                                            AS crm_opportunity_resource_line_description,
        $toString(`id`)                                                                     AS crm_opportunity_resource_line_id,
        $toString(`modified_user_id`)                                                       AS modified_user_id,
        $toString(`name`)                                                                   AS crm_opportunity_resource_line_name,
        $toString(`opportunity_id`)                                                         AS crm_opportunity_id,
        $toString(`opportunity_resource_id`)                                                AS crm_opportunity_resource_id,
...
SELECT * FROM $result
ORDER BY `crm_opportunity_resource_line_id`;
