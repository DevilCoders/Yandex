...
$result = (
    SELECT
        ...
        $toString(case `field_name` 
            when 'description' then 'crm_contact_description'
            when 'id' then 'crm_contact_id'
        ...
        if(`field_name` in (
            'whatsapp_account','twitter','telegram_account','portal_password', 'primary_address_city','primary_address_country','primary_address_postalcode','primary_address_state','primary_address_street',	'phone_fax','phone_home','phone_mobile','phone_other','phone_work','linkedin','first_name','last_name','facebook','googleplus', 'alt_address_city', 'alt_address_country', 'alt_address_postalcode', 'alt_address_state', 'alt_address_street', 'assistant', 'assistant_phone', 'birthdate'
        ), 1, 0)                                                                AS is_pii
    FROM $src_table
);
