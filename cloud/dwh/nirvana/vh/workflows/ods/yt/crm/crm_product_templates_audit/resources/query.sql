...
$result = (
    SELECT
        ...
        $toString(case `field_name` 
            when 'description' then 'crm_product_template_description' 
            when 'id' then 'crm_product_template_id'
            when 'name' then 'crm_product_template_name' 
            when 'currency_id' then 'crm_currency_id'
            when 'balance_id' then 'crm_balance_id'
            when 'billing_id' then 'crm_billing_id'
            when 'category_id' then 'crm_category_id'
            when 'sku_id' then 'crm_sku_id'
            else `field_name`
            end )                                                                           AS field_name,
        $toString(`id`)                                                                     AS id,
