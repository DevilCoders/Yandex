...
$issue_summons_table = {{ param["issue_summons_cdm_table"] -> quote() }};
...
$issue_summonees = (
    SELECT
        summons.issue_id                                AS issue_id,
        SUM(IF(employees.is_external_summon, 1, 0))     AS external_summons_count,
        SUM(IF(employees.is_second_line_summon, 1, 0))  AS second_line_summons_count,
        SUM(IF(employees.is_support_summon, 1, 0))      AS support_summons_count,
        COUNT(employees.staff_user_id)                  AS summons_count
    FROM $issue_summons_table AS summons
        LEFT JOIN $support_staff_employees AS employees
            ON summons.staff_user_id = employees.staff_user_id
    GROUP BY summons.issue_id
);
...
INSERT INTO $dst_table WITH TRUNCATE
    SELECT
        ...
        IF(issue_summonees.issue_id IS NOT NULL, issue_summonees.summons_count, 0)              AS summons_total_cnt,
        IF(issue_summonees.issue_id IS NOT NULL, issue_summonees.second_line_summons_count, 0)  AS summons_second_line_cnt,
        IF(issue_summonees.issue_id IS NOT NULL, issue_summonees.support_summons_count, 0)      AS summons_support_cnt,
        IF(issue_summonees.issue_id IS NOT NULL, issue_summonees.external_summons_count, 0)     AS summons_external_cnt,
    ...
    FROM $issues_table AS issues
        ...
        LEFT JOIN $issue_summonees AS issue_summonees
            ON issues.issue_id = issue_summonees.issue_id
