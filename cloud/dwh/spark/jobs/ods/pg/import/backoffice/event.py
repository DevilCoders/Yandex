...
def main():
    ...
    with spark_session() as spark:
        logger.info("Started.")
        folder = "{}/backoffice/events".format(YT_RAW_LAYER_PATH)
        yt_table = get_latest_yt_table(folder)
        df = spark.read.yt("{}/backoffice/events/{}".format(YT_RAW_LAYER_PATH, yt_table))

        df = df.withColumn("date", to_timestamp("date")) \
            .withColumn("created_at", to_timestamp("created_at")) \
            .withColumn("updated_at", to_timestamp("updated_at")) \
            .withColumn("start_registration_time", to_timestamp("start_registration_time"))
        ...
        df.coalesce(1).write.option("truncate", "true").jdbc(url=url, table="ods.bcf_event",
