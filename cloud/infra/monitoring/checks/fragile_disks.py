...
class DiskOwners:
    MDB_OWNER = "mdb"
    NBS_OWNER = "nbs"
    DEDICATED_OWNER = "dedicated"
    MDB_DEDICATED_OWNERS = [MDB_OWNER, DEDICATED_OWNER]
    MDB_NBS_OWNERS = [MDB_OWNER, NBS_OWNER]
    ALL_OWNERS = [MDB_OWNER, NBS_OWNER, DEDICATED_OWNER]


class Disks:
    NVME_PIECES_NUMBER = 15
    PARTLABEL_PATH = "/dev/disk/by-partlabel"
    NBS_PARTLABEL_PREFIX = "NVMENBS"
...
def check_in_compute_node_config(disks, owner):
    ...
    if (disks == compute_node_disks and owner in DiskOwners.MDB_DEDICATED_OWNERS) or \
        (not compute_node_disks and owner == DiskOwners.NBS_OWNER):
        return True, None
...
def check_symlink_for_nbs_disks():
    ...
    for partition_number in range(1, Disks.NVME_PIECES_NUMBER + 1):
        partition_label = "{}{:02d}".format(Disks.NBS_PARTLABEL_PREFIX, partition_number)
        label_path = os.path.join(Disks.PARTLABEL_PATH, partition_label)
...
def check_fragile_disks(check: JugglerPassiveCheck):
    ...
    isOK, message = check_owner(main_owner, DiskOwners.ALL_OWNERS)
    if not isOK:
        check.crit(message)
    ...
    if not isOK:
        check.crit(message)
    if main_owner == DiskOwners.NBS_OWNER:
        isOK, message = check_symlink_for_nbs_disks()
        if not isOK:
            check.crit(message)
    if main_owner in DiskOwners.MDB_NBS_OWNERS:
