...
class Disk(object):
    ...
    def get_disk_partition(labels: List[str]) -> List[str]:
        partitions = []
        for label in labels:
            full_label_path = "{}/{}".format(DiskLabel.LABEL_PATH, label)
...
class KikimrDisks(object):
    DISK_PARTLABELS_DIR = "/dev/disk/by-partlabel"
    NVME_LABEL = "NVMEKIKIMR"
    HDD_LABEL = "ROTKIKIMR"

    ALL = [NVME_LABEL, HDD_LABEL]
    ...
    def get_kikimr_labeled_partition() -> List[str]:
        kikimr_labels = list(filter(lambda x: x[:-2] in KikimrDisks.ALL, os.listdir(DiskLabel.LABEL_PATH)))
        if not kikimr_labels:
            return []
        return kikimr_labels

    def get_kikimr_labeled_partition_full_path(self) -> List[str]:
        return [os.path.join(DiskLabel.LABEL_PATH, disk) for disk in self.get_kikimr_labeled_partition()]
    ...
    def get_disk_by_partlabel(self, partlabel) -> str:
        return os.path.basename(os.readlink(os.path.join(self.DISK_PARTLABELS_DIR, partlabel)))
...
Run 'sudo -u hw-watcher /usr/sbin/hw_watcher disk run' to restore it.")
        ...
        for partlabel in os.listdir(self.DISK_PARTLABELS_DIR):
            ...
            if partlabel not in expected_labels:
                if partlabel.startswith(self.NVME_LABEL) or partlabel.startswith(self.HDD_LABEL):
                    check.warn("Found unknown KIKIMR partlabel '{}'".format(partlabel))
        for partlabel in expected_labels:
            if partlabel in existing_labels_on_disks and \
                    existing_labels_on_disks[partlabel] != expected_labels[partlabel]:
