...
def command_exec(check: JugglerPassiveCheck, command: str) -> str:
    """Exec shell command through python"""
    run = Popen(shlex.split(command), stdout=PIPE, stderr=PIPE)
    stdout, stderr = run.communicate()
    if run.returncode != 0:
        check.crit("Command: {}. ret_code: {}. STDERR: {}".format(command, run.returncode, stderr))
...
class NtpCheck(JugglerPassiveCheck):
    ...
    def _read_ntp_stats(check):
        command = "ntpq -p"
        ntp_info = command_exec(check, command).strip().split("\n")
        return ntp_info
