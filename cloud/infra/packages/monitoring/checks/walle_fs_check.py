...
class Checker(object):
    ...
    def run_blkid(pattern):
        ...
        _part = None
        stdout = None
        _command = "blkid -U {}".format(pattern)
        try:
            returncode, stdout, stderr = Popen().exec_command(_command)
            if returncode != 0:
                raise Exception("Command: {}. ret_code: {}. STDERR: {}".format(_command, returncode, stderr))
        except Exception as e:
            log.debug(e)
        if stdout:
            _part_full = stdout.strip()
