...
#  }
  ...
  instance_template {
    name     = "${var.indexer_prefix}-{instance.internal_dc}-{instance.index_in_zone}"
    hostname = "${var.indexer_prefix}-{instance.internal_dc}-{instance.index_in_zone}"
    fqdn     = "${var.indexer_prefix}-{instance.internal_dc}-{instance.index_in_zone}.${var.hostname_suffix}"
    ...
    network_interface {
      ...
      primary_v6_address {
        ...
        name    = "ipv6"
        dns_record_spec {
          fqdn        = "${var.indexer_prefix}-{instance.internal_dc}-{instance.index_in_zone}"
...
#      host_group = var.host_group
      placement_group_id = var.yc-search-indexer-pg
    }
...
#      })
      ...
      application-pod = templatefile("${path.module}/files/manifests/indexer/podmanifest.tpl.yaml", {
        application_version    = var.indexer_docker_version
        cloud_zone    = "{instance.internal_dc}"
        cloud_index = "{instance.index_in_zone}"
        prefix = var.indexer_prefix
      })
      logrotate-pod = templatefile("${path.module}/files/manifests/logrotate/logrotate_manifest.tpl.yaml", {
        logrotate_version    = var.indexer_logrotate_docker_version
        prefix = var.indexer_prefix
      })
