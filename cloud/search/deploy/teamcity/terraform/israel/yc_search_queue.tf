resource "ycp_microcosm_instance_group_instance_group" "yc_search_queue" {
#  lifecycle {
#    prevent_destroy = true
#  }

  name               = var.queue_prefix
  ...
  health_checks_spec {
    health_check_spec {
      http_options {
        port = var.healthcheck_queue_port
  ...
  instance_template {
    name     = "${var.queue_prefix}-{instance.internal_dc}-{instance.index_in_zone}"
    hostname = "${var.queue_prefix}-{instance.internal_dc}-{instance.index_in_zone}"
    fqdn     = "${var.queue_prefix}-{instance.internal_dc}-{instance.index_in_zone}.${var.hostname_suffix}"
    ...
    network_interface {
      ...
      primary_v6_address {
        ...
        name    = "ipv6"
        dns_record_spec {
          fqdn        = "${var.queue_prefix}-{instance.internal_dc}-{instance.index_in_zone}"
...
#      host_group = var.host_group
      placement_group_id = var.yc-search-queue-pg
    }
...
#      })
      ...
      application-pod = templatefile("${path.module}/files/manifests/queue/podmanifest.tpl.yaml", {
        application_version    = var.queue_docker_version
        cloud_zone    = "{instance.internal_dc}"
        cloud_index = "{instance.index_in_zone}"
        suffix = var.hostname_suffix
        prefix = var.queue_prefix
      })
      logrotate-pod = templatefile("${path.module}/files/manifests/logrotate/logrotate_manifest.tpl.yaml", {
        logrotate_version    = var.queue_logrotate_docker_version
        prefix = var.queue_prefix
      })
