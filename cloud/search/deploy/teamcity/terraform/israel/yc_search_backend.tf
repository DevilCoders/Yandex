resource "ycp_microcosm_instance_group_instance_group" "yc_search_backend" {
#  lifecycle {
#    prevent_destroy = true
#  }

  name               = var.backend_prefix
  ...
  instance_template {
    name     = "${var.backend_prefix}-{instance.internal_dc}-{instance.index_in_zone}"
    hostname = "${var.backend_prefix}-{instance.internal_dc}-{instance.index_in_zone}"
    fqdn     = "${var.backend_prefix}-{instance.internal_dc}-{instance.index_in_zone}.${var.hostname_suffix}"
    ...
    network_interface {
      ...
      primary_v6_address {
        ...
        name    = "ipv6"
        dns_record_spec {
          fqdn        = "${var.backend_prefix}-{instance.internal_dc}-{instance.index_in_zone}"
...
#      })
      ...
      application-pod = templatefile("${path.module}/files/manifests/backend/podmanifest.tpl.yaml", {
        application_version    = var.backend_docker_version
      ...
      logrotate-pod = templatefile("${path.module}/files/manifests/logrotate/logrotate_manifest.tpl.yaml", {
        logrotate_version    = var.backend_logrotate_docker_version
        prefix = var.backend_prefix
      })
