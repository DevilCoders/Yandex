resource "ycp_microcosm_instance_group_instance_group" "yc_search_queue" {
  ...
  instance_template {
    name               = join("", ["ycsearch-queue", var.instance_name_suffix])
    fqdn               = join("", ["ycsearch-queue", var.instance_name_suffix, var.domain])
    service_account_id = var.service_account_id
    platform_id        = var.instance_platform

    boot_disk {
      disk_spec {
        image_id = local.services.yc_search_queue.image_id
        type_id  = "network-hdd"
        size     = 10
      }
    }
    ...
    metadata = {
      ...
      "user-data"          = module.metadata["yc_search_queue"].metadata
    }
