module "iam_service_backend_groups_preprod" {
  source = "../modules/backend_group/v1"

  for_each = local.iam_service_settings_preprod

  is_http               = each.value.is_http
  name                  = format("%s-backend-preprod", each.value.name)
  backend_name          = lookup(each.value, "backend_name", null)
  description           = format("%s-backend at preprod", each.value.name)
  ...
  # sni = "oauth.private-api.cloud-preprod.yandex.net"
...
resource "ycp_platform_alb_backend_group" iam_cp_server_backend {
  ...
  grpc {
    backend {
      ...
      target {
        endpoint {
          hostname = "iam.private-api.cloud-preprod.yandex.net"
          port     = 4283
        }
      }

      tls {
        sni = "iam.private-api.cloud-preprod.yandex.net"
      }
    }
  }
}
...
resource "ycp_platform_alb_backend_group" rm_cp_server_backend {
  ...
  grpc {
    backend {
      ...
      target {
        endpoint {
          hostname = "rm.private-api.cloud-preprod.yandex.net"
          port     = 4284
        }
      }

      tls {
        sni = "rm.private-api.cloud-preprod.yandex.net"
      }
    }
  }
}
...
resource "ycp_platform_alb_backend_group" ts_server_backend {
  ...
  grpc {
    backend {
      ...
      target {
        endpoint {
          hostname = "ts.private-api.cloud-preprod.yandex.net"
          port = 4282
        }
      }

      tls {
        sni = "ts.private-api.cloud-preprod.yandex.net"
      }
    }
  }
}

module "iam_service_backend_groups_testing" {
  source = "../modules/backend_group/v1"

  for_each = local.iam_service_settings_testing

  is_http               = each.value.is_http
  name                  = format("%s-backend-testing", each.value.name)
  backend_name          = lookup(each.value, "backend_name", null)
  description           = format("%s-backend at testing", each.value.name)
  backend_port          = each.value.backend_port
  healthcheck_port      = each.value.healthcheck_port
  http_healthcheck_path = each.value.http_healthcheck_path
  healthcheck_interval  = "2s"
  ...
  # sni = "oauth.private-api.cloud-testing.yandex.net"
