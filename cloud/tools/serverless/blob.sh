...
usage() {
  echo "$(basename ${0}) -a|--action {copy|generate} -s|--setup {preprod|prod} -r|--release RELEASE_VERSION [-d|--debug]"
  exit -1
}
...
generate() {
  ...
  ${debug} && echo "will generate runtime config for ${setup} stand with ${release} release"
  ...
  for blob_info in \
    ...
    blob=$(echo -n ${blob_info} | awk '{print $4}')
    size=$(echo -n ${blob_info} | awk '{print $3}')
    echo "${blob} = {"
    echo "  key  = \"${blob}\""
    echo "  size = ${size}"
    echo "}"
