...
from cloud.marketplace.functests.yc_marketplace_functests.utils import _create_simple_product


def _create_simple_product_id(*args, **kwargs) -> ResourceIdType:
    return _create_simple_product(*args, **kwargs).metadata.simple_product_id


def test_get_product_error(marketplace_client: MarketplaceClient):
    with pytest.raises(ApiError) as api_err:
        marketplace_client.get_simple_product(PRODUCT_ID)
...
def test_product_create(marketplace_client: MarketplaceClient):
    ...
    create_operation = _create_simple_product(marketplace_client, name)
...
def test_get_non_public_product(marketplace_client: MarketplaceClient, gauthling_context):
    ...
    with pytest.raises(ApiError), gauthling_context(auth=True, authz=False):
        marketplace_client.get_simple_product(create_operation.metadata.simple_product_id)
    #  non-public product should not be accessible via public get endpoint
    with pytest.raises(ApiError):
        marketplace_client.get_public_simple_product(create_operation.metadata.simple_product_id)
...
def test_get_product_slug(marketplace_client: MarketplaceClient,
    ...
    vendor = "Мазай"
    public_product = _create_simple_product(marketplace_client, name, vendor=vendor, meta={}, slug=name)
    product_id = public_product.metadata.simple_product_id
    marketplace_private_client.publish_simple_product(product_id)
    pp = marketplace_client.get_public_simple_product(product_id)
    assert pp.status in SimpleProduct.Status.PUBLIC
    assert pp.vendor == vendor
    assert name == pp.slugs[0].slug
    pp2 = marketplace_client.get_public_simple_product(name)
    assert pp2.id == pp.id
    marketplace_private_client.add_slug("test_slug2", pp2.id, product_type=ProductType.SIMPLE.upper())
    pp3 = marketplace_client.get_public_simple_product("test_slug2")
    assert pp3.id == pp.id
    ...
    pp4 = marketplace_client.get_public_simple_product(name)
    assert pp4.id == pp.id
...
def test_get_product_slug_update(marketplace_client: MarketplaceClient,
    ...
    assert pp.vendor == vendor
    assert name == pp.slugs[0].slug
    marketplace_client.update_simple_product(
        product_id=pp.id,
        slug=name,
    )
...
def test_get_public_product(marketplace_client: MarketplaceClient,
    ...
    public_product = _create_simple_product(marketplace_client, name, vendor=vendor, meta={
        meta_key: {
            "en": meta_value,
            "ru": meta_value,
        },
...
def test_own_product_list(marketplace_client: MarketplaceClient, generate_id,
    ...
    category = marketplace_private_client.create_category({"en": "test cat {}".format(timestamp())},
                                                          ...
                                                          0).response
    first_product_id = _create_simple_product_id(marketplace_client, "product-product", category_ids=[category.id])
    marketplace_private_client.publish_simple_product(first_product_id)
    second_product_id = _create_simple_product_id(marketplace_client, "product-product", category_ids=[category.id])
    filter_query = "categoryId='{}'".format(category.id)
    simple_products = marketplace_client.list_simple_products(BA_ID, filter_query=filter_query).products
    assert len(simple_products) == 2
    assert sorted(x.id for x in simple_products) == sorted([first_product_id, second_product_id])
    assert [x.status for x in simple_products if x.status == SimpleProduct.Status.ACTIVE]
    assert [x.status for x in simple_products if x.status == SimpleProduct.Status.CREATING]
...
def test_public_product_list(marketplace_client: MarketplaceClient, generate_id,
    ...
    category = marketplace_private_client.create_category({"en": "test cat {}".format(timestamp())},
                                                          ...
                                                          0).response
    first_product_id = _create_simple_product_id(marketplace_client, "product-product", category_ids=[category.id])
    _create_simple_product_id(marketplace_client, "product-product", category_ids=[category.id])
    marketplace_private_client.publish_simple_product(first_product_id)
    list_result = marketplace_client.list_public_simple_products(filter_query="categoryId='{}'".format(category.id))
    assert len(list_result.products) == 1
    assert list_result.products[0].id == first_product_id
    assert list_result.products[0].status == SimpleProduct.Status.ACTIVE
...
def test_product_update(marketplace_client: MarketplaceClient, generate_id,
    ...
    category = marketplace_private_client.create_category({"en": "test cat {}".format(timestamp())},
                                                          ...
                                                          0).response
    _create_simple_product_id(marketplace_client, "product-product", category_ids=[category.id])
    list_result = marketplace_client.list_simple_products(BA_ID)
    ...
    update_operation = marketplace_client.update_simple_product(first_product.id, new_name,
                                                                short_description=new_short_description,
                                                                description=new_description)
    assert update_operation.metadata.simple_product_id == first_product.id
...
def test_product_listing_by_non_unique_field(marketplace_client: MarketplaceClient, generate_id,
    ...
    for i in range(n):
        ...
        create_op = _create_simple_product(marketplace_client, name, [category.id],
                                           slug="test_product_listing_by_non_unique_field_{}".format(i))
        pid = create_op.metadata.simple_product_id
        marketplace_private_client.publish_simple_product(pid)
        ids.append((name, pid))
    # Act
    query = "categoryId='{}'".format(category.id)
    products = marketplace_client.list_public_simple_products(3, filter_query=query, order_by="name")
    listed_ids = [p.id for p in products.products]
    next_page_token = products.next_page_token
    while next_page_token:
        more_products = marketplace_client.list_public_simple_products(3,
...
def test_put_product_on_place_in_category(marketplace_client: MarketplaceClient, generate_id,
    ...
    for i in range(n):
        ...
        ids[c] = pid
    # Act
    marketplace_private_client.put_simple_product_on_place(ids["A"], category.id, 2)
    marketplace_private_client.put_simple_product_on_place(ids["C"], category.id, 2)
    marketplace_private_client.put_simple_product_on_place(ids["F"], category.id, 2)
    marketplace_private_client.put_simple_product_on_place(ids["H"], category.id, 2)
    query = "categoryId='{}'".format(category.id)
    public_products = marketplace_client.list_public_simple_products(10, filter_query=query).products
...
def test_product_update_does_not_affect_ordering(marketplace_client: MarketplaceClient, generate_id,
                                                 marketplace_private_client: MarketplacePrivateClient,
                                                 default_simple_product_category):
    ...
    for i in range(n):
        c = chr(ord("A") + i)
        name = "Product {}".format(c)
        create_op = _create_simple_product(marketplace_client, name, [category_a.id, category_b.id])
        pid = create_op.metadata.simple_product_id
        marketplace_private_client.publish_simple_product(pid)
        ids[c] = pid
    marketplace_private_client.put_simple_product_on_place(ids["D"], category_a.id, 0)
    new_d_name = "Updated D"
    marketplace_client.update_simple_product(ids["D"], new_d_name, category_ids=[category_a.id])
    updated_d = marketplace_client.get_simple_product(ids["D"])
    assert updated_d.name == new_d_name
    assert sorted(updated_d.category_ids) == sorted([category_a.id, default_simple_product_category])
    query = "categoryId='{}'".format(category_a.id)
    category_a_list = marketplace_client.list_public_simple_products(filter_query=query)
