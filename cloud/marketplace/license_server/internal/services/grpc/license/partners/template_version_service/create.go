...
func (s *templateVersionService) Create(ctx context.Context, request *partners.CreateTemplateVersionRequest) (*operation.Operation, error) {
	...
	result, opErr := templateversions.NewCreateAction(s.Env).Do(ctx, templateversions.CreateParams{
	...
	var meta *partners.CreateTemplateVersionMetadata
	var res *license.TemplateVersion
	if opErr == nil {
		meta = &partners.CreateTemplateVersionMetadata{TemplateVersionId: result.TemplateVersion.ID}
