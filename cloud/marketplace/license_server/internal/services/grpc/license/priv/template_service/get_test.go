...
func (suite *GetGrpcTestSuite) TestGetOk() {
	...
	ltRes, err := suite.service.Get(context.Background(), &priv.GetTemplateRequest{Id: lt.ID})
...
func (suite *GetGrpcTestSuite) TestGetError() {
	ltRes, err := suite.service.Get(context.Background(), &priv.GetTemplateRequest{Id: "NO_ID"})
