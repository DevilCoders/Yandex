...
func (ltvgp *GetParams) validate() error {
...
func (a *GetAction) checkParams(ctx context.Context, params GetParams) error {
	err := params.validate()
	if err != nil {
		ctxtools.Logger(ctx).Debug("license template version get params did not pass validation")
		return err
	}
	ctxtools.Logger(ctx).Debug("license template version get params passed validation")
	return nil
}
