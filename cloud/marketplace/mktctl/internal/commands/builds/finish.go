...
func init() {
	finish.Flags().StringVar(&buildID, "build-id", "", "")
	_ = finish.MarkFlagRequired("build-id")

	buildsCmd.AddCommand(finish)
}
...
var finish = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		resp, err := mkt.NewRequest().
			...
			Get("/builds/{build_id}:finish")
		if err != nil {
			logger.FatalCtx(ctx, "finish", zap.Error(err))
		}
