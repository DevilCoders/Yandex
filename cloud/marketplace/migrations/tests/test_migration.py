...
@pytest.mark.parametrize(
    ["numbers", "has_class"],
    [
        (["0000", "0001", "0000"], [False, True, True]),
        (["0000"], [False]),
    ],
)
def test_get_disk_migrations_raise(mocker, numbers, has_class):
    disk_migrations = MagicMock()
    disk_migrations.__file__ = None
    disk_migrations.__package__ = "yc_marketplace_migrations.example"

    os_mock = mocker.patch("yc_marketplace_migrations.migration.os")
    os_mock.listdir.side_effect = numbers
...
@pytest.mark.parametrize(
    ...
    [
        ([None, "0001"], {"0001": "test"}),
        (["0001", "0002", "0003"], {"0001": "test", "0002": "test", "0003": "test"}),
    ],
)
def test_get_disk_migrations(mocker, numbers, expected_result):
    result = MigrationsPropagate(log, None, query_get, query_set, table_name).disk_migrations
    assert result == expected_result
