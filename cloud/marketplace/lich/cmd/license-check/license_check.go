...
import (
	...
	"a.yandex-team.ru/cloud/marketplace/lich/internal/app"
)

const initTimeout = 60 * time.Second

var licenseCheckServiceCmd = &cobra.Command{
	Use:   "license-check",
	Short: "Run License Check miniservice",
	Run:   runLicenseCheckService,
}

var appConfigsPath []string

func init() {
	licenseCheckServiceCmd.PersistentFlags().StringArrayVarP(
		&appConfigsPath, "config", "c", nil, "service config files paths",
	)

	if err := licenseCheckServiceCmd.MarkPersistentFlagRequired("config"); err != nil {
		log.Fatal(err)
	}
}

func runLicenseCheckService(_ *cobra.Command, _ []string) {
