...
class EulaCreateRequest(MktBasePublicModel):
    pass


class Eula(AbstractMktBase):
    @property
    ...
    data_model = DataModel((
        Table(name="eulas", spec=KikimrTableSpec(
            columns={
    ...
    def to_public(self) -> "EulaResponse":
        return EulaResponse.new(id=self.id, uri=self.meta["url"])


class EulaResponse(MktBasePublicModel):
    id = ResourceIdType(required=True)
