...
from cloud.marketplace.common.yc_marketplace_common.db.models import simple_products_table
...
from cloud.marketplace.common.yc_marketplace_common.models.simple_product import SimpleProduct as SimpleProductScheme
...
from cloud.marketplace.common.yc_marketplace_common.models.task import SimpleProductCreateOrUpdateParams
from cloud.marketplace.common.yc_marketplace_common.utils.errors import SimpleProductIdError
...
class SimpleProduct:
    @staticmethod
    @mkt_transaction()
    def get(product_id: ResourceIdType, *, tx) -> SimpleProductScheme:
        product = tx.with_table_scope(simple_products_table).select_one(
            "SELECT " + SimpleProductScheme.db_fields() + " " +
            "FROM $table "
            "WHERE id = ?", product_id,
            model=SimpleProductScheme)
        return product

    @staticmethod
    @mkt_transaction()
    def rpc_get(product_id: ResourceIdType, *, tx) -> SimpleProductScheme:
        product = SimpleProduct.get(product_id, tx=tx)
        if product is None:
            raise SimpleProductIdError()
        return product

    @staticmethod
    @mkt_transaction()
    def rpc_get_full(slug: Union[ResourceIdType, str], *, tx) -> SimpleProductResponse:
        ...
        product = marketplace_db().with_table_scope(
            products=simple_products_table,
    ...
    def rpc_public_list(cursor: Optional[str],
                        ...
                        *,
                        ...
                        filter_query: Optional[str] = None) -> SimpleProductList:
        start_method_time = time.monotonic()
        where_query, where_args = SimpleProduct._prepare_list_sql_conditions(cursor, filter_query, limit, order_by)
        start_query_time = time.monotonic()
        iterator = tx.with_table_scope(
            product=simple_products_table,
        ...
        product_list = SimpleProduct._enrich_product_list(iterator, limit)
    ...
    def rpc_list(cursor: Optional[str],
        ...
        iterator = marketplace_db().with_table_scope(products=simple_products_table,
                                                     order=ordering_table,
            ...
            *where_args, model=SimpleProductResponse, ignore_unknown=True,
            strip_table_from_columns=ColumnStrippingStrategy.STRIP_AND_MERGE)
        product_list = SimpleProduct._enrich_product_list(iterator, limit)
        return product_list
    ...
    def _enrich_product_list(iterator, limit, *, tx):
        ...
        product_list = SimpleProductList({
            "products": products,
        })
        ...
        for p in product_list.products:
            ...
            p.simple_product_family_ids = None
    ...
    def _prepare_list_sql_conditions(cursor: Optional[ResourceIdType],
        ...
        cursor_product = {}  # type: Union[dict, SimpleProductResponse]
        filter_query_list, filter_args, category_id = parse_filter_with_category(filter_query, SimpleProductScheme)
        mapping = {
        ...
        if cursor:
            product = marketplace_db().with_table_scope(products=simple_products_table,
                ...
                model=SimpleProductResponse, ignore_unknown=True,
                strip_table_from_columns=ColumnStrippingStrategy.STRIP_AND_MERGE)
            if product is None:
                raise SimpleProductIdError()
    ...
    @staticmethod
    @mkt_transaction()
    def rpc_create(request: SimpleProductCreateRequest, *, tx) -> SimpleProductOperation:
        lib.Publisher.check_state(request.billing_account_id)
        meta = request.meta
        request.meta = {}
        product = SimpleProductScheme.from_request(request)
        if request.slug is not None:
            lib.ProductSlug.add_slug(request.slug, product.id, product_type=ProductType.SIMPLE, tx=tx)
        for key in meta:
            product.meta[key] = I18n.set("simple_product.{}.meta.{}".format(product.id, key), meta[key])
        ...
        with marketplace_db().transaction() as tx:
            tx.with_table_scope(simple_products_table).insert_object("INSERT INTO $table", product)
        ...
        return SimpleProduct.task_create(
    ...
    @staticmethod
    @mkt_transaction()
    def rpc_update(request: SimpleProductUpdateRequest, *, tx) -> SimpleProductOperation:
        product = SimpleProduct.rpc_get_full(request.product_id)
        ...
        if product_update:
            ...
            if "meta" in product_update:
                for key in product_update["meta"]:
                    product_update["meta"][key] = I18n.set("simple_product.{}.meta.{}".format(product.id, key),
            ...
            tx.with_table_scope(simple_products_table).update_object("UPDATE $table $set WHERE id = ?",
                                                                     product_update,
                                                                     request.product_id)
        ...
        product_update_task = SimpleProduct.task_update(
            task_group_id=task_group_id,
            product_id=product.id,
    ...
    def rpc_set_logo_uri(product_id: ResourceIdType, uri: str, *, tx):
        ...
        tx.with_table_scope(simple_products_table) \
    ...
    def rpc_publish(product_id: ResourceIdType, *, tx):
        product_update = {
            "status": SimpleProductScheme.Status.ACTIVE,
        }
        tx.with_table_scope(simple_products_table) \
            .update_object("UPDATE $table $set WHERE id = ?",
                           product_update,
    ...
    def task_create(*, task_group_id, product_id, depends, tx):
        return lib.TaskUtils.create(
            ...
            group_id=task_group_id,
            params=SimpleProductCreateOrUpdateParams({"id": product_id}).to_primitive(),
            depends=depends,
            metadata=SimpleProductMetadata({"simple_product_id": product_id}).to_primitive(),
            tx=tx,
        )
    ...
    def task_update(*, task_group_id, product_id, depends, tx):
        return lib.TaskUtils.create(
            operation_type="simple_product_update",
            group_id=task_group_id,
            params=SimpleProductCreateOrUpdateParams({"id": product_id}).to_primitive(),
            depends=depends,
            metadata=SimpleProductMetadata({"simple_product_id": product_id}).to_primitive(),
