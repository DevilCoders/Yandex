...
function Test-NvidiaCUDAPackage {
  ...
  if ($inflight) {
    ...
    #if (Test-Path $smi) { $v = (& $smi -L) -match "Tesla V100" } # for GRID it would be *GRID V100D-*
    #else { $v = $false }
    #"[TEST]::nvidia-smi detects Tesla V100? $v" | Out-InfoMessage

    $deviceQuery = ls -Filter deviceQuery.exe -Path c:\bootstrap -Recurse -ea:SilentlyContinue | select -exp fullname
    $dq = ( (& $deviceQuery) -contains 'Result = PASS' ) -and ( (& $deviceQuery) -notcontains 'Result = FAILED' )
    "[TEST]::Device query test passed? $dq" | Out-InfoMessage

    $bandwidthTest = ls -Filter bandwidthTest.exe -Path c:\bootstrap -Recurse -ea:SilentlyContinue | select -exp fullname
    $bw = ( (& $bandwidthTest) -contains 'Result = PASS' ) -and ( (& $bandwidthTest) -notcontains 'Result = FAILED' )
    "[TEST]::Bandwidth test passed? $bw" | Out-InfoMessage

    # if it failed - we must fail whole build
    # l8r we'll refactor another script that will 'exit 1' for every runned test
