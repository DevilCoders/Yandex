# ToDo: split in multiple libs?

$global:__BOOTSTRAP_DIR__ = "C:\BOOTSTRAP"
$global:__BOOTSTRAP_LIBS_DIR__ = "C:\BOOTSTRAP\WINDOWS-SCRIPTS"
$global:__BOOTSTRAP_APPS_DIR__ = "C:\BOOTSTRAP\APPS"
...
function Clear-WindowsSystemLogs {
  # another odd place
  Get-ChildItem "$ENV:SystemRoot\Logs\*" -File -Recurse | Remove-Item
}

function Clear-EventLogs {
  # theese two 'eventlogs' always crits, l8r need to look at'em
  & wevtutil el | `
    Where-Object { ($_ -ne 'Microsoft-Windows-LiveId/Analytic') -and ($_ -ne 'Microsoft-Windows-LiveId/Operational') } | `
...
function Set-ActivePowerScheme ($PowerPlanScheme) {
  powercfg -setactive $PowerPlanScheme
}
...
function Enable-IcmpFirewallRule {
  Get-NetFirewallRule -Name "vm-monitoring-icmpv4" | `
    Enable-NetFirewallRule
}

function Get-IcmpFirewallRule {
  $e = Get-NetFirewallRule -Name "vm-monitoring-icmpv4" | `
    Select-Object -ExpandProperty Enabled

  # in FW module 'Enabled' property is odd -_-
  return $e -eq 'true'
}
...
function Get-ShutdownWithoutLogonPolicy {
  return (Get-ItemProperty `
    -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" `
    -Name "ShutdownWithoutLogon" `
    -ErrorAction 'SilentlyContinue').shutdownwithoutlogon -eq 1
}

function Enable-ShutdownWithoutLogonPolicy {
  Set-ItemProperty `
    -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" `
    -Name "ShutdownWithoutLogon" `
    -Value 1
...
# ToDo: add cmd invoker to pass stdout & stderr as json's 2 logs
