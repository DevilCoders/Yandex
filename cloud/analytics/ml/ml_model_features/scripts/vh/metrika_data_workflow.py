...
def main(local_src: bool = False,  is_prod: bool = False, with_start: bool = False, rebuild: bool = False) -> None:
    ...
    run_small_job_op(_name='Metrika Raw Hits',
                     max_ram=128, input=package,
                     script='scripts/metrika/metrika_raw_hits.py',
                     script_args=['--is_prod' if is_prod else '', '--rebuild' if rebuild else ''],
                     yql_token='robot-clanalytics-yql',
                     yt_token='robot-clanalytics-yt')

    run_small_job_op(_name='Metrika Raw Visits',
                     max_ram=128, input=package,
                     script='scripts/metrika/metrika_raw_visits.py',
                     script_args=['--is_prod' if is_prod else '', '--rebuild' if rebuild else ''],
                     yql_token='robot-clanalytics-yql',
                     yt_token='robot-clanalytics-yt')

    workflow_id = 'ca425719-c57f-4f5a-9a35-d4529e633cb2' if is_prod else '1b2cc312-2518-4b68-b1e3-94433b4229d2'
    vh.run(wait=False,
           keep_going=True,
           start=with_start,
