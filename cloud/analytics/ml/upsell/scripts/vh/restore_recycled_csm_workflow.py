...
def main(local_src,  is_prod, with_start):
    ...
    deps_path = '//home/cloud_analytics/ml/upsell/csm_restore_leads/deps'
    leads_table = "//home/cloud_analytics/export/crm/update_call_center_leads/update_leads" + prod_postfix
    leads_table_history = "//home/cloud_analytics/ml/upsell/csm_restore_leads/csm_restore_history" + prod_postfix
    script_generate_leads = 'restore_recycled_csm.py'
    ...
    with vh.wait_for(write_src_zip, write_driver):
        spark_op(_name='Restore Recycled (CSM)',
                 ...
                 spyt_driver_args=[f'--leads_table {leads_table}', f'--leads_table_history {leads_table_history}'])

    workflow_id = '9f5e888f-c84c-4bf0-a582-04075b0a03f4' if is_prod else '4f8ed00e-7e4d-4750-8d30-5d3f21400f86'
    vh.run(wait=False,
           keep_going=True,
           start=with_start,
           ...
           label=f'Restore Recycled (CSM) ({"prod" if is_prod else "test"})',
