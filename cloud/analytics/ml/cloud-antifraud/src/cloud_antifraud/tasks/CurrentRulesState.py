...
class CurrentRulesState(luigi.Task):
    ...
    def run(self):
        query = dedent(f'''
                ...
                $result_table = '//home/cloud_analytics/tmp/antifraud/current_rules_state';
                ...
                $rules_structs = (SELECT
                        ...
                        AsStruct({ ','.join([str(i) + " in cloud_rules AS cloud_rule" + str(i) for i in range(12)]) }) as cloud_rules_struct,
                        AsStruct({ ','.join([str(i) + " in created_rules AS created_rule" + str(i) for i in range(8)]) }) as created_rules_struct,
                        AsStruct(0 in suspicious_rules AS suspicious_rule0,
                                 1 in suspicious_rules AS suspicious_rule1) as susp_rules
                FROM  $last_non_empty_table($bad_accounts));
                --endsql
    ...
    def output(self):
        return luigi.LocalTarget(join('data', 'cache', 'sql', 'current_rules_state.sql'))
