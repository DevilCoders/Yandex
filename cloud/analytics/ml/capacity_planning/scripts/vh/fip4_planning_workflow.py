...
def main(local_src: bool,  is_prod: bool, with_start: bool) -> None:
    ...
    deps_path = '//home/cloud_analytics/ml/capacity_planning/fip4/deps'
    ...
    driver_predict = extract_op(_name='Extract driver', archive=package, out_type='exec',
                                 path=f'scripts/fip4/{script_predict_data}').exec_file
    ...
    with vh.wait_for(write_src_zip, write_driver_predict):
        spark_op(_name='Forecast data',
                 ...
                 cluster='adhoc',  # use porto-layer with fresh library versions
                 spyt_driver_filename=script_predict_data,
                 spyt_driver_args=['--is_prod'] if is_prod else [])

    workflow_id = 'a347c60e-aff9-499d-8b4a-42c73bf659a9' if is_prod else 'dab41ae8-ef2c-4882-8c5a-6266ae9b715c'
    vh.run(wait=False,
           keep_going=True,
           start=with_start,
           ...
           label=f'Capacity planning - FIPv4 ({"prod" if is_prod else "test"})',
