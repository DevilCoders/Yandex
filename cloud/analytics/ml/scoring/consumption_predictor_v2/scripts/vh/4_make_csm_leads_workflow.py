...
def main(local_src,  is_prod, with_start):
    ...
    # pathes in yt
    deps_path_yt = '//home/cloud_analytics/ml/scoring/consumption_predictor_v2/deps/wf4'
    add_csm_leads_script = '4_make_csm_leads.py'
    ...
    leads_table_history = "//home/cloud_analytics/ml/scoring/consumption_predictor_v2/crm/upsell/csm_history" + postfix
    ...
    spyt_driver = extract_op(_name='Extract driver script', archive=package, out_type='exec', path=f'scripts/{add_csm_leads_script}').exec_file
    write_spyt_driver = yt_write_file(_name='Driver script -> YT', file=spyt_driver, path=f'{deps_path_yt}/{add_csm_leads_script}')
    ...
    with vh.wait_for(write_src_zip, write_spyt_driver):
        spark_op(_name='Run spyt-driver',
                 ...
                 spyt_driver_filename=add_csm_leads_script,
    ...
    workflow_id = '4434540d-cc64-4a5e-bf1c-aaa3aa4c1f18' if is_prod else 'a4151731-335d-4779-8601-7d088b26e792'
    vh.run(wait=False,
           keep_going=True,
           start=with_start,
           ...
           label=f'Make CSM Leads ({"prod" if is_prod else "test"})',
