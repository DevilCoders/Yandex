...
DEFINE SUBQUERY $first_trial_cons_gov() AS
    DEFINE SUBQUERY $res($param,$period) AS 
        SELECT 
            `date`,
            'first_trial_cons_gov' as metric_id,
            'First Trial Cons Gov' as metric_name,
        ...
        FROM (
            ...
            WHERE 
                crm_account_segment in ('Public sector')
                and is_fraud=0
