/*___DOC___
====Plan-Fact Revenue SMB WoW
Отношение фактической выручки к запланированной оптимистичной для сегменета SMB по неделям
<{Код
{{include href="https://a.yandex-team.ru/api/tree/blob/trunk/arcadia/cloud/analytics/yc-metrics/metrics/SMB/plan_fact_revenue_smb_wow.sql" formatter="yql"}}
}>
___DOC___*/
...
DEFINE SUBQUERY $plan_fact_revenue_smb_wow() AS
    DEFINE SUBQUERY $cons_fact($period) AS
        ...
        WHERE 
            event = 'day_use'
            AND crm_account_segment in ('Medium','Mass')
...
--            AND $format_date(DateTime::StartOfYear($parse_datetime(event_time))) = $format_date(DateTime::StartOfYear(CurrentUtcDatetime()))
    ...
    DEFINE SUBQUERY $res($param,$period) AS
        SELECT 
            per as `date`,
            'plan_fact_revenue_smb_wow' as metric_id,
            'Plan Fact Revenue Running Total SMB WoW' as metric_name,
            'general' as metric_group,
            'general' as metric_subgroup,
            '%' as metric_unit, 
            0 as is_ytd,
            $period as period,
            (real_consumption_vat/plan_optim+0.0) * 100 AS metric_value
...
EXPORT $plan_fact_revenue_smb_wow;