/*___DOC___
====Plan-Fact Revenue SMB YTD
Отношение фактической выручки к запланированной оптимистичной для сегменета SMB с начала календарного года.
<{Код
{{include href="https://a.yandex-team.ru/api/tree/blob/trunk/arcadia/cloud/analytics/yc-metrics/metrics/SMB/plan_fact_revenue_smb.sql" formatter="yql"}}
}>
___DOC___*/
...
DEFINE SUBQUERY $plan_fact_revenue_smb() AS
    ...
    DEFINE SUBQUERY $res($param,$period) AS
        SELECT 
            per as `date`,
            'plan_fact_revenue_smb' as metric_id,
            'Plan Fact Revenue Running Total SMB' as metric_name,
