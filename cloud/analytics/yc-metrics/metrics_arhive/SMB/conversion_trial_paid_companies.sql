/*___DOC___
====Conversion Trial -> Paid (Company)
Средняя конверсия Trial->Paid за последние 10 недель. Выбирается кол-во конверсий Trial -> Paid за неделю среди биллинг-аккаунтов начавших триал не более чем за 10 недель до недели расчета метрики(биллинг-аккаунты, начавшие платное потребление без триала не учитываются). Это кол-во делится на общее кол-во биллинг-аккаунтов начавших триал не болле чем за 10 недель до недели расчета метрики и умножается на 10. Рассчитывается для ba_person_type = 'Company'
<{Код
{{include href="https://a.yandex-team.ru/api/tree/blob/trunk/arcadia/cloud/analytics/yc-metrics/metrics/SMB/conversion_trial_paid_companies.sql" formatter="yql"}}
}>
___DOC___*/
...
DEFINE SUBQUERY $conversion_trial_paid_companies() AS
    $first_paid_cons = (
        SELECT
            billing_account_id,
    ...
    DEFINE SUBQUERY $res($param,$period) AS
        SELECT 
            ...
            'conv_trial_to_paid_company' as metric_id,
            'Conversion Trial -> Paid (Company)' as metric_name,
            'SMB' as metric_group,
    ...
    SELECT * FROM 
    $metric_growth($res, '', '','straight', 'standart', '')

END DEFINE;

EXPORT $conversion_trial_paid_companies;