...
DEFINE SUBQUERY $marketing_leads() AS 
    DEFINE SUBQUERY $res($param,$period) AS
        ...
        FROM (
            SELECT
                --$format_date(DateTime::StartOfWeek(DateTime::FromSeconds(CAST (date_entered AS Uint32)))) as `date`,
                $round_period_format($period, $format_from_second(date_entered)) as `date`,
                lead_source,
                lead_id
            FROM
        ...
        WHERE 
            ...
            and if($period = 'year', `date` <= $round_current_time($period),
