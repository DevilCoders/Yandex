/*___DOC___
====New BAs (Companies) Plan-Fact Week-over-week
Отношение количества биллинг-аккаунтов-юрлиц (%%ba_person_type='company'%%), зарегистрировавшихся за неделю за исключением фрода (%%is_fraud = 0%%) к Плановому значению (segment = 'Mass_C').
<{Код
{{include href="https://a.yandex-team.ru/api/tree/blob/trunk/arcadia/cloud/analytics/yc-metrics/metrics/marketing/new_bas_companies_plan_fact_wow.sql" formatter="yql"}}
}>
___DOC___*/
...
DEFINE SUBQUERY $new_bas_companies_plan_fact_wow() AS
    ...
    DEFINE SUBQUERY $res($param,$period) AS
        SELECT 
            w_date as `date`,
            'new_ba_companies_plan_fact_wow' as metric_id,
            'New BAs (Companies) Plan-Fact Week-over-Week' as metric_name,
...
EXPORT $new_bas_companies_plan_fact_wow;