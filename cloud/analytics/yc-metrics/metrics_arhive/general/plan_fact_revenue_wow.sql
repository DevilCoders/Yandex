/*___DOC___
====Plan-Fact Revenue Week-over-Week
Отношение фактической недельной выручки к запланированной оптимистичной недельной выручки по неделям
<{Код
{{include href="https://a.yandex-team.ru/api/tree/blob/trunk/arcadia/cloud/analytics/yc-metrics/metrics/general/plan_fact_revenue_wow.sql" formatter="yql"}}
}>
___DOC___*/
...
DEFINE SUBQUERY $plan_fact_revenue_wow() AS
    ...
    DEFINE SUBQUERY $res($param,$period) AS
        SELECT 
            per as `date`,
            'plan_fact_revenue_wow' as metric_id,
            'Plan Fact Revenue Running Total WoW' as metric_name,
