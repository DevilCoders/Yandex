...
Отношение фактической выручки к запланированной оптимистичной с начала календарного года.
<{Код
...
DEFINE SUBQUERY $plan_fact_revenue() AS
...
--            AND $format_date(DateTime::StartOfYear($parse_datetime(event_time))) = $format_date(DateTime::StartOfYear(CurrentUtcDatetime()))
    ...
    DEFINE SUBQUERY $res($param,$period) AS
        SELECT 
            per as `date`,
            'plan_fact_revenue' as metric_id,
            'Plan Fact Revenue Running Total' as metric_name,
