/*___DOC___
====SMB Revenue
Выручка сегмента SMB по неделям
<{Код
{{include href="https://a.yandex-team.ru/api/tree/blob/trunk/arcadia/cloud/analytics/yc-metrics/metrics/general/smb_revenue.sql" formatter="yql"}}
}>
___DOC___*/
...
DEFINE SUBQUERY $smb_revenue() AS
    DEFINE SUBQUERY $res($param,$period) AS 
        SELECT 
            `date`,
            'smb_revenue' as metric_id,
            'SMB Revenue' as metric_name,
