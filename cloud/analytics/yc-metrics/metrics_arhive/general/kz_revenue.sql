/*___DOC___
====KZ Revenue
Выручка по Казахстану в рублях по неделям.
<{Код
{{include href="https://a.yandex-team.ru/api/tree/blob/trunk/arcadia/cloud/analytics/yc-metrics/metrics/general/kz_revenue.sql" formatter="yql"}}
}>
___DOC___*/
...
DEFINE SUBQUERY $kz_revenue() AS
    DEFINE SUBQUERY $res($param,$period) AS 
        SELECT 
            `date`,
            'kz_revenue' as metric_id,
            'KZ Revenue' as metric_name,
