/*___DOC___
====Plan-Fact Revenue PoP
Отношение фактической выручки к запланированной на выбранном периоде.
В качестве плана на H1 на данный момент берется факт H1. 
В качестве плана на H2 берется обновленный план на H2

<{Код
{{include href="https://a.yandex-team.ru/api/tree/blob/trunk/arcadia/cloud/analytics/yc-metrics/metrics/general/plan_fact_revenue_pop.sql" formatter="yql"}}
}>
___DOC___*/
...
DEFINE SUBQUERY $plan_fact_revenue_pop() AS
    DEFINE SUBQUERY $cons_fact($params) AS 
        ...
        WHERE 
            CASE
                ...
                WHEN $params['segment'] = 'Public_sector' THEN crm_segment = 'Public sector'
                ELSE True
            END 
    ...
    DEFINE SUBQUERY $agg_fact($params) AS 
        SELECT 
            per,
            sum(billing_record_real_consumption_rub_vat) as billing_record_real_consumption_rub_vat
        FROM 
            $cons_fact($params)
        GROUP BY 
            per
    ...
    DEFINE SUBQUERY $cons_plan($params) AS 
        ...
        WHERE 
            CASE
                ...
                WHEN $params['segment'] = 'Public_sector' THEN segment = 'Public sector'
                ELSE True
            END 
            and plan_type = 'basic'
        UNION ALL
    ...
    DEFINE SUBQUERY $res($params) AS
        SELECT 
            ...
            (billing_record_real_consumption_rub_vat/plan+0.0) * 100 AS metric_value
