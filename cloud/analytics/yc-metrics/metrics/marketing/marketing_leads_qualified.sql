...
Количество лидов CRM с %%lead_source like 'mkt%'%% созданных за указанный период, для которых статус перешел в Converted.
<{Код
...
DEFINE SUBQUERY $marketing_leads_qualified() AS 
    ...
    DEFINE SUBQUERY $get_id($params) AS 
        SELECT 
            id,
            $round_period_format($params, $format_from_second(date_created)) as `date`
        FROM
