...
DEFINE SUBQUERY $new_bas_paid_companies_plan_fact() AS
    DEFINE SUBQUERY $cons_fact($params) AS
        ...
        JOIN `//home/cloud-dwh/data/prod/cdm/dm_events` as events
        ON events.billing_account_id = hist.billing_account_id
        ...
        WHERE 
            ...
            and events.event_type = 'billing_account_first_paid_consumption'
            and action = 'create'
            and (
                hist.crm_segment = 'Medium' OR (
                    hist.crm_segment in ('Mass') 
                    and billing_account_type in ('company') 
                )
                AND yandex_office in  ('russia','kazakhstan')
            )
    ...
    DEFINE SUBQUERY $plan($params) AS
        SELECT 
            $round_period_date($params['period'],d_date) as plan_date,
            plan
        FROM `//home/cloud_analytics/analytics/prod/plans/2022/plan_new_paid_ba_v01` as p   
    END DEFINE;
...
EXPORT $new_bas_paid_companies_plan_fact;