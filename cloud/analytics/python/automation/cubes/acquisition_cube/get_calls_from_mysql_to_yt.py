#!/usr/bin/env python2
# -*- coding: utf-8 -*-
import os, sys, pandas as pd, vh,click,pymysql,datetime
...
@vh.lazy.hardware_params(vh.HardwareParams(max_ram=1500))
@vh.lazy(
    object,
    mysql_token=vh.Secret,
    yt_token=vh.Secret,
)
def calls_from_mysql_to_yt(mysql_token, yt_token):
    ...
    cluster = clusters.yt.Hahn(
        token = yt_token.value,
        pool = 'mobile-research',
    ...
    print(mysql_token.value)
    cnx = pymysql.connect(user='readonly', password=mysql_token.value,
    ...
    LEFT JOIN  email_addr_bean_rel l2_1 ON l1.id=l2_1.bean_id AND l2_1.deleted=0 AND l2_1.bean_module = 'Leads' AND l2_1.primary_address = 1
...
def main(local, project, option):
    run_config = {
        'start': False,
        'project': 'cloud_analytics',
        'secrets': {},
        'global_options': {},
    }
    ...
    mysql_token_name = vh.add_global_option('ktereshin_crm_mysql_password', 'secret', default='ktereshin_crm_mysql_password')
    yt_token_name = vh.add_global_option('ktereshin_yt_token', 'secret', default='ktereshin_yt_token')

    calls_from_mysql_to_yt(mysql_token_name, yt_token_name)
