...
def create_table_by_urls(conductor_url, day=None):
    if (day is None):
...
# while today.isoformat().split("-")[1] == past.isoformat().split("-")[1]:
#     today = past
#     days.append(today)
#     past = today - datetime.timedelta(days=1)
# for day in days:
#     loop = asyncio.get_event_loop()
#     future = asyncio.ensure_future(create_tables_async(day))
#     loop.run_until_complete(future)
