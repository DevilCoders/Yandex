...
from scoring_important_features import important_features
...
def get_count_vectors(bag_of_events):
    vectorizer = CountVectorizer(token_pattern=u'[^\t]+', min_df = 20)
...
def get_tfidf_vectors(bag_of_events):
    vectorizer = TfidfVectorizer(token_pattern=u'[^\t]+', min_df = 20)
    vectorizer.fit(list(bag_of_events['event']))
...
def main():
    ...
    cluster_yt.write('//home/cloud_analytics/scoring/learning_dataset', result[important_features])
    print('End upload data')
