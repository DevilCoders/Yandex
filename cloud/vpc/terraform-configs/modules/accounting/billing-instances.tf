resource "ycp_microcosm_instance_group_instance_group" "vpc_billing" {
  folder_id = var.folder_id

  name               = "billing-ig"
  description        = "Group of accounting services, used for filling billing topic"
  service_account_id = ycp_iam_service_account.yc_vpc_accounting_ig_sa.id
  labels             = {}

  instance_template {
    name = "vpc-billing-{instance.index_in_zone}-{instance.internal_dc}"

    description        = "Just one of billing accounting backends"
    hostname           = "{instance.index_in_zone}.{instance.internal_dc}.billing.${var.dns_zone}"
    ...
    network_interface {
      ...
      primary_v6_address {
        name = "ipv6"
        dns_record_spec {
          fqdn        = "{instance.index_in_zone}.{instance.internal_dc}.billing.${var.dns_zone}."
          ...
          files = [
            ...
            },
            {
              ...
              content = templatefile("${path.module}/files/vpc-accounting/config.yaml", {
                ...
                output_topic               = var.logbroker_billing_topic
                consumer                   = var.logbroker_billing_consumer
                loadbalancer_topic         = var.logbroker_loadbalancer_topic
                ...
                bill_elb_egress_traffic    = false
                bill_healtcheck_traffic    = false
                bill_unknown_balancers     = false
                bill_internal_balancers    = false
                compression_level          = 9
              })
            },
            {
              ...
              content = templatefile("${path.module}/files/vpc-accounting/balancer-metrics.yml", {
                ...
                output_channel = "multishard_metrics_pull_channel"
              })
            },
            {
  ...
  scale_policy {
    fixed_scale {
      size = var.accounting_instances_count
    }
  }
