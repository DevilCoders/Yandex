...
variable "startup_duration" {
  default     = 60 // After usual restart it still wait for this time
...
variable "services_proxy_version" {
  description = "Version of docker image with grpc-proxy"
}

variable "translation_server_version" {
  description = "Version of docker image with translation server"
}
...
variable "solomon_agent_version" {
  type    = string
  default = "2019-05-30T15-23"
}

variable "push_client_version" {
  type    = string
  default = "2020-01-01T18-32"
}
