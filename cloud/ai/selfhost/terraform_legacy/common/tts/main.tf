...
data "template_file" "podmanifest" {
  ...
  vars = {
    ...
    tts_server_port                = var.tts_server_port
  }
}
...
module "user_data" {
  ...
  extra_user_data = {
      write_files: [
          {
              path: "/etc/skm-bundle.yaml",
              content: module.skm_bundle.bundle
          }
      ]
  }
  extra_runcmd = [
      [ "bash", "-c", "/usr/bin/skm decrypt-bundle --bundle /etc/skm-bundle.yaml"]
  ]
}
