locals {
  default_env_vars = [
    ...
    },
    {
      ...
      value: var.environment == "preprod" ? "staging" : var.environment
    },
    {
      name = "ZOOKEEPER_SERVERS"
      value: join(",", [for s in module.constants.by_environment.zk_global_endpoints : format("%s:%s", s.host, s.port)])
    },
    {
      name = "LOG_LEVEL"
      value = "DEBUG"
  ...
  env_secrets = [
      ...
      {
          name = "SURROGATE_API_KEY"
          path: module.skm_bundle.path.surrogate-api-key
      }
  ]
...
module "solomon_config" {
    ...
    shard_service = var.solomon_shard_service
