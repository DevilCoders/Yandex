...
data "template_file" "configs" {
  ...
  vars = {
    ...
    asr_remote_nodes              = data.template_file.asr_remote_nodes.rendered
  }
}

data "template_file" "asr_remote_nodes" {
  template = file("${path.module}/files/asr_remote_nodes.json.tpl")

  vars = {
    remote_server                  = var.asr_remote_server
  }
}
...
data "template_file" "podmanifest" {
  ...
  vars = {
    ...
    redis_password                 = module.yav_secret_yc_ai_redis-password.secret
