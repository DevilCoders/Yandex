...
module "service_container" {
  ...
  repositories = {
    staging = "registry.yandex.net/cloud-ai/services-proxy"
    prod    = "registry.yandex.net/cloud-ai/services-proxy"
  }
...
locals {
  ...
  target_group_name_by_environment = {
    staging = ["ai-services-proxy-stt-preprod", null]
    prod    = ["ai-services-proxy-stt-prod", null]
  }
}
...
module "service" {
  ...
  # Service description
  name        = "speechkit-services-proxy"
  description = "SpeechKit Services Proxy"
  ...
  networks = [
    {
      ...
      dns = [{
        ...
        fqdn        = "speechkit-services-proxy-${var.environment}-{instance.index}.${module.well_known_dns_zones.ds_private_dns_zone.zone}"
      }]
    }
  ]
