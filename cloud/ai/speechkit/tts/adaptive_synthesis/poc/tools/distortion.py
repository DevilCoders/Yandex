...
def distort(samples: List[QualityEvaluationSample], ratio_samples_to_distort=0.2):
    ...
    for index in ids_distorted:
        audio = AudioSample(file=samples[index].path_to_reference)

        distortion_idx = np.random.randint(0, len(ALL_DISTORTIONS))
        distortion_type = ALL_DISTORTIONS[distortion_idx]

        if distortion_type not in group_by_distortion_samples:
            group_by_distortion_samples[distortion_type] = {'audios': [], 'samples': []}

        group_by_distortion_samples[distortion_type]['audios'].append(audio)
        group_by_distortion_samples[distortion_type]['samples'].append(samples[index])
    ...
    for distortion_type, values in group_by_distortion_samples.items():
        ...
        for new_audio, sample in zip(new_audios, current_samples):
            new_sample = sample.set_distorted_with_copy(new_audio, distortion_type)
            new_samples.append(new_sample)

    return QualityEvaluationPool(samples + new_samples)
