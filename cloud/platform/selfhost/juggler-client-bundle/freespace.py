...
def find_cfg_file():
    if os.path.isfile("/etc/api/configs/juggler_checks/freespace.conf"):
        return "/etc/api/configs/juggler_checks/freespace.conf"
    else:
        return "{}/freespace.conf".format(os.path.dirname(os.path.realpath(__file__)))

def main():
    status, message = [], []
    cfgpath = find_cfg_file()
    ...
    for partition in get_partitions():
        if partition in limits:
            ...
            absolute_check = check_limits(partition,
                                          convert2blocks(abs_limits.get('absolute_warn', "0M")),
                                          convert2blocks(abs_limits.get('absolute_crit', "0M")), False)
