...
OLD_MANAGEMENT_URL = "http://localhost:8081/plugins/json"
...
def get_solomon_agent_state():
    try:
        plugin_state = urllib2.urlopen(MANAGEMENT_URL).read()
    except urllib2.HTTPError as e:
        if e.code == 404:
            plugin_state = urllib2.urlopen(OLD_MANAGEMENT_URL).read()
        else:
            raise
