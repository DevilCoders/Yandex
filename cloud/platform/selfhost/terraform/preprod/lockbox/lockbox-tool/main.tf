...
module "yav-secret-sa-backup-access-key" {
  source     = "../../../modules/yav-secret"
  yt_oauth   = var.yandex_token
  id         = "sec-01esh1jmtmvanyp4vc3nsxgf8q"
...
data "template_file" "configs" {
  ...
  vars = {
    ...
    run_tool_sh = file("${path.module}/files/lockbox/run-tool.sh")
  }
}
...
module "lockbox-tool-instance-group" {
  source          = "../../../modules/kubelet_instance_group_ycp_v2"
  name_prefix     = "lockbox-tool"
  hostname_prefix = "lockbox-tool"
  hostname_suffix = var.hostname_suffix
  role_name       = "lockbox-tool"
