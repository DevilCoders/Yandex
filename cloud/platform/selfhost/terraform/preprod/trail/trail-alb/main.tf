...
resource "ycp_platform_alb_target_group" "trail_target_group" {
  name        = "trail-target-group-preprod"
  description = "Target Group for Cloud Trail ALB (preprod)"
  folder_id   = var.yc_folder
...
resource "ycp_platform_alb_backend_group" "trail_backend_group" {
  name        = "trail-backend-group-preprod"
  description = "Backend Group for Cloud Trail (preprod)"
  folder_id   = var.yc_folder
  labels      = {
    layer   = "paas"
    abc_svc = "yccloudtrail"
  ...
  grpc {
    backend {
      name = "trail-api-backend-preprod"
      port = 443
      weight = 100
