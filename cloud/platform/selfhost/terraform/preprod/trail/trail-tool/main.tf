...
data "template_file" "run_tool_sh" {
  template = "${file("${path.module}/files/trail/run-tool.sh")}"
...
module "tool-instance-group" {
  ...
  metadata = {
    ...
    run-tool-sh = templatefile("${path.module}/files/trail/run-tool.sh", {
      application_version = "${var.application_version}"
    })
