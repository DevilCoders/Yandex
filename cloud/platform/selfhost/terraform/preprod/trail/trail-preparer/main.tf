...
data "template_file" "application_yaml" {
  ...
  vars = {
    ...
    logbroker_destination_installation = "${var.logbroker_destination_installation}"
    logbroker_destination_topic_prefix = "${var.logbroker_destination_topic_prefix}"
    logbroker_cloud_logging_topic = "${var.logbroker_cloud_logging_topic}"
    logbroker_source_installation = "${var.logbroker_source_installation}"
    logbroker_source_topic = "${var.logbroker_source_topic}"
    logbroker_ingest_installation = "${var.logbroker_ingest_installation}"
    logbroker_ingest_topic = "${var.logbroker_ingest_topic}"
    logbroker_organizationmanager_source_topic = "${var.logbroker_organizationmanager_source_topic}"
    logbroker_storage_source_topic = "${var.logbroker_storage_source_topic}"
    logbroker_lockbox_data_source_topic = "${var.logbroker_lockbox_data_source_topic}"
    logbroker_source_consumer = "${var.logbroker_source_consumer}"
    logbroker_source_max_messages = "${var.logbroker_source_max_messages}"
    e2e_topic_pool_size = "${var.e2e_topic_pool_size}"
  }
}
...
data "template_file" "infra_configs" {
  ...
  vars = {
    solomon_agent_conf  = "${file("${path.module}/files/solomon-agent/solomon-agent.conf")}"
...
module "preparer-instance-group" {
  ...
  metadata = {
    ...
    solomonagent-config = "${file("${path.module}/files/solomon-agent/solomon-agent.conf")}"
    ...
    solomon-agent-pod = templatefile("${path.module}/files/solomon-agent/solomon-agent-pod.tpl.yaml", {
      solomon_version = "${var.solomon_agent_image_version}"
    })
