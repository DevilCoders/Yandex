...
data "template_file" "configs" {
  ...
  vars = {
    ...
    run_ydb_dumper_sh = file("${path.module}/files/certificate-manager/run-ydb-dumper.sh")
  }
}

data "template_file" "infra_configs" {
  template = file("${path.module}/files/certificate-manager/infra-configs.tpl")
...
module "certificate-manager-ydb-dumper-instance-group" {
  source          = "../../../modules/kubelet_instance_group_ycp_v2"
  name_prefix     = "ydb-dumper"
  hostname_prefix = "dumper"
