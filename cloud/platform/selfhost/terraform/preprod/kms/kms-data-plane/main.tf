...
locals {
  ...
  "kms-data-${lookup(module.common.yc_zone_suffix, element(var.custom_yc_zones, count))}-${floor(count / length(var.custom_yc_zones)) + count % length(var.custom_yc_zones) - index(var.custom_yc_zones, element(var.custom_yc_zones, count)) + 1}.${module.common.hostname_suffix}"
  ]
