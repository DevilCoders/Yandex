...
resource "ycp_certificatemanager_certificate_request" "trail_ingest_certificate_internal" {
  ...
  domains = ["ingest.trail.cloud.yandex.net"]
  description = "Internal certificate for Cloud Trail Ingest API ALB (prod)"
  folder_id   = var.yc_folder
  cert_provider = "INTERNAL_CA"
}

resource "ycp_platform_alb_target_group" "trail_ingest_target_group" {
  name        = "trail-ingest-target-group-prod"
  description = "Target Group for Cloud Trail Ingest API ALB (prod)"
  folder_id   = var.yc_folder
...
resource "ycp_platform_alb_backend_group" "trail_ingest_backend_group" {
  name        = "trail-ingest-backend-group-prod"
  description = "Backend Group for Cloud Trail Ingest API (prod)"
  folder_id   = var.yc_folder
  labels      = {
    layer   = "paas"
    abc_svc = "yccloudtrail"
  ...
  grpc {
    backend {
      name = "trail-ingest-api-backend-prod"
      port = 443
      backend_weight = 100
...
resource ycp_platform_alb_load_balancer trail_ingest_alb_load_balancer {
  ...
  solomon_cluster_name = "trail_ingest_prod_alb-l7"

  # Хост для алертов в juggler.
  juggler_host = "cloud_prod_l7-trail_ingest_alb"
}
