  ...
  mqtt_instances = [
    {
      "zone" = "vla"
      "ipv4" = "172.16.0.41"
      "ipv6" = "2a02:6b8:c0e:500:0:f803:0:16f"
    },
    {
      "zone" = "sas"
      "ipv4" = "172.17.0.25"
      "ipv6" = "2a02:6b8:c02:900:0:f803:0:103"
    },
    {
      "zone" = "myt"
      "ipv4" = "172.18.0.25"
      "ipv6" = "2a02:6b8:c03:500:0:f803:0:21e"
    },
  ]
...
data "template_file" "mqtt_name" {
  ...
  template = "$${name}"
  vars = {
    name = "${local.mqtt_name_prefix}-${local.environment}-${local.mqtt_instances[count.index][local.zone_key]}${format("%02d", floor(1 + (count.index / length(var.zone_suffix))))}"
  }
}
...
data "template_file" "mqtt_yaml" {
  ...
  vars = {
    tvm_token = module.yav-secret-tvm_token.value
    mqtt_id   = count.index
    mqtt_name = data.template_file.mqtt_name[count.index].rendered
    zone      = local.mqtt_instances[count.index][local.zone_key]
...
data "template_file" "tvmtool_conf" {
  template = file("${path.module}/files/mqtt/tvmtool.tpl.conf")

  vars = {
    secret    = module.yav-secret-tvm_secret.value
    client_id = module.yav-secret-tvm_client_id.value
  }
}
...
data "template_file" "mqtt_manifest" {
  ...
  vars = {
    ...
    tvmtool_auth_token = module.yav-secret-tvm_token.value
...
resource "yandex_compute_disk" "mqtt_disk" {
  ...
  type  = "network-hdd"
  zone = local.zones[local.mqtt_instances[count.index][local.zone_key]]
...
resource "yandex_compute_instance" "mqtt-instance-group" {
  ...
  network_interface {
    subnet_id    = local.subnets[local.mqtt_instances[count.index][local.zone_key]]
    ipv6         = "true"
    ip_address   = local.mqtt_instances[count.index][local.ipv4_key]
    ipv6_address = local.mqtt_instances[count.index][local.ipv6_key]
  }
  ...
  labels = {
    ...
    conductor-dc         = local.mqtt_instances[count.index][local.zone_key]
  }
}
