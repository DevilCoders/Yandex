resource ycp_platform_alb_backend_group k8sapi_l7_bg {
  name        = "mk8s-private-api-backend-group-prod"
  description = "Backend for mk8s-private-api service (prod)"

  grpc {
    backend {
      name   = "k8s-api-backend-preprod-ii"
      weight = 100
      port   = 443
      target_group {
...
resource ycp_platform_alb_backend_group k8sapi_proxy_l7_bg {
  name        = "k8s-api-proxy-backend-prod"
