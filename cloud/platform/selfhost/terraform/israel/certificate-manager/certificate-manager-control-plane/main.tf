...
data "template_file" "application_yaml" {
  ...
  vars = {
    ...
    api_port          = local.backend_private_api_port
    system_kms_key_id = var.system_kms_key_id
  }
}
...
module "api-locallb" {
  ...
  service_id              = "yandex.cloud.certificatemanager.v1.CertificateService"
...
data "template_file" "podmanifest" {
  ...
  vars = {
    ...
    tool_version        = module.common.application_version
    java_pod_memory     = var.java_pod_memory
    java_heap_memory    = var.java_heap_memory
    tool_pod_memory     = var.tool_pod_memory
    tool_heap_memory    = var.tool_heap_memory

    api_podmanifest     = module.api-locallb.api_podmanifest
  }
}

module "certificate-manager-cpl-instance-group" {
  ...
  placement_group_id   = module.common.cpl-placement-group-id
...
resource "ycp_vpc_inner_address" "cpl-load-balancer-address" {
  name = "cpl-address"
...
resource "ycp_load_balancer_target_group" "cpl-load-balancer-tg" {
  name      = "cpl-tg"
  ...
  dynamic "target" {
    ...
    content {
      address   = element(module.certificate-manager-cpl-instance-group.all_instance_ipv6_addresses, count.value)
...
resource "ycp_load_balancer_network_load_balancer" "cpl-load-balancer" {
  name      = "cpl-lb"
  region_id = module.common.yc_region
  type      = "EXTERNAL"
  ...
  attached_target_group {
    ...
    target_group_id = ycp_load_balancer_target_group.cpl-load-balancer-tg.id
  }
  ...
  listener_spec {
    ...
    external_address_spec {
      ip_version  = "IPV6"
      yandex_only = false
      address     = ycp_vpc_inner_address.cpl-load-balancer-address.address
    }
    port        = var.api_port
...
resource "ycp_dns_dns_record_set" "cpl-load-balancer-fqdn" {
  name    = var.endpoint_name
  zone_id = module.common.dns_zone_id
  type    = "AAAA"
  ttl     = 300
  data    = [ycp_vpc_inner_address.cpl-load-balancer-address.address]
}

// public balancer

resource "ycp_vpc_address" "cpl-load-balancer-v4-address" {
...
resource "ycp_load_balancer_target_group" "cpl-load-balancer-v4-tg" {
  name      = "cpl-v4-tg"
  region_id = module.common.yc_region
  ...
  dynamic "target" {
    ...
    content {
      address   = element(module.certificate-manager-cpl-instance-group.all_instance_ipv4_addresses, count.value)
      subnet_id = lookup(module.common.subnets, element(module.common.yc_zones, count.value))
    }
  }
...
resource "ycp_load_balancer_network_load_balancer" "cpl-v4-load-balancer" {
  name      = "cpl-v4-lb"
  region_id = module.common.yc_region
  type      = "EXTERNAL"
  ...
  attached_target_group {
    ...
    target_group_id = ycp_load_balancer_target_group.cpl-load-balancer-v4-tg.id
  }
  ...
  listener_spec {
    ...
    external_address_spec {
      ip_version  = "IPV4"
      yandex_only = false
      address     = ycp_vpc_address.cpl-load-balancer-v4-address.external_ipv4_address[0].address
    }
    port        = var.api_port
    target_port = var.api_port
    protocol    = "TCP"
...
resource "ycp_dns_dns_record_set" "cpl-public-v4-load-balancer-fqdn" {
  name    = var.public_endpoint_name
  zone_id = module.common.dns_zone_id
  type    = "A"
  ttl     = 300
  data    = [ycp_vpc_address.cpl-load-balancer-v4-address.external_ipv4_address[0].address]
}

// public ipv6 balancer

//resource "ycp_vpc_address" "cpl-load-balancer-v6-address" {
//  lifecycle {
//    prevent_destroy = true
//    ignore_changes  = [reserved, labels]
//  }
//
//  description = "cpl-v6-address"
//  name        = "cpl-v6-address"
//  folder_id   = module.common.folder_id
//
//  ipv6_address_spec {
