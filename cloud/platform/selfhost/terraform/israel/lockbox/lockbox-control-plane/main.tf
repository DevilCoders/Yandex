...
data "template_file" "application_yaml" {
  ...
  vars = {
    api_port           = local.backend_private_api_port
    default_kms_key_id = var.default_kms_key_id
...
module "api-locallb" {
  ...
  zones                   = module.common.yc_zones
  id_prefix               = var.id_prefix
  operation_service_id    = "yandex.cloud.priv.lockbox.v1.OperationService"
  service_id              = "yandex.cloud.lockbox.v1.SecretService"
...
module "lockbox-cpl-instance-group" {
...
resource "ycp_load_balancer_target_group" "cpl-load-balancer-tg" {
  ...
  dynamic "target" {
    ...
    content {
      address   = element(module.lockbox-cpl-instance-group.all_instance_ipv6_addresses, count.value)
...
resource "ycp_load_balancer_target_group" "cpl-load-balancer-v4-tg" {
  ...
  dynamic "target" {
    ...
    content {
      address   = element(module.lockbox-cpl-instance-group.all_instance_ipv4_addresses, count.value)
      subnet_id = lookup(module.common.subnets, element(module.common.yc_zones, count.value))
    }
  }
