...
module "api-locallb" {
  ...
  service_id              = "yandex.cloud.kms.v1.SymmetricKeyService"
...
data "template_file" "application_yaml" {
  ...
  vars = {
    private_api_port = local.backend_private_api_port
    id_prefix        = var.id_prefix
  }
}
...
resource "ycp_load_balancer_target_group" "cpl-load-balancer-tg" {
  ...
  dynamic "target" {
    ...
    content {
      address   = element(module.kms-cpl-instance-group.all_instance_ipv6_addresses, count.value)
...
resource "ycp_load_balancer_network_load_balancer" "cpl-load-balancer" {
  ...
  listener_spec {
    ...
    external_address_spec {
      ip_version  = "IPV6"
      yandex_only = false
      address     = ycp_vpc_address.cpl-load-balancer-v6-address.ipv6_address[0].address
    }
    port        = var.api_port
...
resource "ycp_load_balancer_target_group" "cpl-load-balancer-v4-tg" {
  ...
  dynamic "target" {
    ...
    content {
      address   = element(module.kms-cpl-instance-group.all_instance_ipv4_addresses, count.value)
      subnet_id = lookup(module.common.subnets, element(module.common.yc_zones, count.value))
    }
  }
