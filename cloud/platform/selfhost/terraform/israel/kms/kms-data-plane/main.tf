...
locals {
  kms_addrs = [
  for hostname in module.hostnames.all_instance_group_fqdns :
  "${hostname}:${local.backend_private_api_port}"
  ]
}
...
module "api-locallb" {
  ...
  service_id              = "yandex.cloud.kms.v1.SymmetricCryptoService"
...
resource "ycp_load_balancer_target_group" "dpl-load-balancer-tg" {
  ...
  dynamic "target" {
    ...
    content {
      address   = element(module.kms-dpl-instance-group.all_instance_ipv6_addresses, count.value)
...
resource "ycp_load_balancer_network_load_balancer" "dpl-load-balancer" {
  ...
  listener_spec {
    ...
    external_address_spec {
      ip_version  = "IPV6"
      yandex_only = false
      address     = ycp_vpc_address.dpl-load-balancer-v6-address.ipv6_address[0].address
    }
    port        = var.api_port
...
resource "ycp_load_balancer_target_group" "dpl-load-balancer-v4-tg" {
  ...
  dynamic "target" {
    ...
    content {
      address   = element(module.kms-dpl-instance-group.all_instance_ipv4_addresses, count.value)
      subnet_id = lookup(module.common.subnets, element(module.common.yc_zones, count.value))
    }
  }
...
resource "ycp_vpc_address" "dpl-load-balancer-v6-address" {
  lifecycle {
    prevent_destroy = true
    ignore_changes  = [reserved, labels]
  }

  description = "dpl-v6-address"
  name        = "dpl-v6-address"
  folder_id   = module.common.folder_id
...
resource "ycp_dns_dns_record_set" "dpl-public-v6-load-balancer-fqdn" {
  name    = var.public_endpoint_name
  zone_id = module.common.dns_zone_id
  type    = "AAAA"
  ttl     = 300
  data    = [ycp_vpc_address.dpl-load-balancer-v6-address.ipv6_address[0].address]
}
