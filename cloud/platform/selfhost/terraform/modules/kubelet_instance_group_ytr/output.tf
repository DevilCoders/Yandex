...
output "all_instance_group_fqdns" {
  value = [
    "${yandex_compute_instance.node.*.hostname}",
  ]
}

output "all_instance_group_names" {
  value = [
...
output "all_instance_ids" {
  value = [
    "${yandex_compute_instance.node.*.id}",
  ]
}
...
output "instances_fqdn_ipv4" {
  value = [
    "${formatlist("%s -- %s", yandex_compute_instance.node.*.fqdn, yandex_compute_instance.node.*.network_interface.0.ip_address)}",
  ]
}
