...
if [[ "$UPSTREAM" == "true" ]]; then
    ...
    UPSTREAM_FILTER="grep \"\\\"authority\\\":\\\"serverless-functions.private-api.ycp.\" | "
fi
...
$(dirname "$0")/ycf-cpl.sh "${CAT} | ${UPSTREAM_FILTER}${COMMAND} | ${HOSTNAME_TO_LOG}" | eval $(jq_sort_by_timestamp)
