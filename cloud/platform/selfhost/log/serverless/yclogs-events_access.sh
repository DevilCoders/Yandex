...
if [[ "$UPSTREAM" == "true" ]]; then
    ...
    UPSTREAM_FILTER="grep \"\\\"authority\\\":\\\"log-events.private-api.ycp.\" | "
fi
...
$(dirname "$0")/yclogs-events.sh "${CAT} | ${UPSTREAM_FILTER}${COMMAND} | ${HOSTNAME_TO_LOG}" | eval $(jq_sort_by_timestamp)
