...
if [[ -z "$@" ]]; then
    LAST_REQUEST_ID=$(PROFILE=${PROFILE} JQ_TSV=.request_id $(dirname "$0")/api-adapter_access.sh | tail -n 1)
...

{
    ...
    PROFILE=${PROFILE} JQ_DISABLE=true UPSTREAM=true $(dirname "$0")/api-adapter_access.sh ${COMMAND} ;
