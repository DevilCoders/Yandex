...
if [[ -z "$@" ]]; then
    LAST_REQUEST_ID=$(PROFILE=${PROFILE} JQ_TSV=.request_id $(dirname "$0")/instance-groups_access.sh grep request_id | tail -n 1)
...

{
    ...
    PROFILE=${PROFILE} JQ_DISABLE=true UPSTREAM=true $(dirname "$0")/../cpl-router_access.sh ${COMMAND} ;
    PROFILE=${PROFILE} JQ_DISABLE=true UPSTREAM=true $(dirname "$0")/instance-groups_access.sh ${COMMAND} ;
