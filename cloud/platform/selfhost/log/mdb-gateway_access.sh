...
$(dirname "$0")/mdb-gateway.sh "cat /var/log/fluent/access_log.api-gateway.${DAY//-/}.log | grep \"^{\" | ${HEALTH_CHECK_FILTER}${UNAVAILABLE_FILTER}${UPSTREAM_FILTER}${COMMAND} | ${HOSTNAME_TO_LOG}" | eval $(jq_sort_by_timestamp)
