...
inline void TestCase_MakeCaseInsensitive(WC_TYPE t) {
    ...
    for (size_t i = 0; i < lowerSet.length(); ++i) {
        UNIT_ASSERT_C(us.Has(lowerSet[i]), ",char='" << WideToUTF8(lowerSet.data() + i, 1) << "'");
    }
    for (size_t i = 0; i < upperSet.length(); ++i) {
        UNIT_ASSERT_C(us.Has(upperSet[i]), ",char='" << WideToUTF8(upperSet.data() + i, 1) << "'");
    }
    for (size_t i = 0; i < nonletterSet.length(); ++i) {
        UNIT_ASSERT_C(!us.Has(nonletterSet[i]), ",char='" << WideToUTF8(nonletterSet.data() + i, 1) << "'");
    }
...
Y_UNIT_TEST_SUITE(UnicodeSet) {
    ...
    Y_UNIT_TEST(SetString) {
        ...
        for (size_t i = 0; i < strSet.length(); ++i) {
            UNIT_ASSERT_C(us.Has(strSet[i]), ",char='" << WideToUTF8(strSet.data() + i, 1) << "'");
        }
