...
int main() {
    ...
    TSkyShareManagerConfig config;
    config.TvmSecret = GetEnv("TVM_SECRET");
    Y_ENSURE(GetEnv("SRC_TVM_ID"));
    config.SrcTvmId = FromString<int>(GetEnv("SRC_TVM_ID"));
    TSkyShareManager manager{config};

    manager.DeleteSkyShare(ExpectedRBTorrent);

    auto share = manager.MakeSkyShare();
...
//    file1->Finish();
    ...
    manager.RegisterSkyShareAsync(std::move(share));
    manager.FinishAllAsyncShares();

    Cerr << "torrent registered\n";

    Y_ENSURE(rbtorrent == ExpectedRBTorrent);
