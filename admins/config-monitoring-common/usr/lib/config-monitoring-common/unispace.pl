...
sub dfLinux
{
        open(DF, "sudo df -a -P -l -k -t ext2 -t ext3 -t ext4 -t xfs -t jfs -t simfs -t tmpfs 2>/dev/null |") || die "df: $!";
        $_ = <DF>;
        while (<DF>)
        {
                chomp;
        ...
        open(DF, "sudo df -a -P -l -k -i -t ext2 -t ext3 -t ext4 -t xfs -t jfs -t simfs -t tmpfs 2>/dev/null |") || die "df: $!";
        $_ = <DF>;
        while (<DF>)
        {
                chomp;
