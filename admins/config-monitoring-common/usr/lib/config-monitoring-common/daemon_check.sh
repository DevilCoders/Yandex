...
check_daemon_regular(){
	daemon_regular=$(ps -e -o state,cmd | egrep -v '^Z|awk|grep|monrun' | grep -v "$SCRIPT_NAME" | grep -c "$DAEMON");
...
check_daemon_dom0(){
	daemon_dom0=$(for n in $(ps waux | egrep -v 'grep|daemon_check|monrun' |grep $DAEMON 2>/dev/null |awk '{print $2}');
			do
