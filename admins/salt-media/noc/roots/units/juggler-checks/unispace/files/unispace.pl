...
$defaultlimitcrit = (defined $crit{'limit'}) ? $crit{'limit'} : 0.91;
...
sub dfLinux
{
        my $fs_types = " -t ext2 -t ext3 -t ext4 -t xfs -t jfs -t simfs -t tmpfs -t zfs ";
        open(DF, "sudo df -a -P -l -k ".$fs_types." 2>/dev/null |") || die "df: $!";
        $_ = <DF>;
        while (<DF>)
        {
                chomp;
                @F = split;
                $name = $F[5];
                if (defined $exclude_pattern and $name =~ /$exclude_pattern/)
                {
                        next;
                }
        ...
        open(DF, "sudo df -a -P -l -k -i ".$fs_types." 2>/dev/null |") || die "df: $!";
        $_ = <DF>;
        while (<DF>)
        {
                chomp;
...
sub dfBSD
{
        open(DF, "df -k -i -t ufs,zfs,tmpfs,unionfs 2>/dev/null |") || die "df: $!";
        $_ = <DF>;
        while (<DF>)
        {
                chomp;
...
sub Netsaint 
{
        ...
        print "PASSIVE-CHECK:$serv;$state;$mesg\n";
}
