...
{{ salt['pillar.get']('cassandra:config:env:max_heap_size') }}
{{ salt['pillar.get']('cassandra:config:env:heap_newsize') }}
...
# clear JVM_OPTS to avoid duplications
JVM_OPTS=""
...
{{ salt['pillar.get']('cassandra:config:env:enable_ipv4_prefer') }}
...
JVM_EXTRA_OPTS="{{ salt.pillar.get('cassandra:config:env:extra_opts', [])|join(' ')}}"

if ! [[ "$JVM_EXTRA_OPTS" =~ metricsReporterConfigFile ]];then
    JVM_EXTRA_OPTS="$JVM_EXTRA_OPTS -Dcassandra.metricsReporterConfigFile=/etc/cassandra/report.yaml"
fi
