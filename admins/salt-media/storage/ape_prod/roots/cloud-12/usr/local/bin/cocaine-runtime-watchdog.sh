...
then
	...
	echo `date` too many open files `ubic status cocaine-runtime | grep -o "[0-9]*" | xargs lsof -p | wc -l` >> /var/log/cocaine-runtime-watchdog.log
        #/bin/bash /usr/local/bin/cocaine-graceful-restart.sh >> /var/log/cocaine-runtime-watchdog.log
    ...
    then
        ...
        if [ `cocaine-tool i | grep depth | wc -l` -le 8 ]
        then
            echo `date` `ubic status cocaine-runtime | grep -o "[0-9]*"` bad runlist >> /var/log/cocaine-runtime-watchdog.log
            ubic restart cocaine-runtime
            #/bin/bash /usr/local/bin/cocaine-graceful-restart.sh >> /var/log/cocaine-runtime-watchdog.log
            exit 0
        fi
