...
/etc/cocaine/mecoll.yaml:
  file.managed:
    - source: salt://front-12/etc/cocaine/mecoll.yaml
    - makedirs: True

/etc/cocaine-http-proxy/cocaine-http-proxy.yml:
  file.managed:
    - contents_pillar: yavape:cocaine.cocaine-http-proxy.yml
    - makedirs: True
...
/etc/nginx/include/imageparser_ratelimits.conf:
  file.managed:
    - source: salt://front-12/etc/nginx/include/imageparser_ratelimits.conf_{{ salt['grains.get']('conductor:root_datacenter') }}
    - makedirs: True
...
/usr/local/bin/:
  file.recurse:
    - source: salt://front-12/usr/local/bin/
    - file_mode: '0755'
...
/etc/cron.d/:
  file.recurse:
    - source: salt://front-12/etc/cron.d/

#/etc/init.d/:
#  file.recurse:
#    - source: salt://front-12/etc/init.d/

/etc/nginx/sites-enabled:
  file.recurse:
    - source: salt://front-12/etc/nginx/sites-enabled/

/etc/cocaine/cocaine.conf:
  file.managed:
    - source: salt://front-12/etc/cocaine/cocaine.conf
    - template: jinja
...
/etc/nginx/common:
  file.recurse:
    - source: salt://front-12/etc/nginx/common/
...
/etc/nginx/nginx.conf:
  file.managed:
    - source: salt://front-12/etc/nginx/nginx.conf

/etc/nginx/conf.d/01-accesslog.conf:
  file.managed:
    - source: salt://front-12/etc/nginx/conf.d/01-accesslog.conf
...
/etc/cocaine-tornado-proxy/cocaine-tornado-proxy-production.conf:
  file.managed:
    - contents_pillar: yavape:cocaine.cocaine-tornado-proxy-production.conf
    - makedirs: True

/etc/cocaine-tornado-proxy/srwconfig.conf:
  file.managed:
    - source: salt://front-12/etc/cocaine-tornado-proxy/srwconfig.conf

/etc/default/:
  file.recurse:
    - source: salt://front-12/etc/default/
