...
Update lxd profiles:
  cmd.run:
    - name: find /etc/yandex/lxd/profiles -type f | while read line; do name=$(echo $line | awk -F\/ '{print $NF}'); lxc_name=$({{lxc_binary}} profile list | awk -v name=$name '$2==name {print $2}'); if [[ "$name" != "$lxc_name" ]]; then {{lxc_binary}} profile create $name ; fi; cat $line | {{lxc_binary}} profile edit $name ; done
    - onchanges:
      {% if lxd.projects %}
      {% for project in lxd.projects %}
      {% if project.name %}
