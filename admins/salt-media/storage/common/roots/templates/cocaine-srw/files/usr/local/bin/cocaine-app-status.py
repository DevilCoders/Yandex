...
def main():
    ...
    for app_name in app_list:
        ...
        try:
            ...
            print "2; %s app is not running" % app_name
            exit(1)

        app_exclude = re.match('(imageparse|ocraas|iexlib-srw|tikaite-ml|tikaite-srw).*', app_name)

        if state == "broken":
            app_broken += "{0}: broken, ".format(app_name)
        elif float(depth)/capacity > 0.9 and not app_exclude:
