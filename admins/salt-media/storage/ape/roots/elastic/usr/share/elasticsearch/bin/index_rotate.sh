...
if [ -n "${NODE_ID}${MASTER_ID}" ] ; then
	if [[ "$NODE_ID" == "$MASTER_ID" ]] ; then
		if [[ "$KEEP_DAYS" -lt 1 && "$KEEP_HOURS" -lt 1 ]] ; then
			...
			for i in `curl -s -XGET localhost:9200/_cat/indices|awk "{if(\\$3~/^${INDEX_PREFIX}/){print \\$3}}"|sed -n "/^${INDEX_PREFIX}[[:digit:]]\{4\}[._-]\{1\}[[:digit:]]\{2\}[._-]\{1\}[[:digit:]]\{2\}\([t._:-]\{1\}[[:digit:]]\{2\}\)\{0,1\}$/ p"|sort -n ` ; do
				CUR_UTS=$( date -d "`echo $i|sed -n -e 's/^[[:graph:]]\+\([[:digit:]]\{4\}[._-]\{1\}[[:digit:]]\{2\}[._-]\{1\}[[:digit:]]\{2\}\)\([t._:-]\{1\}[[:digit:]]\{2\}\)\{0,1\}$/\1T\2/p'|sed -e 's/T$/T00/' -e 's/T[t._:-]\{1\}\([[:digit:]]\{2\}\)$/T\1/' -e 's/\./-/g' -e 's/_/-/g'`:00:00,000000-0000" "+%s" )
