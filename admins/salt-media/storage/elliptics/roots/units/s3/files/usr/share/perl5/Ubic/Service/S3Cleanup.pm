package Ubic::Service::S3Cleanup;
...
sub new {
    ...
    my $params = validate_with(
        ...
        spec   => {
            ...
            run_dir => { type => SCALAR, default => "/var/run/s3-goose-cleanup", optional => 1 },
        },
        allow_extra => 1,
    );
    ...
    $params->{bin} = "/usr/bin/s3-goose-cleanup -c $daemon_config";
...
sub status_impl {
    ...
    my $broke = 0;
    for my $attempt ( 1..50 ) {
        my $ping_url = URI::URL->new("http://localhost:3355/$ping_uri");
...
Ubic::Service::S3Cleanup - run S3-cleanup worker's
