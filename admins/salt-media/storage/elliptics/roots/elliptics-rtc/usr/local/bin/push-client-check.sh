...
dlog(){
   ...
   lineno=`caller 0`
   printf -- "$ts: ${o}line #%-4s ${g}%-8s$w: !l}m$@$w\n" ${lineno% *}
}

#DAEMON_CONF="/etc/yandex/statbox-push-client/push-client.yaml"
DAEMON_CONF="/etc/yandex/statbox-push-client/cocaine-test-backend.yaml /etc/yandex/statbox-push-client/disk.yaml /etc/yandex/statbox-push-client/mds_cleanup.yaml /etc/yandex/statbox-push-client/rt.yaml"
declare -A _ERRORS
TS=`date +%s`
