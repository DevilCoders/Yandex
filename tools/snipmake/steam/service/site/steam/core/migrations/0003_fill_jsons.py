...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for est in orm['core.Estimation'].objects.all():
            est.json_value = dict(zip(
                Estimation.Criterion.NAMES,
                self.digits(est.value)
            ))
            est.save()
    ...
    def digits(self, value):
        base = 3
        unpacked = []
        for i in range(len(Estimation.Criterion.NAMES)):
            (value, digit) = self.div_operation_shifted(value, base)
            unpacked.append(digit)
        return unpacked

    def div_operation_shifted(self, value, base):
        div = value // base
        mod = value - div * base
        if base - 2 * mod < 0:
            mod -= base
