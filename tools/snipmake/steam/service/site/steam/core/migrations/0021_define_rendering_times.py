...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm['core.Estimation'].objects.filter(
            r_time__lt=F('start_time')
        ).update(
            rendering_time=F('r_time'),
        )
        orm['core.Estimation'].objects.filter(
            r_time__gte=F('start_time')
        ).update(
            rendering_time=F('start_time'),
        )
        orm['core.Estimation'].objects.filter(
            r_time__gt=F('start_time'),
