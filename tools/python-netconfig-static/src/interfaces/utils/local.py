...
from ctypes import (
    ...
    pointer, cast, c_ushort, c_byte, c_void_p, c_char_p, c_uint, c_uint16, c_uint32,
)
import ctypes.util
import ctypes
import os
...
def get_first_uplink_name():
    ...
    links = [link for link in os.listdir(
        '/sys/class/net') if link.count('eth') or link.count('enx')]
    for link in links:
        try:
