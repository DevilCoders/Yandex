...
class BalancerIfaceGenerator(object):
    def generate(self, parameters, iface_utils, network_info, interfaces_group):
        ...
        if ip['ip4']:
            ...
            addresses['ip4'] = sorted(
                list(set(addresses['ip4'])), cmp=_compare_ip4)
        ...
        if addresses['ip6']:
            ...
            if filtered:
                ...
                actions = self.postup_for_ip6_balancers(
                    parameters.default_iface, network_info, addresses['ip6'])
        ...
        if addresses['ip4']:
            if 'ipvs_internal' in parameters.host_tags:  # ADMINTOOLS-354
                ...
                mtu = 1450
            interfaces_group.add_interfaces(self.generate_balancer_interfaces(
                addresses['ip4'], mtu, parameters.default_iface, network_info))
            if not parameters.no_antimartians:
                interfaces_group.add_interfaces(self.generate_loopbacks(
                    parameters.fqdn, iface_utils, network_info, interfaces_group))
    ...
    def generate_balancer_interfaces(self, aliases, mtu, default_iface, network_info):
        ...
        for address in aliases:
            ...
            postup.append(Route().replace().ip4().default().via(gate).dev(
                default_iface).src(iface['address']).mtu(mtu).table(iface_num))
            postup.append(Rule().add().ip4().from_addr(
                iface['address']).lookup(iface_num).priority(iface_num))
            predown.append(Rule().delete().ip4().from_addr(
                iface['address']).lookup(iface_num).priority(iface_num))
            predown.append(Route().delete().ip4().default().via(gate).dev(
                default_iface).src(iface['address']).mtu(mtu).table(iface_num))
            iface['postup'] = postup
    ...
    def generate_loopbacks(self, fqdn, iface_utils, network_info, interfaces):
        ...
        for l in loopbacks:
            ...
            if not vip_fqdn:
                vip_fqdn = l['vip']
            iface.add_comment('dummy: ' + vip_fqdn + ':' +
                              str(l['vs_port']) + ' ' + l['protocol'] + ', ' + l['comment'])
    ...
    def postup_for_ip6_balancers(self, main_iface, network_info, aliases):
        ...
        for address in aliases:
            ...
            netw = "%s/%s" % (network_info.get_net_from_ip(
                net['ip'], net['mask']), net['mask'])
            postup.append(Address().ip6().add().address(cidr).dev(main_iface))
            postup.append(Rule().ip6().add().from_addr(net['ip']).table(table))
            postup.append(Route().ip6().add().route(
                netw).dev(main_iface).table(table))
            postup.append(Route().ip6().add().default().via(
                network_info.get_ipv6_gw_from_cidr(cidr)).table(table))
            table += 1
        predown = []
        if postup:
            predown.append(
    ...
    def generate_loopback_postup_ip6(self, iface_utils, network_info, fqdn, interfaces):
        ...
        for l in loopbacks:
            ...
            comment = "dummy6: %s:%s %s, %s" % (
                vip_fqdn, l['vs_port'], l['protocol'], l['comment'])
            result['postup'].append(Address().ip6().add().address(
                vip + "/128").dev("lo").comment(comment))
            result['predown'].append(Address().ip6().delete().address(
                vip + "/128").dev("lo").comment(comment))
        return result
