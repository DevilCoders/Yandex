...
class FastboneIfaceGenerator(object):
    ...
    def generate(self, parameters, iface_utils, network_info, interfaces_group):
        fastbone_params = dict(iface=parameters.default_iface,
        ...
        fastbone_fqdns = [
            'fb.' + parameters.fqdn,
            'fb-' + parameters.fqdn,
            re.sub(r'yandex\.(ru|net)', 'fb.\\g<0>', parameters.fqdn),
            re.sub(r'(market|vertis)\.yandex\.net',
                   'fb.\\g<0>', parameters.fqdn),
            re.sub(r'\.storage\.yandex\.net', '-fb\\g<0>', parameters.fqdn),
        ...
        for fb_fqdn in fastbone_fqdns:
            ...
            fb_ifaces = self.generate_fastbone_interface(
                network_info, fastbone_params, main_ips)
            if fb_ifaces:
                break
        ...
        if fb_ifaces:
            ...
            main_ipv6 = filter(lambda i: i.name == parameters.default_iface and i.address == main_ips['ip6'],
                               interfaces_group)
    ...
    def generate_fastbone_interface(self, network_info, opts, main_ips):
        ...
        for additional_ip in add_ips:
            dummy = {'ip4': None, 'ip6': None,
                     'ip%d' % additional_ip[0]: additional_ip[1]}
            ...
            if nets['net4']:
                ...
                if current_opts['tagged_vlan']:
                    ...
                    self.fastbone_vlans.append(
                        ('vlan' + nets['net4']['vlan'], current_opts['name']))
            ...
            if nets['net6']:
                ...
                if current_opts['tagged_vlan']:
                    ...
                    self.fastbone_vlans.append(
                        ('vlan' + nets['net6']['vlan'], current_opts['name']))
